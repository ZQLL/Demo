...
public abstract class AceHxzq_fpxxPubServiceImpl {
    //新增
    public FpxxBillVO[] pubinsertBills(FpxxBillVO[] vos) throws BusinessException {
        try {
          // 数据库中数据和前台传递过来的差异VO合并后的结果
          BillTransferTool<FpxxBillVO> transferTool =new BillTransferTool<FpxxBillVO>(vos);
    ...
    public void pubdeleteBills(FpxxBillVO[] vos) throws BusinessException {
  	  ...
  	  //修改
    public FpxxBillVO[] pubupdateBills(FpxxBillVO[] vos) throws BusinessException {
        try {
    	      //加锁 + 检查ts
		          BillTransferTool<FpxxBillVO> transTool = new BillTransferTool<FpxxBillVO>(vos);
		          //补全前台VO
  ...
  public FpxxBillVO[] pubquerybills(IQueryScheme queryScheme)
    ...
    try {
      ...
      BillLazyQuery<FpxxBillVO> query =
          new BillLazyQuery<FpxxBillVO>(FpxxBillVO.class);
