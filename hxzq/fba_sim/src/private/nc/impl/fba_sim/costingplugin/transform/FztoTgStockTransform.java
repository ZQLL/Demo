...
import nc.impl.fba_sim.costplugin.stock.FzmrYjlxUtils;
...
/**
 * 负载转托管 ：两种情况：
 *1、负债转换时 ,一个同转出同负债买入，转入同负债卖出，需要按份额占比计算出转出负债的应收计提利息。
 * 2、融入转换时，一个同融入，一个同融出
 * 
 */
public class FztoTgStockTransform extends AbstractTransform {
	...
	protected void calculate(ICostingTool costingtool, TransformtradeVO tradevo) throws Exception {
		...
		if(tradevo.getPk_busitype()!=null){
			BusitypeVO busivo= (BusitypeVO) new HYPubBO().queryByPrimaryKey(BusitypeVO.class, tradevo.getPk_busitype())	;
			...
			if(busivo!=null&&busivo.getCode().equals("0404")){
				...
				if (zqjdtallyvo == null
						|| (zqjdtallyvo != null && zqjdtallyvo.getBargain_num().compareTo(UFDouble.ZERO_DBL) <= 0)) {
				...
				if(lastnum.compareTo(new UFDouble(0))!=0){
				   ...
				   UFDouble zcsum = (lastsum.div(lastnum)).multiply(tradevo.getBargain_num());	
				   tradevo.setBargain_sum(zcsum);
					}
				//计算出负债的应收利息计提
				TransformFzmrYjlxUtils utils =new TransformFzmrYjlxUtils();
				//总的应收利息
				UFDouble lx = utils.forwardInterestDistill(costingtool, outstockbalancevo,tradevo);
			    //负债库存数量要大于0才行
						if(lastnum.compareTo(new UFDouble(0))>0){
							tradevo.setInterest(lx.multiply(tradevo.getBargain_num().div(new UFDouble(lastnum))).setScale(2, 4));
						}
						else{
							tradevo.setInterest(new UFDouble(0));
						}
						//存取应收计提利息
						utils.saveInterestDistill(costingtool, outstockbalancevo, tradevo);
				
				//减去转出负债库存的数量和金额
				outstockbalancevo.setVdef1(lastnum.sub(tradevo.getBargain_num()).toString());
				outstockbalancevo.setVdef2(lastsum.sub(tradevo.getBargain_sum()).toString());	
				
				// 计算结转公允价值
//				UFDouble fairvalue = costcalc.forwardFairValueDistill_debt(costingtool, zqjdtallyvo, tradevo);
//				tradevo.setFairvalue(fairvalue);
				// 负债类 减少台账 20150417 ln
				UFDouble sellcost = pm.multiply(tradevo.getBargain_num(), zqjdtallyvo.getBargain_sum());
				sellcost = pm.div(sellcost, zqjdtallyvo.getBargain_num());
				tradevo.setSellcost(sellcost);// 根据台账反写销售成本
				this.savesellbalance(tradevo, sellcost.multiply(new UFDouble(-1)));
				// 保存公允价值
//				costcalc.saveFairValueDistill(costingtool, tradevo);
				...
				instockbalancevo.setVdef1(lastnum2.add(tradevo.getBargain_num2()).toString());
				instockbalancevo.setVdef2(lastsum2.add(tradevo.getBargain_sum2()).toString());
				// 记录台账
...
//				tradevo.setBargain_sum2(stocks_sum);
	...
	private void  savesellbalance(TransformtradeVO tradevo,UFDouble sellscost) throws DAOException, ValidationException{
	 ...
	 * 默认审核 业务有特殊需求的话需要重写
	 */
	public void checkBills(ICostingTool costingtool, TradeDataTool tradedatatool) throws Exception {
		// 处理单据上的应收利息、转出利息
		...
		if (ibills != null && ibills.length > 0) {
			for (IBill aggvo : ibills) {
				TransformtradeVO fathervo = (TransformtradeVO) aggvo.getParent();// 无孩子
	...
	protected void calculateWhenUnCheck(ICostingTool costingtool,
		 ...
		 * @date 2015-12-07
		 */
		tradevo.setBargain_sum((UFDouble) tradevo.getAttributeValue("bargain_sum_his"));
		tradevo.setBargain_sum2((UFDouble) tradevo.getAttributeValue("bargain_sum2_his"));
		tradevo.setBargain_num2((UFDouble) tradevo.getAttributeValue("bargain_num2_his"));
		...
		IZqjdTallyService iZqjd = NCLocator.getInstance().lookup(IZqjdTallyService.class);
