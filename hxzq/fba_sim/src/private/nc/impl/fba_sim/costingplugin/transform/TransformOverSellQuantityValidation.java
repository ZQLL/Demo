...
public class TransformOverSellQuantityValidation implements IValidationService {
	...
	public void validate(Object obj) throws ValidationException {
		...
		Collection<ZqjdTallyVO> tallyVOs = QueryZqjdTallyTool
				.querySellBuyDetails(params);

		if (tallyVOs == null || tallyVOs.size() == 0) {
			ValidationFailure failure = new ValidationFailure();
			failure.setMessage("单据号：" + tallyVO.getSrcbillno()
					+ ",负债转换负债转出数量大于负债数量！");
	 ...
	 * @param vlist
	 */
	private void validateQuantity(Collection<ZqjdTallyVO> tallyVOs,
			List<String> vlist) {
		...
		if (result.doubleValue() < 0) {
			vlist.add("单据号：" + tallyVO.getSrcbillno()
					+ ",债券借贷交易（负债类）买入数量大于卖出数量！");
		}
	}
