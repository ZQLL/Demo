...
public class DebtequitySwap extends AbstractTransform {
	...
	protected void calculate(ICostingTool costingtool,TransformtradeVO tradevo) throws Exception {
		...
		// 库存余额表
		StockBalanceVO stockbalancevo = balanceTool.getStockbalanceVO(queryvo,costingtool);
...
//		tradevo.setBargain_sum2(stocks_sum);  // 转入成本=转出成本+利息---暂无实际利息字段
		...
		if (tradevo.getBargain_sum2() == null || tradevo.getBargain_sum2().compareTo(new UFDouble(0)) == 0) {
			...
			String typecode = tradevo.getAttributeValue("transtypecode").toString();
...
//			tradevo.setBargain_sum2(pm.add(tradevo.getBargain_sum(),tradevo.getInterest()));
		                                                                                    //2014-05-20 曲宁说 算法应该为：D.	转入金额：如果转入金额默认为空，那么转入金额=转出金额+结转应收利息；但是当前的审核后没有将结转应收利息计入转入金额中。
			// update by lihaibo end
			tradevo.setAttributeValue("bargain_sum2_his", null);//==================================cjh
		}else{
			tradevo.setAttributeValue("bargain_sum2_his", tradevo.getBargain_sum2());//==================================cjh
		}
		// 把计算好的数据保存到VO中
		...
		if (stockbalancevo == null) {
			stockbalancevo = balanceTool.getStockbalanceVOByVO(tradevo,costingtool);
