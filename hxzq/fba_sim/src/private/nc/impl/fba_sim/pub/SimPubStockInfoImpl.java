...
public class SimPubStockInfoImpl implements ISimPubStockInfoService {
	 ...
	 * 取市值和流通盘
	 */
	public Hashtable<String, UFDouble[]> getCaptialHT(String trade_date) throws Exception {
		String sql = "select a.pk_securities,capital,pass_num from sim_securities_b a inner join "
				+ "(select b.pk_securities,max(b.trade_date) as max_date from sim_securities_b b " + "where b.trade_date<='" + trade_date
				+ "' group by b.pk_securities) c on a.trade_date=c.max_date " + "and a.pk_securities=c.pk_securities";
		Hashtable<String, UFDouble[]> returnHt = new Hashtable<String, UFDouble[]>();
		ISimPubService service = (ISimPubService) NCLocator.getInstance().lookup(ISimPubService.class.getName());
		ArrayList<ArrayList<Object>> listArr = service.queryData(sql.toString());
	...
	public UFDouble getStockClosePrice(String distill_date, String pk_securities)
		...
		sf.append(" isnull(dr, 0) = 0 and trade_date <= '"+distill_date+" 59:59:59' ");
		...
		if(list != null && list.size() > 0 )
			marketvo = list.get(0);
		if(marketvo == null)
			throw new BusinessException("证券["+querySecuritesName(pk_securities)+"],日期:"+distill_date+"收盘价为空 !");
