...
public class Purchase extends AbstractStock {
	 ...
	 */
	protected void calculateWhenCheck(ICostingTool costingtool, StocktradeVO tradevo) throws Exception {
		...
		UFDouble stocks_num = pm.add(stockbalancevo.getStocks_num(), tradevo.getBargain_num());
		...
		if (isAsset(tradevo.getPk_assetsprop())) {
			...
			if (SystemConst.assetsprop[0] == pm.getAssetsProp(tradevo.getPk_assetsprop(), tradevo.getPk_org())) {
				stocks_sum = pm.add(stockbalancevo.getStocks_sum(), tradevo.getBargain_sum());
			} else {
				stocks_sum = pm.add(stockbalancevo.getStocks_sum(),
						pm.sub(tradevo.getFact_sum(), tradevo.getAccrual_sum()));
			}
			// 税金
			UFDouble stocks_tax = pm.add(stockbalancevo.getStocks_tax(), tradevo.getTaxexpense());
			...
			stockbalancevo.setPk_billtypegroup(costingtool.getCurrbilltypegroupvo().getPk_billtypegroup());
			...
			if (check.isRealTradevo(tradevo.getPk_glorgbook())) {
				ICalcPluginMaintain mail = (ICalcPluginMaintain) NCLocator.getInstance().lookup(
			...
			if(tradevo.getTranstypecode().equals("HV3A-0xx-21")){
				...
				/***********有关债券借贷的证券部在此计算应收利息 lqm**************/
				 ...
				 String str="select max(a.pk_group) pk_group," +
				 		...
				 		"a.pk_assetsprop = '"+tradevo.getPk_assetsprop()+"' " +
				 		"and  a.transtypecode = 'HV7A-0xx-01' " +
				 		"and  a.pk_securities = '"+tradevo.getPk_securities()+"' " +
				 		"and '"+tradevo.getTrade_date()+"' > trade_date and '"+tradevo.getTrade_date().toString().substring(0, 10)+"' < date_end group by   " +
				 		"a.pk_group,a.pk_org,a.pk_org_v,a.pk_securities,a.pk_assetsprop,a.pk_capaccount,a.pk_glorgbook";
				         ArrayList<ZqjdVO> 	nowBowResult = (ArrayList<ZqjdVO>) dao.executeQuery(
						  str, new BeanListProcessor(ZqjdVO.class));
					...
					if ((nowBowResult != null && nowBowResult.size() > 0))
					 {
						//应收利息结转
						//应收利息结转  lqm 
//						ZqYjlxUtils utils =NCLocator.getInstance().lookup(ZqYjlxUtils.class);
						FzmrYjlxUtils utils =new FzmrYjlxUtils();
					
							UFDouble lx = utils.forwardInterestDistill(costingtool, stockbalancevo,tradevo);
							tradevo.setInterest(lx.multiply(tradevo.getBargain_num().div(nowBowResult.get(0).getBargain_num().sub(stockbalancevo.getStocks_num()))));
							//存取应收利息
							utils.saveInterestDistill(costingtool, stockbalancevo, tradevo);
					}
	 ...
	 */
	@Override
	protected void calculateWhenUnCheck(ICostingTool costingtool, StocktradeVO tradevo) throws Exception {
		if (isAsset(tradevo.getPk_assetsprop())) {
			...
			String calc = sysinit.getSysInitValue(pk_glorgbook, CostConstant.PARAM_CALCBUYSELLINTEREST);
			boolean iscalc = "Y".equals(calc) ? true : false;
			String conver = sysinit.getSysInitValue(pk_glorgbook, CostConstant.PARAM_ISCONVERINTEREST);
