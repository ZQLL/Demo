...
public class FzmrYjlxUtils  implements ZqYjlxUtils {
	...
	public UFDouble forwardInterestDistill1(ICostingTool costingtool,
		...
		if (fvo == null)
			return null;
		UFDouble bargain_num = stockbalancevo.getBargain_num();
		if ((fvo.getStock_num() == null)
				|| (fvo.getStock_num().doubleValue() == 0.0D))
			return UFDouble.ZERO_DBL;
		UFDouble lx = this.averageMny1(fvo.getStock_num(),
				bargain_num, fvo.getInterest(), 8);

		lx = lx.setScale(2, 4);
		fvo.setInterest(SafeCompute.sub(fvo.getInterest(), lx));
		fvo.setStock_num(SafeCompute.sub(fvo.getStock_num(), bargain_num));
		return lx;
	}
...
//		}
	...
	public UFDouble queryInterestDate(ICostingTool costingtool, ITrade_Data   vo)
			throws BusinessException {
...
//		UFDate firstdate = info.queryFirstFxDate(pk_securities,trade_date);//查询当前债券的起息日期，考虑牌价倒闸，不需要考虑利率设置档案，上述期初应收利息可以做一次性查询，考虑效率问题。
	...
	public InterestdistillVO[] queryQcInterest(UFDate firstdate,CostPlanVO costplanvo,
			LoginContext context,UFDate trade_date,ITrade_Data  stvo)throws BusinessException{
...
//				queryByWhereClause(InterestdistillVO.class, sf.toString());
	...
	public void saveInterestDistill1(ICostingTool costingtool, 	StockBalanceVO stockbalancevo,ITrade_Data vo)
			throws BusinessException{		
		//校验
		if(check(vo))
			return;
		//开始存数
		fzmrBulidDistillVOInfo info = new fzmrBulidDistillVOInfo();
