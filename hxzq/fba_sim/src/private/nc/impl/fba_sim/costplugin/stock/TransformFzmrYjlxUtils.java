...
import nc.bs.fba_scost.cost.interest.pub.BulidDistillVOInfo;
import nc.bs.trade.business.HYPubBO;
import nc.bs.dao.BaseDAO;
import nc.bs.dao.DAOException;

 
import nc.impl.fba_sim.trade.costplugin.zjjdBulidDistillVOInfo;
...
import nc.itf.fba_sim.ZqYjlxUtils;

public class TransformFzmrYjlxUtils    {
	public UFDouble forwardInterestDistill(ICostingTool costingtool,
			StockBalanceVO stockbalancevo, TransformtradeVO   vo)
			throws BusinessException {
		String pk_glorg = costingtool.getCostParaVO().getCheckParavo().getPk_glorgbook();
		boolean falg = this.
				getBooleanFromInitcode(pk_glorg, CostConstant.PARAM_INTERESTFORWARD);
		UFDouble lx = null;
		if(falg){ 
	 ...
	 * 根据证券档案ID查询是否债券
	 */
	public boolean isSecuritiesAcc(String pk_securities) throws BusinessException {
		SecuritiesVO vo = querySecuritiesVO(pk_securities);
		boolean falg = false;
		if(vo != null)
			falg = vo.getIf_accrual() == null ? false : vo.getIf_accrual().booleanValue();
		return falg;
	}
	 ...
	 * 根据证券档案ID查询证券档案
	 */
	public SecuritiesVO querySecuritiesVO(String pk_securities) throws BusinessException {
		...
		Object obj = DBCacheFacade.runQuery(sql, param, new BeanListProcessor(SecuritiesVO.class));
	...
	public UFDouble forwardInterestDistill1(ICostingTool costingtool,
		...
		UFDouble lx = this.averageMny1(new UFDouble(1),
				new UFDouble(1), fvo.getInterest(), 8);
...
//		}
	...
	public UFDouble queryInterestDate(ICostingTool costingtool, TransformtradeVO   vo)
			throws BusinessException {
	 ...
	 * 计算单支债券的期初应收利息
	 */
	public UFDouble  calcQcInterest(TransformtradeVO      stvoz,CostPlanVO costplanvo,
			LoginContext context,UFDate trade_date)throws BusinessException{
	 
//		String pk_securities = stvoz.getPk_securities();
//		UFDate firstdate = info.queryFirstFxDate(pk_securities,trade_date);//查询当前债券的起息日期，考虑牌价倒闸，不需要考虑利率设置档案，上述期初应收利息可以做一次性查询，考虑效率问题。
		UFDate firstdate = new UFDate(CostConstant.DEFAULT_DATE);
		InterestdistillVO[] distillvos = this.queryQcInterest(firstdate, costplanvo, context, trade_date,stvoz);//wangshzh,在当天有还款，体现到第二天
	...
	public InterestdistillVO[] queryQcInterest(UFDate firstdate,CostPlanVO costplanvo,
			LoginContext context,UFDate trade_date,TransformtradeVO    stvo)throws BusinessException{
		if(firstdate == null || costplanvo == null || context == null || trade_date == null)
			return null;
		StringBuffer sf = new StringBuffer();
		//start 20150612 性能优化 by:mx
		sf.append("select * from sim_interestdistill where ");
		//end 20150612 性能优化 by:mx
		sf.append(" distill_date >= '"+firstdate.asBegin()+"' and distill_date <= '"+trade_date.asEnd()+"' ");
...
//		sf.append(" and genstate = (1,2,3) ");//生成状态全部都算
		...
		sf.append(" and pk_assetsprop = '"+stvo.getHc_pk_assetsprop()+"' ");
		...
		if(costplanvo.getIf_glorgbook() != null && costplanvo.getIf_glorgbook().booleanValue()){
			sf.append(" and  pk_glorgbook = '"+stvo.getPk_glorgbook()+"' ");
		}
		if(costplanvo.getIf_capaccount() != null && costplanvo.getIf_capaccount().booleanValue()){
			sf.append(" and  pk_capaccount = '"+stvo.getHc_pk_capaccount()+"' ");
		}
		if(costplanvo.getIf_selfsgroup() != null &&  costplanvo.getIf_selfsgroup().booleanValue()){
			sf.append(" and  pk_selfsgroup =  '"+stvo.getPk_selfsgroup()+"'  ");
		}
		if(costplanvo.getIf_partnaccount() != null &&  costplanvo.getIf_partnaccount().booleanValue()){
			sf.append(" and  pk_partnaccount =  '"+stvo.getPk_partnaccount()+"'  ");
		}
		if(costplanvo.getIf_operatesite() != null &&  costplanvo.getIf_operatesite().booleanValue()){
			sf.append(" and  pk_operatesite =  '"+stvo.getPk_operatesite()+"'  ");
		}
		if(costplanvo.getIf_client() != null &&  costplanvo.getIf_client().booleanValue()){
...
//		SuperVO[] superVos = (SuperVO[]) values.toArray((SuperVO[]) Array.newInstance(InterestdistillVO.class, 0));
...
//				queryByWhereClause(InterestdistillVO.class, sf.toString());
	...
	public void saveInterestDistill(ICostingTool costingtool, 	StockBalanceVO stockbalancevo, TransformtradeVO   vs)
			throws BusinessException {
		String pk_glorg = costingtool.getCostParaVO().getCheckParavo().getPk_glorgbook();
		boolean falg = this.
				getBooleanFromInitcode(pk_glorg, CostConstant.PARAM_INTERESTFORWARD);
	...
	public void saveInterestDistill1(ICostingTool costingtool, 	StockBalanceVO stockbalancevo,TransformtradeVO     vo)
			throws BusinessException{		
		//校验
		if(check(vo))
			return;
		//开始存数
		TransformBulidDistillVOInfo info = new TransformBulidDistillVOInfo();
		List<InterestdistillVO> list = new ArrayList<InterestdistillVO>();
		InterestdistillVO distillvo = info.buildInterestdistill(costingtool,stockbalancevo, vo);
		if ((distillvo != null) && (distillvo.getInterestdistill() != null)
			...
			if(vo.getClass().toString().equals("class nc.vo.fba_sim.simtrade.transformtrade.TransformtradeVO")){	
				if(((TransformtradeVO)vo).getPk_busitype()!=null){
					BusitypeVO busivo= (BusitypeVO) new HYPubBO().queryByPrimaryKey(BusitypeVO.class, ((TransformtradeVO)vo).getPk_busitype())	;
					//负债转换时，转入单也要生成一个转入的应收利息 lqm
					if(busivo!=null&&busivo.getCode().equals("0404")){
						InterestdistillVO invo = info.buildInterestdistillVO_in(costingtool, ((TransformtradeVO)vo));	
						list.add(invo);
						list.add(distillvo);	
