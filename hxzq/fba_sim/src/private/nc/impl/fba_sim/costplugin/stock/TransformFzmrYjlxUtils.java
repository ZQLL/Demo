...
public class TransformFzmrYjlxUtils {
	...
	public UFDouble forwardInterestDistill1(ICostingTool costingtool,
		...
		UFDouble lx = TransformFzmrYjlxUtils.averageMny1(new UFDouble(1),
	...
	public void saveInterestDistill1(ICostingTool costingtool,
		...
		if ((distillvo != null) && (distillvo.getInterestdistill() != null)
			...
			if (vo.getClass()
					.toString()
					.equals("class nc.vo.fba_sim.simtrade.transformtrade.TransformtradeVO")) {
				if (vo.getPk_busitype() != null) {
					BusitypeVO busivo = (BusitypeVO) new HYPubBO()
							.queryByPrimaryKey(BusitypeVO.class,
									vo.getPk_busitype());
					// 负债转换时，转入单也要生成一个转入的应收利息 lqm
					if (busivo != null && busivo.getCode().equals("0404")) {
						InterestdistillVO invo = info
								.buildInterestdistillVO_in(costingtool, vo);
						list.add(invo);
						list.add(distillvo);
