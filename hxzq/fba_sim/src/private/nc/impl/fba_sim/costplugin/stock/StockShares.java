...
public class StockShares extends AbstractStock {
	...
	protected void calculateWhenCheck(ICostingTool costingtool, StocktradeVO tradevo) throws Exception {
		...
		/*if (SystemConst.assetsprop[0] == pm.getAssetsProp(tradevo.getPk_assetsprop(), tradevo.getPk_org())) {
			...
			stocks_sum = pm.add(stockbalancevo.getStocks_sum(), tradevo.getFact_sum());
		}*/
		stocks_sum = pm.add(stockbalancevo.getStocks_sum(), tradevo.getBargain_sum());
		...
		/**
		 *  计算结转未付收益 add by lhbj
		 *  结转未付收益=应收股利=季末计提未付收益-结转未付收益VDEF3+转换转入-转换转出
		 */
	...
	private UFDouble sumJZWFSY(StocktradeVO tradevo) {
		...
		try {
			...
			sum_jzwfsy = sum_jzwfsy.add(sumJZWFSY_ZH(tradevo)).sub(sumVDEF3(tradevo));
...
//						"   and a.transtypecode = 'HV3A-0xx-05'\n" + 
		...
		para.addParam(repVo.getPk_org());
		try {
			String total = (String) new BaseDAO().executeQuery(sql_sh, para, new ColumnProcessor());
