 ...
 * 
 */
public class zjjdBulidDistillVOInfo {

	/*	*//**
	 * 根据库存构造应收利息计提单据
	 */
	/*
	 * protected InterestdistillVO buildInterestdistillVO(LoginContext
	 * context,UFDate trade_date, StockBalanceVO vs,UFDouble qm,UFDouble qc)
	 * throws BusinessException{ InterestdistillVO distilvo = new
	 * InterestdistillVO();
	 ...
	 * distilvo.setDistill_date(new UFDate(trade_date.toLocalString())); String
	 * pk_group = context.getPk_group(); String pk_org = context.getPk_org();
	 ...
	 * distilvo.setStocks_sum(vs.getStocks_sum()); distilvo.setNqclx(qc);
	 * distilvo.setNqmlx(qm); distilvo.setInterestdistill(SafeCompute.sub(qm,
	 * qc)); distilvo.setPk_costplan(vs.getPk_costplan());
	 ...
	 * distilvo.setAttributeValue("dr", 0); // distilvo.setPk_corp(pk_corp); //
	 * distilvo.setPk_interestdistill(pk_interestdistill); //
	 * distilvo.setPk_org_v(pk_org_v); // distilvo.setPk_voucher(pk_voucher); //
	 * distilvo.setApprovedate(approvedate); // distilvo.setApprover(approver);
	 * // distilvo.setModifiedtime(modifiedtime); //
	 * distilvo.setModifier(modifier); return distilvo; }
	 */
	/**
	 * 根据库存构造应收利息计提单据 20150615性能优化by:mx
	 */
	protected InterestdistillVO buildInterestdistillVO(UFDate trade_date,
			StockBalanceVO vs, UFDouble qm, UFDouble qc, int order,
			String[] vbillcodes, String billtype) throws BusinessException {
	 ...
	 * 构造应收利息计提单据---转出、买入、卖出
	 */
	public InterestdistillVO buildInterestdistill(ICostingTool costingtool,
			StockBalanceVO stockbalancevo, ZqjdVO vs) throws BusinessException {
		InterestdistillVO distilvo = new InterestdistillVO();
		distilvo.setBillmaker(AppContext.getInstance().getPkUser());
		distilvo.setMakedate(AppContext.getInstance().getServerTime());
		distilvo.setCreationtime(AppContext.getInstance().getServerTime());
		...
		distilvo.setState(SystemConst.statecode[1]);
		distilvo.setStocks_num(vs.getBargain_num().sub(
				stockbalancevo.getBargain_num() == null ? new UFDouble(0)
						: stockbalancevo.getBargain_num()));
		distilvo.setStocks_sum(null);
		distilvo.setInterestdistill(getLxRecord(vs));
		distilvo.setPk_costplan(costingtool.getCostParaVO().getCostplanvo()
				.getPk_costplan());
		distilvo.setPk_glorgbook(vs.getPk_glorgbook());
		distilvo.setPk_assetsprop(vs.getPk_assetsprop());
		distilvo.setPk_stocksort(vs.getPk_stocksort());
	 ...
	 * 构造应收利息计提单据---转入
	 */
	public InterestdistillVO buildInterestdistillVO_in(
			ICostingTool costingtool, ITrade_Data vs) throws BusinessException {
		InterestdistillVO distilvo = new InterestdistillVO();
		distilvo.setBillmaker(AppContext.getInstance().getPkUser());
		distilvo.setMakedate(AppContext.getInstance().getServerTime());
		distilvo.setCreationtime(AppContext.getInstance().getServerTime());
	...
	public String getBillNO(String pk_group, String pk_org, InterestdistillVO fv)
			throws BusinessException {
		String vbillcode = null;
		try {
			IBillcodeManage ser = NCLocator.getInstance().lookup(
					IBillcodeManage.class);
			vbillcode = ser.getBillCode_RequiresNew(CostConstant.CJT_BILLTYPE,
					pk_group, pk_org, fv);
	...
	/**
	 * 20150615性能优化by:mx 在数据量大的时候根据所要计提的单据数量批量生成单据号 避免重复查询数据库
	 */
	public String[] getBillNOs(String pk_group, String pk_org,
			InterestdistillVO fv, int length) throws BusinessException {
		String[] vbillcodes = null;
		try {
			IBillcodeManage ser = NCLocator.getInstance().lookup(
					IBillcodeManage.class);
			vbillcodes = ser.getBatchBillCodes_RequiresNew(
					CostConstant.CJT_BILLTYPE, pk_group, pk_org, fv, length);
		} catch (BusinessException be) {
			ExceptionHandler.handleException(be);
		}
		return vbillcodes;
	 ...
	 * @throws BusinessException
	 */
	public String getBillNO(String[] vbillcodes, int order)
			throws BusinessException {
		String billno = vbillcodes[order];
		return billno;
	}

	public UFDouble getLxRecord(ZqjdVO vs) {
		UFDouble z = new UFDouble(0);
		if (vs.getVdef1() != null) {
			z = new UFDouble(vs.getVdef1());
		}

		return z;
	}

	public int getGenstate(ZqjdVO vs) {
		int index = -1;
	...
	public String getBilltypePK(String pk_billtype) throws BusinessException {
		...
		// String where =
		// " pk_billtypecode = '"+pk_billtype+"' and isnull(dr,0) = 0 ";
		// BilltypeVO[] typevos = (BilltypeVO[])new
		// HYSuperDMO().queryByWhereClause(BilltypeVO.class, where);
		String sql = "SELECT * FROM BD_BILLTYPE WHERE PK_BILLTYPECODE = '"
				+ pk_billtype + "' and isnull(dr,0) = 0";
		Collection values = (Collection) DBCacheFacade.runQuery(sql, null,
				new BeanListProcessor(BilltypeVO.class));
		SuperVO[] superVos = (SuperVO[]) values.toArray((SuperVO[]) Array
				.newInstance(BilltypeVO.class, 0));
		BilltypeVO[] typevos = (BilltypeVO[]) superVos;
		// end 20150615 性能优化 by:mx
		if (typevos == null || typevos.length == 0)
			throw new BusinessException(" 应收利息单据类型["
					+ CostConstant.CJT_BILLTYPE + "]没有在系统里注册 ! ");
