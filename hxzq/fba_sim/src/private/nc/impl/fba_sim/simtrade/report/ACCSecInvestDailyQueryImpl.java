...
public class ACCSecInvestDailyQueryImpl implements IACCSecInvestDailyQuery {
	...
	private DataSet genACCSecInvestDailySet(IContext context, ConditionVO[] conVos) throws BusinessException {
		...
		UFDate startDate = new UFDate(SimReportUtils.getStartDate(dateValue)).getDateBefore(ReportConst.DAY).asLocalEnd();
		
		String endDate = SimReportUtils.getEndDate(dateValue);

		CostPlanVO costPlanVO = PubMethod.getInstance().getCostPlanVO(org, glBook, de_group);
		List<CheckplanVO> checkPlanVOs = ReportSqlUtil.getInstance().findCheckPlanVo(org, de_group, glBook);

		/* 查明细并且按维度分组求和 */
		String[] manFieldV = SimReportUtils.getManFieldArray(costPlanVO);
		List<ReportDataVO> reportDatas = this.queryData(checkPlanVOs, conVos, costPlanVO);
		Map<String, Accrual_SecInvestDailyVO> trademap = mergerData(reportDatas, manFieldV);

		List<StockBalanceVO> firBal = null;
		List<StockBalanceVO> endBal = null;
		String conditon = SimReportUtils.getQueryCondition(ReportConst.FUND_B_MARK, conVos, true, true);
		PubReportDataQuery query = new PubReportDataQuery();
		try {
			// 查询库存期初
			firBal = query.getStockBalance(conditon, startDate.toString(), costPlanVO);
			// 查期末
		 ...
		 * 2014-07-11
		 */
		Map firstibvomap = query.getInterest(startDate.toLocalString() + " 00:00:00", costPlanVO, true);
//		// 查询期初库存利息(期初库存利息取值待处理)
//		Map<String, InterestdistillVO> firstibvomap = new QueryInterestUtil().getInterestBalanceMap(conditon, startDate.toString(), costPlanVO);
	// 遍历期初库存利息，如果利息为0，忽略，不为0，则在记录中插入利息
		Iterator<Entry<String, InterestdistillVO>> it = firstibvomap.entrySet().iterator();
...
//			if (ibvo.getNqclx() == null || new UFDouble(0).compareTo(ibvo.getNqclx()) == 0) {
//				continue;
//			}
			...
			if (drvo == null) {
				...
				for (int j = 0; j < manFieldV.length; j++) {
					drvo.setAttributeValue(manFieldV[j], ibvo.getAttributeValue(manFieldV[j]));
				}
		...
		for (int i = 0; i < firSize; i++) {
			sbv = firBal.get(i);
			if (sbv.getStocks_num() == null || new UFDouble(0).compareTo(sbv.getStocks_num()) == 0) {
				continue;
			}
			...
			if (trademap.containsKey(key)) {
				...
				for (int j = 0; j < manFieldV.length; j++) {
					drvo.setAttributeValue(manFieldV[j], sbv.getAttributeValue(manFieldV[j]));
				}
				trademap.put(key, drvo);
			}
...
//			drvo.setInit_total_win(drvo.getTotalWin());
			drvo.setTotalWin(pm.add(drvo.getInit_total_win(), sbv.getTotal_win()));//累计收益=本期收益 +查询开始日期的期初累计收益
		}
		firBal.clear();
		/**
		 * 获取公允价值 YangJie2014-07-10
		 */
		Map map = query.getFairValue(endDate.substring(0, 10) + " 00:00:00", costPlanVO, true);
		...
		for (int i = 0; i < endSize; i++) {
			...
			if (trademap.containsKey(key)) {
				...
				try {
					// 市价 = 收盘价
					drvo.setMartetprice(SimINFOPubMethod.getInstance().getClosePrice(endDate, sbv.getPk_securities()));
					// 市值 = 市价 * 期末库存数量
		 ...
		 * 2014-07-11
		 */
		Map endvomap = query.getInterest(endDate.substring(0,10) + " 00:00:00", costPlanVO, true);
//		Map<String, InterestdistillVO> endvomap = new QueryInterestUtil().getInterestBalanceMap(conditon, endDate, costPlanVO);
...
//			drvo.setAccrual_sum(pm.sub(endAccrualSum,drvo.getInit_accrual_sum()));
			...
			UFDouble accrual_sum=pm.add(pm.add(pm.sub(pm.sub(endAccrualSum,drvo.getInit_accrual_sum()), drvo.getBuy_accrual_sum()), drvo.getSell_accrual_sum()), drvo.getF_accrual_sum());
			drvo.setAccrual_sum(accrual_sum);
			// 浮动盈亏 = 最新市值 – 库存金额
			drvo.setProfit_loss(pm.sub(pm.add(drvo.getFairvalueSum(), new UFDouble(0.0)).compareTo(new UFDouble(0)) == 0 ? drvo.getMarketSum() : drvo.getFairvalueSum(),
					drvo.getEndSum()));
			// 期末应收利息
			drvo.setEnd_accrual_sum(endAccrualSum);
			// 期初应收利息
			// drvo.setInit_accrual_sum(initAccrualSum);
			drvo.setAccrual_balance(pm.add(drvo.getEnd_accrual_sum(), drvo.getAccrual_sum()));
			...
			try {
				captial=SimINFOPubMethod.getInstance().getCaptial(endDate, drvo.getPk_securities());
				//期初市值 add by lihaibo
			...
			if (captial != null && !zero.equals(captial) && drvo.getEndNum() != null) {
				drvo.setProportion(pm.div(pm.multiply(drvo.getEndNum(), 100), captial));// 总股本占比
			}
			//添加证券属性历史
//			String propertyhistoryname=getPropertyhistoryNameBySecurities(drvo.getPk_securities(),enddate);
			String propertyhistoryname=secpropertyMap.get(drvo.getPk_securities());
	 ...
	 */
	private Map<String,String> getPropertyhistoryNames(String enddate) throws BusinessException{
		...
		ArrayList<Object[]> tslist=(ArrayList<Object[]>) queryservice.executeQuery(queryTsSQL.toString(), new ArrayListProcessor());
		...
		sql.append("   AND to_date(substr(historydate,0,10),'yyyy-mm-dd') <= to_date('"+enddate+"','yyyy-mm-dd')");
		sql.append(" ORDER by to_date(substr(historydate, 0, 10),'yyyy-mm-dd') desc");
		ArrayList<Object[]> list=(ArrayList<Object[]>) queryservice.executeQuery(sql.toString(), new ArrayListProcessor());
...
//			throw new BusinessException("证券属性历史表为空，请核实数据！");
		for(int i=0;i<list.size();i++){
			...
			String name=list.get(i)[1]==null? null:list.get(i)[1].toString();
			secpropertyMap.put(pk_securities, name);
		}
		return secpropertyMap;
	}
	private String getPropertyhistoryNameBySecurities(String pk_securities,String enddate) throws BusinessException{
		IUAPQueryBS queryservice = (IUAPQueryBS) NCLocator.getInstance().lookup(IUAPQueryBS.class.getName());
		Collection vos=queryservice.retrieveByClause(PropertyHistoryVO.class, " pk_securities ='"+pk_securities+"' and dr = 0 and to_date(substr(historydate,0,10),'yyyy-mm-dd') <= to_date('"+enddate+"','yyyy-mm-dd')  order by to_date(substr(historydate, 0, 10),'yyyy-mm-dd') desc");
	...
	private Map<String, Accrual_SecInvestDailyVO> mergerData(List<ReportDataVO> datavos, String[] manFieldV) throws BusinessException {
		PubMethod pm = PubMethod.getInstance();
		Map<String, Accrual_SecInvestDailyVO> map = new HashMap<String, Accrual_SecInvestDailyVO>();
		if (datavos.isEmpty()) {
			return map;
		}
		Map<String, List<ReportDataVO>> result = pm.hashlizeObjects(datavos, manFieldV);
		List<ReportDataVO> list = null;
		Accrual_SecInvestDailyVO rprtvo = null;
		ReportDataVO datavo = null;
		Iterator<Entry<String, List<ReportDataVO>>> iter = result.entrySet().iterator();
	 ...
	 * @throws Exception
	 */
	private Accrual_SecInvestDailyVO copytoRprtVO(ReportDataVO datavo) throws BusinessException {
		PubMethod pm = PubMethod.getInstance();
		...
		rprtvo.setInit_total_win(pm.sub(datavo.getSellsum(), datavo.getSellcost()));
		return rprtvo;
	}
	 ...
	 * @throws BusinessException
	 */
	private List<ReportDataVO> queryData(List<CheckplanVO> checkPlanVOs, ConditionVO[] conVos, CostPlanVO costPlanVO) throws BusinessException {
		IUAPQueryBS queryservice = (IUAPQueryBS) NCLocator.getInstance().lookup(IUAPQueryBS.class.getName());
		List<ReportDataVO> resultDatas = new ArrayList<ReportDataVO>();

		String condition = SimReportUtils.getQueryCondition(ReportConst.FUND_B_MARK, conVos, false, true);
		...
		for (CheckplanVO planVO : checkPlanVOs) {
			...
			try {
				List<ReportDataVO> reportVos = (List<ReportDataVO>) queryservice.executeQuery(querySql, new BeanListProcessor(ReportDataVO.class));
