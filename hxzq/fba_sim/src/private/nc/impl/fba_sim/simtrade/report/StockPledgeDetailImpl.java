...
public class StockPledgeDetailImpl implements IStockPledgeDetail {
	...
	private DataSet genSecInvestDataSet(IContext context, ConditionVO[] conVos)
			...
			for (int i = 0; i < secLists.size(); i++) {
				...
				if (secFirLists != null && secFirLists.size() > 0) {
					for (int j = 0; j < secFirLists.size(); j++) {
						if (secFirLists.get(j).getPk_operatesite()
								.equals(contract_id)) {
				...
				String lvsql = "select a.rate as sellCost\n"
						...
						+ "' and a.pk_org = '" + SimReportUtils.getOrg(conVos)
						+ "' and a.pk_glorgbook = '"
				...
				List<ReportDataVO> reportVos1 = (List<ReportDataVO>) getService()
						.executeQuery(lvsql,
								new BeanListProcessor(ReportDataVO.class));
				if (reportVos1 != null && reportVos1.size() > 0) {
					repVo.setSellCost(reportVos1.get(0).getSellcost() == null ? UFDouble.ZERO_DBL
							: reportVos1.get(0).getSellcost());
				} else {
					...
					List<ReportDataVO> reportVos_v = (List<ReportDataVO>) getService()
							.executeQuery(lvsql_v,
									new BeanListProcessor(ReportDataVO.class));
					if (reportVos_v != null && reportVos_v.size() > 0) {
						repVo.setSellCost(reportVos_v.get(0).getSellcost() == null ? UFDouble.ZERO_DBL
								: reportVos_v.get(0).getSellcost());
					}
				}
				...
				List<SecInvestDailyRepVO> reportVos2 = (List<SecInvestDailyRepVO>) getService()
						.executeQuery(
								yjlxsql,
								new BeanListProcessor(SecInvestDailyRepVO.class));
				if (reportVos2 != null && reportVos2.size() > 0) {
					repVo.setLocalwin(reportVos2.get(0).getLocalwin() == null ? UFDouble.ZERO_DBL
							: reportVos2.get(0).getLocalwin());
				}
				...
				String qcyjlxsql = "select sum(t.firstPrice) as firstPrice from ("
						...
						+ "' and a.pk_glorgbook = '"
						+ SimReportUtils.getGLBook(conVos)
						+ "' and a.trade_date <= '" + startDate + "' ) t";
				List<SecInvestDailyRepVO> reportVos3 = (List<SecInvestDailyRepVO>) getService()
						.executeQuery(
								qcyjlxsql,
								new BeanListProcessor(SecInvestDailyRepVO.class));
				if (reportVos3 != null && reportVos3.size() > 0) {
					repVo.setFirstPrice(reportVos3.get(0).getFirstPrice() == null ? UFDouble.ZERO_DBL
							: reportVos3.get(0).getFirstPrice());
				}
				...
				if (reportVos4 != null && reportVos4.size() > 0) {
					repVo.setGyz(reportVos4.get(0).getGyz() == null ? UFDouble.ZERO_DBL
							: reportVos4.get(0).getGyz());
				}
		...
		if (resultVOs != null && resultVOs.size() > 0) {
			for (int i = 0; i < resultVOs.size(); i++) {
				...
				UFDouble firstsum = sec.getFirstSum() == null ? UFDouble.ZERO_DBL
						: sec.getFirstSum();
				UFDouble buysum = sec.getBuySum() == null ? UFDouble.ZERO_DBL
						: sec.getBuySum();
				UFDouble sellsum = sec.getSellSum() == null ? UFDouble.ZERO_DBL
						: sec.getSellSum();
				if (firstsum.toDouble() == 0 && buysum.toDouble() == 0
						&& sellsum.toDouble() == 0) {
					resultVOs.remove(i);
				}
			}
	 ...
	 * @throws BusinessException
	 */
	private List<SecInvestDailyRepVO> queryStockPledge(ConditionVO[] conVos)
			throws BusinessException {
		...
		String condition = SimReportUtils.getQueryCondition(
				ReportConst.FUND_B_MARK, conVos, false, true);
		String querySql = "select temp.pk_operatesite,\n"
	 ...
	 * @throws BusinessException
	 */
	private List<SecInvestDailyRepVO> getFirst(ConditionVO[] conVos)
			throws BusinessException {
		IUAPQueryBS queryservice = (IUAPQueryBS) NCLocator.getInstance()
				.lookup(IUAPQueryBS.class.getName());
		// 查询库存期初
