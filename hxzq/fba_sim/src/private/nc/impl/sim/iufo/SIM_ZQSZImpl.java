...
public class SIM_ZQSZImpl extends SIMIufoResultServiceImpl {
	...
	public Object getFuncResult(IufoQuerySIMVO simvo, boolean isqc)
		...
		try {
			...
			List<StockBalanceVO> zl = banlace.queryLastStock(costplan,
					distillDate, condition, simvo, isqc);
			UFDouble result = new UFDouble(0.0);
			if (zl != null && zl.size() > 0) {
				for (StockBalanceVO z : zl) {
					UFDouble stocksum = z.getStocks_sum() == null ? new UFDouble(
							0.0) : z.getStocks_sum();
					...
					result = result.add(stocksum.multiply(price).setScale(2,
