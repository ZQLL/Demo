...
public class SettlementSettleUIAction extends SettleDefaultAction {
	...
	public void doAction(ActionEvent e) throws Exception {
		...
		List<SettlementAggVO> agglist = SettleUtils.filterSettleInfo4HandSettleFlag4UnSettle(selectedAggVOs);
		if(agglist.size()==0){
			ShowStatusBarMsgUtil.showErrorMsg(nc.vo.ml.NCLangRes4VoTransl.getNCLangRes().getStrByID("3607set1_0","03607set1-0078")/*@res "提示"*/,nc.vo.ml.NCLangRes4VoTransl.getNCLangRes().getStrByID("3607set1_0","03607set1-0083")/*@res "没有可手工结算的表体记录"*/, getModel().getContext());
			return;
		}
	 ...
	 */
	private void processCardSettle(SettlementAggVO settlementAggVO) throws BusinessException {
		...
		while (true) {
			try {
				...
				AccountExceptionHandler handler = new AccountExceptionHandler(getEdit());
		...
		ShowStatusBarMsgUtil.showStatusBarMsg(nc.vo.ml.NCLangRes4VoTransl.getNCLangRes().getStrByID("3607set1_0","03607set1-0075")/*@res "操作成功"*/, getModel().getContext());
		return;
	}
	 ...
	 * @throws BusinessException
	 */
	private void processListSettle(SettlementAggVO[] selectedAggVOs) throws BusinessException {
...
//			headvo.setSettlestatus(SettleStatus.SUCCESSSETTLE.getStatus());
	...
	private void validate() throws BusinessException {
		...
		for (SettlementAggVO settlementAggVO : settlementAggVOs) {
			...
			if ("DS".equals(pk_tradetype)) {
				String errMsg = nc.vo.ml.NCLangRes4VoTransl.getNCLangRes().getStrByID("3607set1_0", "03607set1-0222", null, new String[] {headvo.getBillcode()})/*@res "业务单据号为{0}为工资发放单据,不能手工结算*/;
			...
			for (SettlementBodyVO body : bodyvos) {
				if (body.getPk_Inneraccount() != null) {
					...
					String errMsg = nc.vo.ml.NCLangRes4VoTransl.getNCLangRes().getStrByID("3607set_0", "03607set-0017")/* @res "有内部账户的单据，不能手工结算，请重新选择！" */;
					if (StringUtils.isNotNullWithTrim(billcode)) {
						errMsg = nc.vo.ml.NCLangRes4VoTransl.getNCLangRes().getStrByID("3607set1_0", "03607set1-0223", null, new String[] {headvo.getBillcode()})/* @res "单据号为{0}的单据 有内部账户，不能手工结算，请重新选择！" */;
					}
					ExceptionHandler.cteateandthrowException(errMsg);
				}
			}
	...
	private void checkState(SettlementAggVO... selectedAggVOs) throws BusinessException {
		...
		for (SettlementAggVO settlementAggVO : selectedAggVOs) {
			...
			Integer busistatus = headvo.getBusistatus() == null ? BusiStatus.Save.getBillStatusKind() : headvo
					.getBusistatus();
			if (!BusiStatus.convertInt2Enum(busistatus).equals(BusiStatus.Sign)) {
				sbmsg.append(nc.vo.ml.NCLangRes4VoTransl.getNCLangRes().getStrByID("3607set_0", "03607set-0894",null, new String[] {headvo.getBillcode()})/* @res "业务单据号为[{0}]不是签字态单据,不能结算\n"*/);
			}
		}
	...
	public boolean isActionEnable() {
		...
		for (SettlementAggVO settlementAggVO : selectedAggVOs) {
			...
			settleStatus = settleStatus == null ? SettleStatus.NONESETTLE.getStatus() : settleStatus;
	...
	// 提示手工结算成功信息
	public static void showSettleUIInfo(AbstractUIAppModel model, SettlementAggVO... bills) throws BusinessException {
		...
		if (size > 0) {
			...
			sBuffer.append(nc.vo.ml.NCLangRes4VoTransl.getNCLangRes().getStrByID("3607set_0", "03607set-0895", null,new String[]{size+""})/* @res "结算成功{0}张单据。" */);
