...
public class SettleUtils {
	...
	public static BusiInfo convertSettleBeanToBusiInfo(SettlementAggVO agg) {
		...
		if (!CheckException.checkArraysIsNull(items)) {
			...
			info.setPk_bill_detail(CmpUtils.covertListToArrays(ids,
					String.class));
		}
	...
	public static String convertMsgToQueryCondition(CmpMsg msg) {
		...
		sb.append(" pk_busibill = '").append(msg.getBillkey())
				.append("' and pk_tradetype = '").append(msg.getBilltype())
				.append("'").append(" and pk_org = '").append(msg.getPk_org())
				.append("' and pk_group = '").append(msg.getPk_group())
	...
	public static SettlementHeadVO convertAggVOToStandardBean(
			SettlementAggVO agg) {
	...
	public static SettlementAggVO convertStandardBeanToAggVO(
			SettlementHeadVO head) {
	 ...
	 */
	public static void handleBodyNoteInfo(SettlementBodyVO[] bodys)
			throws BusinessException {
		BaseDAO dao = new BaseDAO();
		// 商业汇票
		Collection notetypevos = dao.retrieveByClause(NotetypeVO.class,
				" noteclass=2");
		for (SettlementBodyVO settlementBodyVO : bodys) {
			if (settlementBodyVO.getPk_notetype() != null) {
				for (NotetypeVO typevo : (NotetypeVO[]) notetypevos
						.toArray(new NotetypeVO[] {})) {
					if (typevo.getPk_notetype().equals(
							settlementBodyVO.getPk_notetype())) {
						// 包含商业汇票信息
	 ...
	 * @throws BusinessException
	 */
	public static void handleBodyLocalCurrency(UFDate date,
			SettlementBodyVO[] bodys) throws BusinessException {
		if (date == null) {
			return;
		}
		for (SettlementBodyVO settlebody : bodys) {
			UFDouble groupCurrRate = TMCurrencyUtil.getGroupCurrRate(
					settlebody.getPk_group(), settlebody.getPk_org(),
					settlebody.getPk_currtype(), date);
			UFDouble globalCurrRate = TMCurrencyUtil.getGlobalCurrRate(
					settlebody.getPk_org(), settlebody.getPk_currtype(), date);
			UFDouble orgCurrRate = TMCurrencyUtil.getOrgCurrRate(
					settlebody.getPk_org(), settlebody.getPk_currtype(), date);
			...
			if (settlebody.getDirection().equals(Direction.PAY.VALUE)) {
				settlebody.setPaylocal(TMCurrencyUtil.getOrgLocalMoney(
						settlebody.getPk_org(), settlebody.getPk_currtype(),
						settlebody.getPay(), orgCurrRate, date));
				settlebody.setGrouppaylocal(TMCurrencyUtil.getGroupLocalMoney(
						settlebody.getPk_group(), settlebody.getPk_org(),
						settlebody.getPk_currtype(), settlebody.getPay(),
						groupCurrRate, orgCurrRate, date));
				settlebody.setGlobalpaylocal(TMCurrencyUtil
						.getGlobalLocalMoney(settlebody.getPk_org(),
								settlebody.getPk_currtype(),
								settlebody.getPay(), globalCurrRate,
								orgCurrRate, date));

			} else if (settlebody.getDirection().equals(Direction.REC.VALUE)) {
				settlebody.setReceivelocal(TMCurrencyUtil.getOrgLocalMoney(
						settlebody.getPk_org(), settlebody.getPk_currtype(),
						settlebody.getReceive(), orgCurrRate, date));
				settlebody.setGroupreceivelocal(TMCurrencyUtil
						.getGroupLocalMoney(settlebody.getPk_group(),
								settlebody.getPk_org(),
								settlebody.getPk_currtype(),
								settlebody.getReceive(), groupCurrRate,
								orgCurrRate, date));
				settlebody.setGlobalreceivelocal(TMCurrencyUtil
						.getGlobalLocalMoney(settlebody.getPk_org(),
								settlebody.getPk_currtype(),
								settlebody.getReceive(), globalCurrRate,
								orgCurrRate, date));
			}
			if (settlebody.getAgentreceiveprimal() != null) {
				// 托收
				settlebody.setAgentreceivelocal(TMCurrencyUtil
						.getOrgLocalMoney(settlebody.getPk_org(),
								settlebody.getPk_currtype(),
								settlebody.getAgentreceiveprimal(),
								orgCurrRate, date));
				settlebody.setGroupagentreceivelocal(TMCurrencyUtil
						.getGroupLocalMoney(settlebody.getPk_group(),
								settlebody.getPk_org(),
								settlebody.getPk_currtype(),
								settlebody.getAgentreceiveprimal(),
								groupCurrRate, orgCurrRate, date));
				settlebody.setGlobalagentreceivelocal(TMCurrencyUtil
						.getGlobalLocalMoney(settlebody.getPk_org(),
								settlebody.getPk_currtype(),
								settlebody.getPay(), globalCurrRate,
								orgCurrRate, date));
	...
	public static void handleBodys4Account(SettlementBodyVO[] bodys)
			throws BusinessException {
	 ...
	 */
	public static void handleBodys4Account(SettlementBodyVO[] bodys,
			boolean is4Settle) throws BusinessException {
		for (int i = 0; i < bodys.length; i++) {
			...
			if (pk_account != null) {
				...
				if (is4Settle) {
					...
					// if(payFinanceVO.getOripaymode()!=null &&
					// (payFinanceVO.getOripaymode().equals(IPaymentConst.Pay_Group)||payFinanceVO.getOripaymode().equals(IPaymentConst.Pay_Back))){
					// // 回拨支付或集团支付情况下，取外部银行
					// settleBodyVO.setPk_account(getfundVO(payFinanceVO,
					// aggVO).getPk_fundbankaccount_p());
					// }else{
					// settleBodyVO.setPk_account(payFinanceVO.getPk_inneraccount_p());
					// }
					if (body.getPk_billbalatype() != null
							&& (body.getPk_billbalatype().equals(
									IPaymentConst.Pay_Group) || body
									.getPk_billbalatype().equals(
											IPaymentConst.Pay_Back))) {
						// 回拨支付或集团支付情况下，取外部银行,业务单据修改时内部账户不做修改
					} else if (bankaccbasvo != null
							&& UFBoolean.TRUE.equals(bankaccbasvo
									.getIsinneracc())) {
			...
			if (!is4Settle) {
				...
				if (pk_oppaccount != null) {
					...
					if (bankaccbasvo != null
							&& bankaccbasvo.getIsinneracc() != null
		...
		for (int i = 0; i < bodys.length; i++) {
			...
			if (getNoteType(false).get(body.getPk_notetype()) != null) {
				...
				isFormDB = true;
				if (!isFormDB
						&& getNoteType(false).get(body.getPk_notetype()) != null) {
					body.setIsbillrecord(UFBoolean.TRUE);
				} else {
	 ...
	 * @throws BusinessException
	 */
	private synchronized static Map<String, NotetypeVO> getNoteType(
			boolean isFormDB) throws BusinessException {
		if (noteTypeMap == null) {
	 ...
	 * @throws BusinessException
	 */
	public static void handlebody4Transtype(SettlementAggVO aggvo)
			throws BusinessException {
		Integer direction = ((SettlementHeadVO) aggvo.getParentVO())
				.getDirection();
		if (direction.equals(Direction.CHANGE.VALUE)) {
			// 划账单是企业内部划转，不需要设置内外部交易
			for (SettlementBodyVO body : (SettlementBodyVO[]) aggvo
					.getChildrenVO()) {
		...
		for (SettlementBodyVO body : (SettlementBodyVO[]) aggvo.getChildrenVO()) {
			...
			if (customerVO != null) {
				// 根据客户判断
				body.setTranstype(customerVO.getCustprop().equals(1) ? TransType.IN.VALUE
						: TransType.OUT.VALUE);
				continue;
			}
			...
			if (supplierVO != null) {
				// 根据供应商判断
				body.setTranstype(supplierVO.getSupprop().equals(1) ? TransType.IN.VALUE
						: TransType.OUT.VALUE);
				continue;
			}
			// 不在范围内，肯定是外部交易
	 ...
	 * @throws BusinessException
	 */
	public static void handleSettleInfo(SettlementAggVO... aggVOs)
			throws BusinessException {
		handleSettleInfo(false, aggVOs);
	}

	public static void handleSettleInfo(boolean is4Settle,
		...
		for (SettlementAggVO aggvo : aggVOs) {
			...
			SettlementBodyVO[] bodys = (SettlementBodyVO[]) aggvo
					.getChildrenVO();
			...
			// }
			SettleUtils.handBody4Tradername(is4Settle, head.getPk_org(),
					head.getDirection(), bodys);
	...
	public static void handleModifyInfo(SettlementAggVO... aggVOs) {
		for (SettlementAggVO aggvo : aggVOs) {
			...
			if (head.getPrimaryKey() != null) {
				SettlementBodyVO[] bodys = (SettlementBodyVO[]) aggvo
						.getChildrenVO();
				SettleUtils.fillSettlementVOInfo(DataUtil.getCurrentUser(),
						SettlementHeadVO.MODIFIER, head);
				SettleUtils.fillSettlementVOInfo(DataUtil.getCurrentUser(),
						SettlementHeadVO.MODIFIER, bodys);
				SettleUtils.fillSettlementVOInfo(DataUtil.getCurrentTime(),
						SettlementHeadVO.MODIFIEDTIME, head);
				SettleUtils.fillSettlementVOInfo(DataUtil.getCurrentTime(),
						SettlementHeadVO.MODIFIEDTIME, bodys);
			}
		}
	}
	 ...
	 * @throws BusinessException
	 */
	public static void handleNoteInfo(SettlementAggVO... aggvos)
			throws BusinessException {
		BaseDAO dao = new BaseDAO();
		// 商业汇票
		Collection notetypevos = dao.retrieveByClause(NotetypeVO.class,
		...
		for (SettlementAggVO aggvo : aggvos) {
			for (SettlementBodyVO settlementBodyVO : (SettlementBodyVO[]) aggvo
				...
				NotetypeVO typevo = mapNote.get(settlementBodyVO
						.getPk_notetype());
				if (typevo != null
						&& typevo.getPk_notetype().equals(
								settlementBodyVO.getPk_notetype())) {
					// 包含商业汇票信息
					settlementBodyVO.setPk_notenumber(settlementBodyVO
							.getNotenumber());
				}
			}
	 ...
	 * @throws BusinessException
	 */
	public static void handleMaterialInfo(SettlementBodyVO... bodyvos)
			throws BusinessException {
		...
		Map<String, MaterialVO> queryMaterialBaseInfoByPks = NCLocator
				...
				.queryMaterialBaseInfoByPks(
						CmpUtils.covertListToArrays(materalLst, String.class),
						new String[] { MaterialVO.PK_MARBASCLASS,
								MaterialVO.PK_MATERIAL });
		for (SettlementBodyVO settlementBodyVO : bodyvos) {
			if (settlementBodyVO.getPk_invbasdoc() != null) {
				settlementBodyVO
						.setPk_invcl(queryMaterialBaseInfoByPks.get(
								settlementBodyVO.getPk_invbasdoc())
								.getPk_marbasclass());
			}
		}
	}
	 ...
	 * @throws BusinessException
	 */
	private static Map<String, CustomerVO> getCustomerVOs(SettlementAggVO aggvo)
			throws BusinessException {
		...
		for (SettlementBodyVO body : (SettlementBodyVO[]) aggvo.getChildrenVO()) {
			...
			if (tradertype != null
					&& (tradertype.equals(CmpConst.TradeObjType_CUSTOMER) || tradertype
							.equals(CmpConst.TradeObjType_KeShang))
					&& body.getPk_trader() != null) {
		...
		if (traderlist.size() > 0) {
			...
			CustomerVO[] customerVOs = getCustomerpubService().getCustomerVO(
					traderlist.toArray(new String[] {}), null);
	 ...
	 * @throws BusinessException
	 */
	private static Map<String, SupplierVO> getSupplierVOs(SettlementAggVO aggvo)
			throws BusinessException {
		List<String> traderlist = CmpUtils.makeList();
		for (SettlementBodyVO body : (SettlementBodyVO[]) aggvo.getChildrenVO()) {
			Integer tradertype = body.getTradertype();
		...
		if (traderlist.size() > 0) {
			...
			SupplierVO[] supplierVOs = getSupplierpubService().getSupplierVO(
					traderlist.toArray(new String[] {}), null);
	 ...
	 * @throws BusinessException
	 */
	public static SettlementBodyVO[] getSettleMetas(Object bill,
			String billtype, String pk_group, int autoCombin)
			throws BusinessException {
		ISettlementInfoFetcher fetcher = BusiBuffer.INSTANCE.getInfoFetcher(
				billtype, pk_group);
		SettlementBodyVO[] settlementBodyVOs = (SettlementBodyVO[]) fetcher
				.getSettleMetaInfo(bill);
	...
	private static SettlementBodyVO[] getIncorporateSettlementBodyVO(
			SettlementBodyVO[] settlementBodyVOs) throws BusinessException {
		...
		//
		// }
		return CmpUtils.dealBodys(settlementBodyVOs).toArray(
				new SettlementBodyVO[] {});
		// return rsnList.toArray(new SettlementBodyVO[] {});
	}

	/** 将空值转为默认值，用于数据库未设默认值，从界面得到编辑后的vo的情况 */
	public static SettlementBodyVO[] convertNullToDefaultValue(
			SettlementBodyVO... bodyvos) {
		if (bodyvos == null) {
			return bodyvos;
		}
	...
	public static void convertZeroToNull(SettlementAggVO... aggs) {
		...
		for (SettlementAggVO settlementAggVO : aggs) {
			convertZeroToNull((SettlementBodyVO[]) settlementAggVO
					.getChildrenVO());
		}
	}
	 ...
	 */
	public static AggregatedValueObject getBuisAggVO(String tradrType,
			String pk_busibill, String pk_group) throws BusinessException {
		...
		if ("F5".equals(billType)) {
			ICmpPayBillQueryService iCmpPayBillQueryService = NCLocator
					.getInstance().lookup(ICmpPayBillQueryService.class);
			obj = iCmpPayBillQueryService.queryBillById(pk_busibill);
			bill[0] = (AggregatedValueObject) obj;
		} else if ("F4".equals(billType)) {
			ICmpRecBillQueryService iCmpRecBillQueryService = NCLocator
					.getInstance().lookup(ICmpRecBillQueryService.class);
			obj = iCmpRecBillQueryService.queryBillById(pk_busibill);
			bill[0] = (AggregatedValueObject) obj;
		} else if ("F6".equals(billType)) {
			ICmpChangeBillQueryService iCmpChangeBillQueryService = NCLocator
					.getInstance().lookup(ICmpChangeBillQueryService.class);
			obj = iCmpChangeBillQueryService.queryBillById(pk_busibill);
			bill[0] = (AggregatedValueObject) obj;
		} else if (IArapBillTypeCons.F0.equals(billType)) {
			bill = NCLocator.getInstance()
			...
			bill = NCLocator.getInstance()
					.lookup(IArapGatheringBillPubQueryService.class)
					.findBillByPrimaryKey(new String[] { pk_busibill });
			...
			bill = NCLocator
					...
					.queryVOsByPrimaryKeys(new String[] { pk_busibill },
							BXConstans.BX_DJDL)
	...
	public static SettlementAggVO combinAggBean(SettlementAggVO aggBean)
			throws BusinessException {
		SettlementHeadVO head = (SettlementHeadVO) aggBean.getParentVO();
		SettlementAggVO cloneAgg = new SettlementAggVO();
		SettlementBodyVO[] cloneBodys = CmpUtils
				.CloneObj((SettlementBodyVO[]) aggBean.getChildrenVO());
		cloneBodys = CMPFactory.createAlgorithm(head.getArithmetic())
				.sumBodyVOToUI(cloneBodys);
	 ...
	 * @return String[] ---pk_settlement[]
	 */
	public static SettlementHeadVO[] convertSettlementAggVOToSettlementHeadVOArray(
			SettlementAggVO... aggs) {
	 ...
	 * @return String[] ---pk_settlement[]
	 */
	public static String[] convertSettlementAggVOToSettlementHeadPkArray(
			SettlementAggVO... aggs) {
	 ...
	 */
	public static SettlementBodyVO[] convertSettlementAggVOToSettlementBodyVOArray(
			SettlementAggVO... aggs) {
		List<SettlementBodyVO> bodyList = CmpUtils.makeList();
		for (SettlementAggVO settlementAggVO : aggs) {
			SettlementBodyVO[] bodys = (SettlementBodyVO[]) settlementAggVO
					.getChildrenVO();
	 ...
	 */
	public static Map<String, SettlementBodyVO[]> convertSettlementAggVOToHeadPkToBodyMap(
		...
		for (SettlementAggVO agg : aggs) {
			headpkToBodyMap.put(agg.getParentVO().getPrimaryKey(),
					(SettlementBodyVO[]) agg.getChildrenVO());
		}
	 ...
	 *         ,value--SettlementBodyVO[]
	 */
	public static Map<String, SettlementBodyVO> convertSettlementAggVOToFtsBodyPkToBodyMap(
			SettlementAggVO... aggs) throws BusinessException {
		Map<String, SettlementBodyVO> ftsbodypkTosettleBodyMap = CmpUtils
				.makeMap();
		for (SettlementAggVO settlementAggVO : aggs) {
			SettlementBodyVO[] bodys = (SettlementBodyVO[]) settlementAggVO
					.getChildrenVO();
	 ...
	 *         ,value--SettlementBodyVO[]
	 */
	public static Map<String, String> convertSettlementAggVOToFtsBodyPkToSettleBodyPKMap(
			SettlementAggVO... aggs) throws BusinessException {
		Map<String, String> ftsbodypkTosettleBodyPkMap = CmpUtils.makeMap();
		for (SettlementAggVO agg : aggs) {
			SettlementBodyVO[] bodys = (SettlementBodyVO[]) agg.getChildrenVO();
			for (SettlementBodyVO body : bodys) {
				ftsbodypkTosettleBodyPkMap.put(body.getPk_ftsbilldetail(),
						body.getPk_detail());
			}
		}
	...
	public static Map<String, String> convertAggInnerTransferVOsToFtsBodyPkToUnAcceptReasonMap(
		...
		for (AggInnerTransferVO agg : aggInnerTransfers) {
			...
			for (InnerTransferBVO body : bodys) {
				tsBodyPkToUnAcceptReasonMap.put(body.getPrimaryKey(),
						body.getUnacceptreason());
			}
		}
	 ...
	 *         ,value--SettlementBodyVO[]
	 */
	public static Map<String, String> convertSettlementAggVOToBusiBodyPkToSettleBodyPKMap(
			SettlementAggVO agg) throws BusinessException {
		Map<String, String> busibodypkTosettleBodyPkMap = CmpUtils.makeMap();
		SettlementBodyVO[] bodys = (SettlementBodyVO[]) agg.getChildrenVO();
		for (SettlementBodyVO body : bodys) {
			busibodypkTosettleBodyPkMap.put(body.getPk_billdetail(),
					body.getPk_detail());
		}
	...
	public static Map<String, List<SettlementAggVO>> convertSettlementAggVOsToPk_tradertypeToSettlementAggVOListMap(
			SettlementAggVO... aggs) throws BusinessException {
		Map<String, List<SettlementAggVO>> Pk_tradertypeToSettlementAggVOListMap = CmpUtils
				.makeMap();
		...
		for (SettlementAggVO settlementAggVO : aggs) {
			...
			if (!head.getIsindependent().equals(UFBoolean.TRUE)
					&& head.getPk_tradetype() == null) {
				throw new BusinessException(nc.vo.ml.NCLangRes4VoTransl
						.getNCLangRes()
						.getStrByID("3607set_0", "03607set-0469")/*
																 * @res
																 * "结算信息缺少交易类型信息！"
																 */);
			} else if (key != null) {
				if (Pk_tradertypeToSettlementAggVOListMap.get(key) == null) {
					...
					Pk_tradertypeToSettlementAggVOListMap.put(key,
							settleaggList);
				} else {
					Pk_tradertypeToSettlementAggVOListMap.get(key).add(
							settlementAggVO);
				}
			}
		}
	 ...
	 */
	public static Map<String, SettlementAggVO> convertSettlementAggVOToBusiHeadPkToAggMap(
			SettlementAggVO... aggs) throws BusinessException {
	 ...
	 *         ,value--SettlementAggVO
	 */
	public static Map<String, SettlementAggVO> convertSettlementAggVOToHeadPkToAggMap(
			SettlementAggVO... aggs) throws BusinessException {
		Map<String, SettlementAggVO> headpkToAggMap = CmpUtils.makeMap();
		for (SettlementAggVO agg : aggs) {
	 ...
	 */
	public static Map<String, String> convertSettlementBodyVOToSettleBodyPkToBusiBodyPkMap(
			SettlementBodyVO... bodys) {
		Map<String, String> settlebodyPkTobusiBodyPkMap = CmpUtils.makeMap();
		for (SettlementBodyVO body : bodys) {
			settlebodyPkTobusiBodyPkMap.put(body.getPk_detail(),
					body.getPk_billdetail());
		}
	 ...
	 *         ,value--SettlementBodyVO[]
	 */
	public static Map<String, SettlementBodyVO> convertSettlementAggVOToBodyVOPkToBodyVOMap(
			SettlementAggVO... aggs) throws BusinessException {
	 ...
	 * 通过交易类型得到单据大类
	 * **/
	public static String getDjdlByPk_tradetype(String pk_tradetype,
	 ...
	 * */
	@Deprecated
	public static boolean judgeIsSendTODapAtSignPointBySettlementAgg(
			SettlementAggVO settleagg) throws BusinessException {

		SettlementHeadVO head = (SettlementHeadVO) settleagg.getParentVO();
		// SettlementBodyVO[] bodys
		// =(SettlementBodyVO[])settleagg.getChildrenVO();
		boolean isexistInnerAccount = SettleUtils
				.isExistInnerAccount(settleagg);
		if (!isexistInnerAccount) {// 非资金流程，签字时传会计平台
			return true;
		}
		// 资金流程
		String pkFtsbill = head.getPk_ftsbill();
		boolean iscommpay = head.getIscommpay() == null ? false : head
				.getIscommpay().booleanValue();
		boolean isindependent = head.getIsindependent() == null ? false : head
				.getIsindependent().booleanValue();
		String djdl = SettleUtils.getDjdlByPk_tradetype(head.getPk_tradetype(),
				head.getPk_group());
	...
	public static boolean judgeIsSendToDapBySettlementAgg(Boolean isSignPoing,
			SettlementAggVO settleagg) throws BusinessException {
		SettlementHeadVO head = (SettlementHeadVO) settleagg.getParentVO();
		boolean isexistInnerAccount = SettleUtils
				.isExistInnerAccount(settleagg);
		...
		String billtype = getbilltype(head.getPk_tradetype(),
				head.getPk_group());
		if (!isexistInnerAccount) {// 非资金流程，签字时传会计平台
			// 判断参数
			...
			if (billtype.equals("F4") && billtype.equals("F5")) {
				// 收款结算单、付款结算单根据参数控制
				String dapStr = dapPara.get(head.getPk_org()
		...
		if (pkFtsbill == null) {
			...
			if (isSignPoing
					&& iscommpay
					&& ("fj".equals(djdl) || "fk".equals(djdl) || "bx"
							.equals(djdl))) {
				return false;
			}
	...
	public static Map<String, String> convertAggInnerTransferVOToFtsBodyPKTobusiBodyPKMap(
		...
		for (AggInnerTransferVO aggInnerTransferVO : aggInnerTransferVOs) {
			...
			for (InnerTransferBVO innerTransferBVO : innerTransferBVOs) {
				pk_ftsdetailToPk_busidetailMap.put(
						innerTransferBVO.getPk_innertransfer_b(),
	...
	public static Map<String, String> convertSettlementAggVOsToFtsBodyPKToSettleBodyPKMap(
		...
		for (SettlementAggVO settlementAggVO : settlementAggVOs) {
			SettlementBodyVO[] settlementBodyVOs = (SettlementBodyVO[]) settlementAggVO
					.getChildrenVO();
			for (SettlementBodyVO settlementBodyVO : settlementBodyVOs) {
				pk_ftsdetailToPk_detailMap.put(
						settlementBodyVO.getPk_ftsbilldetail(),
	...
	public static Map<String, String> convertAggInnerTransferVOToFtsBodyPKToSettleBodyPKMap(
			SettlementAggVO upSettleAgg,
			AggInnerTransferVO... aggInnerTransferVOs) throws BusinessException {
	...
	// 填充表体的数据
	public static void fillSettlementBodyVOsJeAndHL(SettlementBodyVO[] bodys)
			throws BusinessException {
		for (SettlementBodyVO settlementBodyVO : bodys) {
	 ...
	 * @param local
	 */
	public static void fillSettlementBodyLocol(SettlementBodyVO body,
			UFDouble local) {
		// @wuzhwa 取消负数限制，
		// TODO 是否需要根据汇率计算
		// if
	...
	public static void fillSettlementBodyJeAndHL(SettlementBodyVO body)
			throws BusinessException {
	 ...
	 * @throws BusinessException
	 */
	public static UFDouble[] computeYFB(SettlementBodyVO body)
			throws BusinessException {
		...
		return Currency.computeYFB(pkOrg, 1, pkCurrtype, primal, null, null,
				null, zbhv, date);
	}
	...
	public static UFDouble getSettlementBodyPrimal(SettlementBodyVO body) {
		...
		if (body.getPay() != null
				&& body.getPay().compareTo(UFDouble.ZERO_DBL) > 0) {
			primal = body.getPay();
		} else if (body.getReceive() != null
				&& body.getReceive().compareTo(UFDouble.ZERO_DBL) > 0) {
			primal = body.getReceive();
		}
		return primal;
	}

	public static UFDouble getRate(SettlementBodyVO body)
			throws BusinessException {
		if (body.getPk_org() == null) {
			throw new BusinessException(nc.vo.ml.NCLangRes4VoTransl
					.getNCLangRes().getStrByID("3607set_0", "03607set-0612")/*
																			 * @res
																			 * "根据组织获取汇率时，组织值不能为空"
																			 */);
		}
		if (body.getPk_currtype() == null) {
			throw new BusinessException(nc.vo.ml.NCLangRes4VoTransl
					.getNCLangRes().getStrByID("3607set_0", "03607set-0613")/*
																			 * @res
																			 * "获取汇率时，币种值不能为空"
																			 */);
		}
	...
	// 填充表头的原币和本币
	public static void fillSettlementHeadPrimalAndOrglocalField(
			SettlementAggVO settleaggvo) {
		SettlementHeadVO headVO = (SettlementHeadVO) settleaggvo.getParentVO();
		SettlementBodyVO[] bodys = (SettlementBodyVO[]) settleaggvo
				.getChildrenVO();
		...
		for (SettlementBodyVO settlementBodyVO : bodys) {
			if (settlementBodyVO.getPay() != null) {
				...
				if (!CmpUtils.isObjectNull(settlementBodyVO.getBilldate())) {
					payOrgLocal = payOrgLocal.add(settlementBodyVO
							.getPaylocal() == null ? UFDouble.ZERO_DBL
							: settlementBodyVO.getPaylocal());
					payGroupLocal = payGroupLocal.add(settlementBodyVO
							.getGrouppaylocal() == null ? UFDouble.ZERO_DBL
							: settlementBodyVO.getGrouppaylocal());
					payGlobalLocal = payGlobalLocal.add(settlementBodyVO
							.getGlobalpaylocal() == null ? UFDouble.ZERO_DBL
							: settlementBodyVO.getGlobalpaylocal());
				}
				paynum++;
			} else if (settlementBodyVO.getReceive() != null) {
				recPrimal = recPrimal.add(settlementBodyVO.getReceive());
				if (!CmpUtils.isObjectNull(settlementBodyVO.getBilldate())) {
					recOrgLocal = recOrgLocal.add(settlementBodyVO
							.getReceivelocal() == null ? UFDouble.ZERO_DBL
							: settlementBodyVO.getReceivelocal());
					recGroupLocal = recGroupLocal.add(settlementBodyVO
							.getGroupreceivelocal() == null ? UFDouble.ZERO_DBL
							: settlementBodyVO.getGroupreceivelocal());
					recGlobalLocal = recGlobalLocal
							.add(settlementBodyVO.getGlobalreceivelocal() == null ? UFDouble.ZERO_DBL
									: settlementBodyVO.getGlobalreceivelocal());
				}
				recnum++;
			}
		}
	 ...
	 *         ,value--SettlementBodyVO[]
	 */
	public static Map<String, SettlementBodyVO> convertSettlementAggVOToBusiBodyPkToSettleBodyMap(
			SettlementAggVO agg) throws BusinessException {
		Map<String, SettlementBodyVO> busibodypkTosettleBodyPkMap = CmpUtils
				.makeMap();
		SettlementBodyVO[] bodys = (SettlementBodyVO[]) agg.getChildrenVO();
		for (SettlementBodyVO body : bodys) {
	...
	public static Map<String, SettlementBodyVO[]> convertSettlementAggVOToBusiBodyPkToSettleBodyArrayMap(
		...
		Map<String, SettlementBodyVO[]> busibodypkTosettleBodyArrayMap = CmpUtils
				.makeMap();
		...
		for (Iterator iterator = busiBodyPKSet.iterator(); iterator.hasNext();) {
			...
			List<SettlementBodyVO> busiBodyPkList = busibodypkTosettleBodyListMap
					.get(pk_billdetail);
			SettlementBodyVO[] bodys = new SettlementBodyVO[busiBodyPkList
					.size()];
	...
	public static Map<String, List<SettlementBodyVO>> convertSettlementAggVOToBusiBodyPkToSettleBodyListMap(
			SettlementAggVO... aggs) throws BusinessException {
		Map<String, List<SettlementBodyVO>> busibodypkTosettleBodyListMap = CmpUtils
				.makeMap();
		for (SettlementAggVO settlementAggVO : aggs) {
			SettlementBodyVO[] bodys = (SettlementBodyVO[]) settlementAggVO
					.getChildrenVO();
			for (int i = 0; i < bodys.length; i++) {
				...
				List<SettlementBodyVO> bodyList = busibodypkTosettleBodyListMap
						.get(bodyVO.getPk_billdetail());
				if (bodyList == null) {
					bodyList = CmpUtils.makeList();
					busibodypkTosettleBodyListMap.put(
							bodyVO.getPk_billdetail(), bodyList);
				}
				bodyList.add(bodyVO);
			}
		}
	 ...
	 * @return NCObject[]
	 */
	public static NCObject[] convertSettlementAggVOsToNCObjects(
			SettlementAggVO... aggs) {
	 ...
	 * 传会计平台前处理
	 * */
	public static BusiStateChangeVO[] convertSettlementAggVOToBusiStateChangeVOs(
			SettlementAggVO... settleAggs) throws BusinessException {
	 ...
	 * @throws BusinessException
	 */
	public static int getKeShangTypeByOrg(String pk_org, Integer direction)
			throws BusinessException {
		ICustsupPubService service = CmpInterfaceProxy.INSTANCE
				.getCustsupPubService();
	 ...
	 * @throws BusinessException
	 */
	public static String getKeShangByOrg(String pk_org)
			throws BusinessException {
		ICustsupPubService service = CmpInterfaceProxy.INSTANCE
				.getCustsupPubService();
	...
	public static void handBody4Tradername(String pk_org, Integer direction,
			SettlementBodyVO... settlementBodyVOs) throws BusinessException {
	 ...
	 * @throws BusinessException
	 */
	public static void handBody4Tradername(boolean is4Settle, String pk_org,
			Integer direction, SettlementBodyVO... settlementBodyVOs)
			throws BusinessException {
		...
		try {
			...
			if (customerList.size() > 0) {
				...
				customerMap = getCustomerVOS(customerList
						.toArray(new String[] {}));
				// CustomerVO[] customervos =
				// NCLocator.getInstance().lookup(ICustomerPubService.class)
			...
			if (supplierList.size() > 0) {
				...
				SupplierVO[] suppliervos = getSupplierpubService()
						.getSupplierVO(supplierList.toArray(new String[] {}),
								null);
				if (!CmpUtils.isListNull(suppliervos)) {
			...
			if (deptList.size() > 0) {
				...
				DeptVO[] deptVOs = NCLocator.getInstance()
						.lookup(IDeptQryService.class)
						.queryAllDeptVOsByOrgID(pk_org);
				if (!CmpUtils.isListNull(deptVOs)) {
					for (DeptVO deptVO : deptVOs) {
						if (deptVO == null) {
							continue;
						}
						deptMap.put(deptVO.getPrimaryKey(),
								VOUtil.getMultiLangText(deptVO, DeptVO.NAME));
					}
				}
			}
			// 人员
		...
		for (SettlementBodyVO bodyvo : settlementBodyVOs) {
			...
			if (bodyvo.getTradertype() == null) {
				...
				if (direction.equals(Direction.PAY.VALUE)) {
					...
					if (supplierMap != null && supplierMap.size() > 0) {
						...
						if (bodyvo.getTradername() == null) {
							bodyvo.setTradername(supplierMap.get(bodyvo
									.getPk_trader()));
						}
						continue;
					}
					...
					if (customerMap != null && customerMap.size() > 0) {
						...
						if (bodyvo.getTradername() == null) {
							bodyvo.setTradername(customerMap.get(bodyvo
									.getPk_trader()));
						}
						continue;
					}
				...
				if (psndocMap != null && psndocMap.size() > 0) {
					...
					if (bodyvo.getTradername() == null) {
						bodyvo.setTradername(psndocMap.get(bodyvo
								.getPk_trader()));
					}
				}
			} else if (bodyvo.getTradertype() == CmpConst.TradeObjType_SanHu) {
	...
	private static Map<String, String> getCustomerVOS(String... pks) {
		...
		IGeneralAccessor accessor = GeneralAccessorFactory
				.getAccessor(IBDMetaDataIDConst.CUSTSUPPLIER);

		for (String pk : pks) {
			BDDataVO bdData = (BDDataVO) accessor.getDocByPk(pk);
			if (bdData != null) {
				MultiLangText name = bdData.getName();
				customerMap.put(pk, name.getText(MultiLangContext.getInstance()
						.getCurrentLangSeq() - 1));
			}
	 ...
	 */
	@Deprecated
	public static void handlebodys4ORGDate(CMPaccStatus status,
			SettlementBodyVO... bodys) throws BusinessException {
		if (CmpUtils.isListNull(bodys)) {
			throw new BusinessException(nc.vo.ml.NCLangRes4VoTransl
					.getNCLangRes().getStrByID("3607set_0", "03607set-0614")/*
																			 * @res
																			 * "结算信息表体信息不能为空，转换多时区信息失败，请检查信息"
																			 */);
	...
	private static void setORGUFDateInfo(CMPaccStatus status,
			SettlementBodyVO settlementBodyVO) throws BusinessException {

		UFDateTime ufDateTime = new UFDateTime(System.currentTimeMillis());

		if (settlementBodyVO.getFundformcode() != null
				&& settlementBodyVO.getFundformcode().equals(
						CmpConst.BANK_DEPOSIT)) {
			UFDateTime orgbusitime = getORGUFDateTimeByOrg(
					settlementBodyVO.getPk_org(), ufDateTime);

			// 银行账户，根据银行账户子户时区做判断XXX
			// modify:改由根据组织时间来获取多时区
			// 业务日期
			if ((status == null || status.equals(CMPaccStatus.SAVEACCOUNT))
					&& settlementBodyVO.getBilldate() != null) {

				// settlementBodyVO.setOrgbusidate(getORGUFDate(settlementBodyVO.getPk_account(),
				// settlementBodyVO
				// .getBilldate()));
				settlementBodyVO.setOrgbusidate(getORGUFDateByOrg(
						settlementBodyVO.getPk_org(),
			...
			if ((status == null || status.equals(CMPaccStatus.SIGNACCOUNT))
					&& settlementBodyVO.getSigndate() != null) {
				// settlementBodyVO.setOrgsigndate(getORGUFDate(settlementBodyVO.getPk_account(),
				// settlementBodyVO
				// .getSigndate()));
				settlementBodyVO.setOrgsigndate(getORGUFDateByOrg(
						settlementBodyVO.getPk_org(),
			...
			if ((status == null || status.equals(CMPaccStatus.SUCCESSACCOUNT))
				...
				// settlementBodyVO
				// .getTallydate()));
				settlementBodyVO.setOrgsettledate(getORGUFDateByOrg(
						settlementBodyVO.getPk_org(),
			...
			if ((status == null || status.equals(CMPaccStatus.SAVEACCOUNT))
				...
				settlementBodyVO.setOrgbusidate(UFLiteralDate
						.getDate(settlementBodyVO.getBilldate().toDate()));
			...
			if ((status == null || status.equals(CMPaccStatus.SIGNACCOUNT))
					&& settlementBodyVO.getSigndate() != null) {
				settlementBodyVO.setOrgsigndate(UFLiteralDate
						.getDate(settlementBodyVO.getSigndate().toDate()));
			...
			if ((status == null || status.equals(CMPaccStatus.SUCCESSACCOUNT))
					&& settlementBodyVO.getTallydate() != null) {
				settlementBodyVO.setOrgsettledate(UFLiteralDate
						.getDate(settlementBodyVO.getTallydate().toDate()));
	 ...
	 */
	@Deprecated
	public static UFLiteralDate getORGUFDate(String pk_account, UFDate date)
			throws BusinessException {
		if (nc.vo.jcom.lang.StringUtil.isEmpty(pk_account) || date == null) {
			// 获取组织日期异常
			return null;
		}
	...
	@Deprecated
	public static UFLiteralDate getORGUFDateByOrg(String pk_org, UFDate date)
			throws BusinessException {
		if (nc.vo.jcom.lang.StringUtil.isEmpty(pk_org) || date == null) {
			// 获取组织日期异常
			return null;
		}
		...
		try {
			...
			throw new BusinessException(nc.vo.ml.NCLangRes4VoTransl
					.getNCLangRes().getStrByID("3607set_0", "03607set-0775")/*
																			 * @res
																			 * "获取当前组织时区失败，请先维护组织多时区信息"
																			 */);
		}
	...
	// }
	@Deprecated
	public static UFDateTime getORGUFDateTimeByOrg(String pk_org,
			UFDateTime datetime) throws BusinessException {
		if (nc.vo.jcom.lang.StringUtil.isEmpty(pk_org) || datetime == null) {
			// 获取组织时间异常
			return null;
		}
		...
		try {
			OrgVO[] orgVOs = NCLocator
					...
					.getOrgs(
							new String[] { pk_org },
							new String[] { OrgVO.PK_ORG, OrgVO.PK_VID,
									OrgVO.PK_GROUP, OrgVO.PK_TIMEZONE });
			if (!CmpUtils.isListNull(orgVOs)) {
				OrgVO orgVO = orgVOs[0];
				TimeZone fundOrgTimeZone = TimezoneUtil.getTimeZone(orgVO
						.getPk_timezone());
				...
				throw new BusinessException(nc.vo.ml.NCLangRes4VoTransl
						.getNCLangRes()
						.getStrByID("3607set_0", "03607set-0788")/* @res "无法找到组织" */);
			}
		} catch (BusinessException e) {
			throw new BusinessException(nc.vo.ml.NCLangRes4VoTransl
					.getNCLangRes().getStrByID("3607set_0", "03607set-0775")/*
	 ...
	 * @throws BusinessException
	 */
	public static TimeZone getTimeZoneBySubAcc(String pk_account)
			throws BusinessException {
		// 获取银行档案信息
		...
		if (bankAccbasVO == null || bankAccbasVO.getPk_bankdoc() == null) {
			throw new BusinessException(nc.vo.ml.NCLangRes4VoTransl
					.getNCLangRes().getStrByID("3607set_0", "03607set-0615")/*
																			 * @res
																			 * "根据银行账户子户获取银行档案信息失败"
																			 */);
		}
		// 调用获取时区接口
		TimeZone tz = getBankdocPubQueryService().getTimeZoneVoByBankdoc(
				bankAccbasVO.getPk_bankdoc());
		if (tz == null) {
			throw new BusinessException(nc.vo.ml.NCLangRes4VoTransl
					.getNCLangRes().getStrByID("3607set_0", "03607set-0616",
							null, new String[] { bankAccbasVO.getAccname() })/*
																			 * @res
																			 * "获取账户[{0}]时区失败"
																			 */);
		}
	 ...
	 * @throws BusinessException
	 */
	public static BankAccbasVO getAccbasVObySubBankAccPK(String pk_account)
			throws BusinessException {
		...
		if (pk_account != null) {
			...
			bankSubVOS = getAnkAccSubInfoQueryService().querySubInfosByPKs(
					new String[] { pk_account });
		} else {
			// 退出
			return null;
		}
		...
		if (!CmpUtils.isListNull(bankSubVOS)) {
			bankaccbasvo = getBankaccPubQueryService().queryAccbasInfByAccID(
					bankSubVOS[0].getPk_bankaccbas(), null);
	...
	public static IBankaccPubQueryService getBankaccPubQueryService() {
		if (bankaccPubQueryService == null) {
			bankaccPubQueryService = NCLocator.getInstance().lookup(
					IBankaccPubQueryService.class);
		}
	...
	public static IBankAccSubInfoQueryService getAnkAccSubInfoQueryService() {
		if (bankAccSubInfoQueryService == null) {
			bankAccSubInfoQueryService = NCLocator.getInstance().lookup(
	...
	public static IBankdocPubQueryService getBankdocPubQueryService() {
		if (bankdocPubQueryService == null) {
			bankdocPubQueryService = NCLocator.getInstance().lookup(
					IBankdocPubQueryService.class);
		}
	...
	public static ICustomerPubService getCustomerpubService() {
		if (customerPubService == null) {
			customerPubService = NCLocator.getInstance().lookup(
					ICustomerPubService.class);
		}
	...
	public static ISupplierPubService getSupplierpubService() {
		if (supplierPubService == null) {
			supplierPubService = NCLocator.getInstance().lookup(
					ISupplierPubService.class);
		}
	 ...
	 * @throws BusinessException
	 */
	public static String getbilltype(String pk_tradetype, String pkGroup)
			throws BusinessException {
		...
		if (pkGroup != null) {
			...
			billType = PfDataCache.getBillTypeInfo(InvocationInfoProxy
					.getInstance().getGroupId(), pk_tradetype);
		}
		if (billType == null) {
			throw new BusinessException(nc.vo.ml.NCLangRes4VoTransl
					.getNCLangRes().getStrByID("3607set_0", "03607set-0467",
							null, new String[] { pk_tradetype })/*
																 * @res
																 * "根据交易类型{0}获取单据类型失败，请检查类型设置！"
																 */);
		} else {
	 ...
	 * @return
	 */
	public static void departSettlementAggVOs(
			List<SettlementAggVO> aggvosFromUnit,
	 ...
	 * @throws BusinessException
	 */
	public static Map<String, String> getDAPPara(SettlementAggVO... aggVOs)
			throws BusinessException {
		// 获取生成凭证参数信息
		...
		for (SettlementAggVO settlementAggVO : aggVOs) {
			SettlementHeadVO head = (SettlementHeadVO) settlementAggVO
					.getParentVO();
			String pk_billtype = SettleUtils.getbilltype(
					head.getPk_tradetype(), head.getPk_group());
			String key = head.getPk_org() + head.getPk_tradetype();
			if (pk_billtype != null && pk_billtype.equals("F4")) {
				if (!mapPara.containsKey(key)) {
					String cmp36 = SysInit.getParaString(head.getPk_org(),
							CMPSysParamConst.CMP36);
					if (cmp36 == null
							|| (!cmp36.equals(CMPSysParamConst.CONST_SINGTODAP) && !cmp36
				...
				if (!mapPara.containsKey(key)) {
					String cmp37 = SysInit.getParaString(head.getPk_org(),
							CMPSysParamConst.CMP37);
					if (cmp37 == null
	 ...
	 * @throws BusinessException
	 */
	public static UFBoolean isBillEffect(UFBoolean isSign,
			SettlementAggVO settlementAggVO) throws BusinessException {
		UFBoolean isSignEffect = UFBoolean.TRUE;
		SettlementHeadVO head = (SettlementHeadVO) settlementAggVO
				.getParentVO();
		String pk_billtype = SettleUtils.getbilltype(head.getPk_tradetype(),
				head.getPk_group());
		if (pk_billtype != null && pk_billtype.equals("F4")) {
			...
			if (cmp36 == null
				...
				throw new BusinessException(nc.vo.ml.NCLangRes4VoTransl
						.getNCLangRes()
						.getStrByID("3607set_0", "03607set-0618")/*
																 * @res
																 * "请检查操作数据的CMP36参数设置"
																 */);
			}

			isSignEffect = cmp36.equals(CMPSysParamConst.CONST_SINGTODAP) ? UFBoolean.TRUE
					: UFBoolean.FALSE;
		} else if (pk_billtype != null && pk_billtype.equals("F5")) {
			...
			if (cmp37 == null
				...
				throw new BusinessException(nc.vo.ml.NCLangRes4VoTransl
						.getNCLangRes()
						.getStrByID("3607set_0", "03607set-0619")/*
																 * @res
																 * "请检查操作数据的CMP37参数设置"
																 */);
			}
			isSignEffect = cmp37.equals(CMPSysParamConst.CONST_SINGTODAP) ? UFBoolean.TRUE
					: UFBoolean.FALSE;
		}
	 ...
	 * @throws BusinessException
	 */
	public static void handleSettleInfoEffect(boolean isSettleOperate,
			SettlementAggVO... aggVOs) throws BusinessException {
		...
		for (SettlementAggVO settlementAggVO : aggVOs) {
			...
			} else if (SettleUtils.getbilltype(head.getPk_tradetype(),
					head.getPk_group()).equals("F6")) {
				// 划账结算单没有参数
				isSettleEffect = false;
			} else if (SettleUtils.getbilltype(head.getPk_tradetype(),
					head.getPk_group()).equals("F5")
					|| SettleUtils.getbilltype(head.getPk_tradetype(),
							head.getPk_group()).equals("F4")) {

				String dapStr = dapPara.get(head.getPk_org()
						+ head.getPk_tradetype());
				isSettleEffect = dapStr
						.equals(CMPSysParamConst.CONST_SETTLETODAP);
			}
	 ...
	 * @param aggVOs
	 */
	public static void fillSettleDate(UFDate busidate,
			SettlementAggVO... aggVOs) {
		for (SettlementAggVO settlementAggVO : aggVOs) {
			SettlementHeadVO head = (SettlementHeadVO) settlementAggVO
					.getParentVO();
			head.setSettledate(busidate);
			for (SettlementBodyVO body : (SettlementBodyVO[]) settlementAggVO
					.getChildrenVO()) {
	 ...
	 * @throws BusinessException
	 */
	public static void fillSettleBankrelated_code(SettlementHeadVO head,
			SettlementBodyVO[] bodyvos) throws BusinessException {
		UFBoolean cmp52 = SysinitAccessor.getInstance().getParaBoolean(
				head.getPk_org(), CMPSysParamConst.CMP52);
		if (UFBoolean.TRUE.equals(cmp52)) {
			// 欧盟业务，根据票据号带出对账标识码
			INoteBankrelatedCodeQueryService noteBankrelatedCodeQueryService = NCLocator
					.getInstance().lookup(
							INoteBankrelatedCodeQueryService.class);
			...
			for (SettlementBodyVO childrenVO : bodyvos) {
				if (StringUtils.isNotNullWithTrim(childrenVO.getNotenumber())) {
					...
					String hashkey = queryVO.getPk_org()
							+ queryVO.getBill_number()
							+ queryVO.getPk_notetype()
							+ queryVO.getPk_bankacc()
							+ queryVO.getPk_currtype();
			...
			Map<String, String> noteBankrelatedCode = noteBankrelatedCodeQueryService
					.getNoteBankrelatedCode(queryvolist);
			if (noteBankrelatedCode != null) {
				for (Map.Entry<String, String> entry : noteBankrelatedCode
						.entrySet()) {
		...
		if (nc.cmp.utils.CmpQueryModulesUtil.isOBMEnable(head.getPk_group())) {
			...
			String[] obmLogReconciliationCodes = CmpInterfaceProxy.INSTANCE
					.getObmLogManageService().creatObmLogReconciliationCodes(
							bodyList.size());
			for (int i = 0; i < bodyList.size(); i++) {
				bodyList.get(i).setBankrelated_code(
						obmLogReconciliationCodes[i]);
			}
		}
	 ...
	 * @param aggVOs
	 */
	public static void fillSettlementReverseStatus(boolean isIng,
			SettlementAggVO... aggVOs) {
		...
		for (SettlementAggVO settlementAggVO : aggVOs) {
			...
			if (head.getDirection().equals(Direction.REC.VALUE)) {
				head.setSettlestatus(isIng ? SettleStatus.RECEIVING.getStatus()
						: SettleStatus.NONESETTLE.getStatus());
			} else if (head.getDirection().equals(Direction.PAY.VALUE)) {
				head.setSettlestatus(isIng ? SettleStatus.PAYING.getStatus()
						: SettleStatus.NONESETTLE.getStatus());
			} else if (head.getDirection().equals(Direction.CHANGE.VALUE)) {
				head.setSettlestatus(isIng ? SettleStatus.CHANGEING.getStatus()
						: SettleStatus.NONESETTLE.getStatus());
			}
			...
			for (SettlementBodyVO body : (SettlementBodyVO[]) settlementAggVO
				...
				if (head.getDirection().equals(Direction.REC.VALUE)) {
					body.setSettlestatus(isIng ? SettleStatus.RECEIVING
							.getStatus() : SettleStatus.NONESETTLE.getStatus());

				} else if (head.getDirection().equals(Direction.PAY.VALUE)) {
					body.setSettlestatus(isIng ? SettleStatus.PAYING
							.getStatus() : SettleStatus.NONESETTLE.getStatus());

				} else if (head.getDirection().equals(Direction.CHANGE.VALUE)) {
					body.setSettlestatus(isIng ? SettleStatus.CHANGEING
							.getStatus() : SettleStatus.NONESETTLE.getStatus());
				}
	 ...
	 * @param aggVOs
	 */
	public static <T> void fillSettlementFieldAggInfo(T tvalue,
			String fieldname, SettlementAggVO... aggVOs) {
		for (SettlementAggVO settlementAggVO : aggVOs) {
			fillSettlementVOInfo(tvalue, fieldname,
					(SettlementHeadVO) settlementAggVO.getParentVO());
			fillSettlementVOInfo(tvalue, fieldname,
					(SettlementBodyVO[]) settlementAggVO.getChildrenVO());
		}
	}
	 ...
	 * @param aggVOs
	 */
	public static <T> void fillSettlementVOInfo(T tvalue, String fieldname,
			SuperVO... vos) {
		for (SuperVO vo : vos) {
	 ...
	 * @throws BusinessException
	 */
	public static void fillInnerOrgByHB(SettlementAggVO agg)
			throws BusinessException {
		...
		if (queryInnerAccount != null && queryInnerAccount.size() != 0
			...
			// 回拨存入内部帐户值
			fillSettlementVOInfo(queryInnerAccount.get(srcInfo[0]),
	 ...
	 * @throws BusinessException
	 */
	public static boolean isHBBusiness(SettlementAggVO agg)
			throws BusinessException {

		// 根据账户判断是否是回拨业务
		for (SettlementBodyVO body : (SettlementBodyVO[]) agg.getChildrenVO()) {
			BankAccbasVO bankaccbasvo = getAccbasVObySubBankAccPK(body
					.getPk_account());
			if (bankaccbasvo == null) {
				continue;
			}
	 ...
	 * @throws BusinessException
	 */
	private static String[] getHBSrcPK(SettlementAggVO agg)
			throws BusinessException {
		SettlementHeadVO Headvo = (SettlementHeadVO) agg.getParentVO();
		String billtype = SettleUtils.getbilltype(Headvo.getPk_tradetype(),
				Headvo.getPk_group());
		...
		if (billtype.equalsIgnoreCase(CmpConst.CONST_FITYPE_F3)) {
			...
			ICmpPayBillPubQueryService queryService = NCLocator.getInstance()
					.lookup(ICmpPayBillPubQueryService.class);
			BillAggVO[] aggvos = queryService
					.findBillByPrimaryKey(new String[] { Headvo
							.getPk_busibill() });
			if (CmpUtils.isListNull(aggvos)) {
				return null;
			}
			nc.vo.cmp.bill.BillVO head = (nc.vo.cmp.bill.BillVO) aggvos[0]
					.getParent();
		...
		// 非付款排成不处理
		if (!("36D1".equalsIgnoreCase(src_billtype) || "36D7"
				.equalsIgnoreCase(src_billtype))) {
			return null;
		}
	 ...
	 */
	public static boolean isPsBillFrozen(SettlementAggVO agg)
			throws BusinessException {

		SettlementHeadVO Headvo = (SettlementHeadVO) agg.getParentVO();
		// SettlementBodyVO[] bodys = (SettlementBodyVO[])agg.getChildrenVO();
		if (Headvo.getIsindependent() != null
				&& Headvo.getIsindependent().booleanValue()
		...
		String src_billtype = null;
		String src_billpk = null;
		if (Headvo.getIsindependent().booleanValue()
				|| !CmpQueryModulesUtil.isPSEnable(Headvo.getPk_group())) {
			return false;
		}

		if (billtype.equalsIgnoreCase("F3")
				&& CmpQueryModulesUtil.isAREnable(Headvo.getPk_group())) {
			/**
			...
			PayBillItemVO[] PayBillItemVOs = (PayBillItemVO[]) aggvos[0]
					.getChildrenVO();
		...
		Map<String, Boolean> psBillFrozen = NCLocator.getInstance()
				.lookup(IApplyPubService.class)
				.isPsBillFrozen(src_billtype, new String[] { src_billpk });
		if (psBillFrozen == null || psBillFrozen.size() == 0
				|| psBillFrozen.get(src_billpk) == null
	 ...
	 * @throws BusinessException
	 */
	public static void deleteObmLog(SettlementAggVO aggvo)
			throws BusinessException {
		SettlementHeadVO head = (SettlementHeadVO) aggvo.getParentVO();
		String id = head.getPrimaryKey();
		List<PaymentCrumbVO> list = InterfaceLocator.getInterfaceLocator()
				.getPaymentService().queryCrumb(id);
		...
		if (!CheckException.checkContionsIsNull(list)) {
			...
			LogMgrRequestVO logvo = LogMgrRequestVO.createDelRequest(yus
					.toArray(new String[] {}));
			...
			// 删除OBM日志
			NCLocator.getInstance().lookup(IObmLogManageService.class)
					.deleteObmLog(logvo);
		}
	}

	public static void addDynamicLock(AggregatedValueObject... settleAggs)
			throws BusinessException {
		if (settleAggs == null || settleAggs.length == 0
				|| settleAggs[0] == null) {
			return;
		}
	...
	public static void checkTs(AggregatedValueObject... settleAggs)
			throws BusinessException {
		...
		for (AggregatedValueObject agg : settleAggs) {
			...
			String ts = ((UFDateTime) ((SuperVO) agg.getParentVO())
					.getAttributeValue("ts")).toString();
			sql = "update " + tablename + " set dr=dr where " + tablePk
					+ " = ? and ts = ? ";
			int count = executeTsUpdate(sql, pk, ts);
			if (count < 1) {
				throw new BusinessException(nc.vo.ml.NCLangRes4VoTransl
						.getNCLangRes()
						.getStrByID("3607set_0", "03607set-0620")/*
																 * @res
																 * "数据已被更新！"
																 */);
			}
	...
	public static void checkTs(SettlementBodyVO... bodyVOs)
			throws BusinessException {
	...
	public static void addDynamicLock(SettlementBodyVO... bodyVOs)
			throws BusinessException {
		...
		boolean isLock = PKLock.getInstance().addBatchDynamicLock(
	 ...
	 * @throws BusinessException
	 */
	public static String getGroupPKByOrgPk(String pk_org)
			throws BusinessException {
		OrgVO[] orgs = CmpInterfaceProxy.INSTANCE.getOrgUnitPubService_C()
				.getOrgs(new String[] { pk_org },
						new String[] { OrgVO.PK_ORG, OrgVO.PK_GROUP });
		if (CmpUtils.isListNull(orgs)) {
			throw new BusinessException(nc.vo.ml.NCLangRes4VoTransl
					.getNCLangRes().getStrByID("3607set_0", "03607set-0825")/*
																			 * @res
																			 * "查找组织信息失败"
																			 */);
		} else {
	 ...
	 * @throws BusinessException
	 */
	public static void fillSettleinfoByLastPayInfo(SettlementAggVO aggvo)
			throws BusinessException {
		SettlementHeadVO head = (SettlementHeadVO) aggvo.getParentVO();

		Integer settlestatus = head.getSettlestatus() == null ? SettleStatus.NONESETTLE
				.getStatus() : head.getSettlestatus();

		if (head.getDirection() != null
				&& head.getDirection().equals(Direction.PAY.VALUE)
	 ...
	 * @throws BusinessException
	 */
	public static void fillSettleinfoByLastPayInfo(SettlementBodyVO bodyvo)
			throws BusinessException {
	 ...
	 * @throws BusinessException
	 */
	public static void setRealPayInfo(SettlementBodyVO bodyVO,
			UFDate busi_billdate) throws BusinessException {

		UFDate billdate = busi_billdate; // 单据日期
		// 组织本币信息
		UFDouble orgCurrRate = TMCurrencyUtil.getOrgCurrRate(
				bodyVO.getPk_org(), bodyVO.getPk_currtype_last(), billdate);
		UFDouble orgLocalMoney = TMCurrencyUtil.getOrgLocalMoney(
		...
		UFDouble groupCurrRate = TMCurrencyUtil.getGroupCurrRate(
				DataUtil.getCurrentGroup(), bodyVO.getPk_org(),
				bodyVO.getPk_currtype_last(), billdate);
		UFDouble groupLocalMoney = TMCurrencyUtil.getGroupLocalMoney(
				DataUtil.getCurrentGroup(), bodyVO.getPk_org(),
				bodyVO.getPk_currtype_last(), bodyVO.getPay_last(),
				groupCurrRate, bodyVO.getPaylocalrate_last(), billdate);

		bodyVO.setGrouppaylocal_last(groupLocalMoney == null ? UFDouble.ZERO_DBL
				: groupLocalMoney);
		...
		UFDouble globalCurrRate = TMCurrencyUtil.getGlobalCurrRate(
				bodyVO.getPk_org(), bodyVO.getPk_currtype_last(), billdate);
		UFDouble globalLocalMoney = TMCurrencyUtil.getGlobalLocalMoney(
				bodyVO.getPk_org(), bodyVO.getPk_currtype_last(),
				bodyVO.getPay_last(), globalCurrRate,
				bodyVO.getPaylocalrate_last(), billdate);

		bodyVO.setGlobalpaylocal_last(globalLocalMoney == null ? UFDouble.ZERO_DBL
				: globalLocalMoney);
	 ...
	 * @param bodys
	 */
	public static void setRealChangeBalance(SettlementBodyVO[] bodys)
			throws BusinessException {
		for (SettlementBodyVO bodyVO : bodys) {
			...
			if (bodyVO.getChangerate() != null) {
				if (bodyVO.getPk_currtype()
						.equals(bodyVO.getPk_currtype_last())) {
					fillSettleinfoByLastPayInfo(bodyVO);
				} else {
					// 实付金额=付款原币金额÷套汇汇率。
					bodyVO.setPay_last(bodyVO.getPay().div(
							bodyVO.getChangerate()));
	 ...
	 * @throws BusinessException
	 */
	public static void setSettleStatus(SettlementAggVO... aggvos)
			throws BusinessException {
		...
		for (SettlementAggVO settlementAggVO : aggvos) {
			...
			for (SettlementBodyVO body : (SettlementBodyVO[]) settlementAggVO
				...
				SettleStatus status = SettleStatus.valueof(body
						.getSettlestatus());
				...
				if (status.equals(SettleStatus.PAYING)) {
					...
					isHasEreset = true;
					if (settledate == null
							|| settledate.before(body.getTallydate())) {
					...
					if (body.getPk_balatype() == null) {
						...
						BalaTypeVO btvo = InterfaceLocator
								.getInterfaceLocator().getBalaQry()
								.findBalaTypeVOByPK(body.getPk_balatype());
						if (btvo.getNetbankflag() == null
								|| btvo.getNetbankflag()
										.equals(UFBoolean.FALSE)) {
							isHasSuccesssettle = true;
						} else {
			...
			} else if (isHasPayFAIL
					&& (isHasSuccesssettleForNet || isHasSuccesssettle)) {
				// 网银操作有成功有失败
				head.setSettlestatus(SettleStatus.SUCCESSPART.getStatus());
			} else if (isHasPayFAIL
					&& !(isHasNotSettle || isHasEreset || isHasPaying
							|| isHasPrechange || isHasReceiving
							|| isHasSuccesssettle || isHasSuccesssettleForNet)) {
				// 只有网银支付失败的情况
			...
			// 设置结算日期
			if (head.getSettlestatus().equals(
	 ...
	 * @throws BusinessException
	 */
	public static List<SettlementAggVO> filterSettleInfo4HandSettleFlag4UnSettle(
			SettlementAggVO... aggVOs) throws BusinessException {
			...
			for (SettlementBodyVO body : (SettlementBodyVO[]) settlementAggVO
				...
				if (pk_balatype == null) {
					...
					if (!map.containsKey(pk_balatype)) {
						btvo = InterfaceLocator.getInterfaceLocator()
								.getBalaQry().findBalaTypeVOByPK(pk_balatype);
					...
					if (btvo.getNetbankflag() == null
							|| btvo.getNetbankflag().equals(UFBoolean.FALSE)) {
						bodyLst.add(body);
					}
				}
			}
	 ...
	 * @throws BusinessException
	 */
	public static List<SettlementAggVO> filterSettleInfo4HandSettleFlag4Settle(
			SettlementAggVO... aggVOs) throws BusinessException {
		...
		for (SettlementAggVO settlementAggVO : aggVOs) {
			SettlementHeadVO head = (SettlementHeadVO) settlementAggVO
					.getParentVO();
			if (Integer.valueOf(SettleStatus.SUCCESSSETTLE.getStatus()).equals(
					head.getSettlestatus())
					&& (UFBoolean.TRUE.equals(head.getIsreset()) || head
							.getDirection().equals(Direction.REC.VALUE))) {
				// 已结算的红冲单据或收款结算单不用过滤
				agglst.add(settlementAggVO);
				continue;
			}
			...
			for (SettlementBodyVO body : (SettlementBodyVO[]) settlementAggVO
					.getChildrenVO()) {
				if ((body.getSettlestatus() == null || body.getSettlestatus()
						.equals(SettleStatus.NONESETTLE.getStatus()))) {
					continue;
				}
				...
				if (body.getPk_balatype() == null) {
					...
					BalaTypeVO btvo = InterfaceLocator.getInterfaceLocator()
							.getBalaQry()
							.findBalaTypeVOByPK(body.getPk_balatype());
					if (btvo.getNetbankflag() == null
							|| btvo.getNetbankflag().equals(UFBoolean.FALSE)) {
	 ...
	 * @throws BusinessException
	 */
	public static List<SettlementAggVO> filterSettleInfo4HandSettle(
			SettlementAggVO... aggVOs) throws BusinessException {
		// return filterSettleInfo4NetFlag(true,0, aggVOs);
		List<SettlementAggVO> agglst = CmpUtils.makeList();
		for (SettlementAggVO settlementAggVO : aggVOs) {
	 ...
	 * @throws BusinessException
	 */
	public static List<SettlementAggVO> filterSettleInfo4NetSettleFlagUnSettle(
			SettlementAggVO... aggVOs) throws BusinessException {
		...
		for (SettlementAggVO settlementAggVO : aggVOs) {
			...
			if (headvo.getSettlestatus() != null
					&& SettleStatus.SUCCESSSETTLE.getStatus() == headvo
							.getSettlestatus()) {
				// 已结算的跳过
				continue;
			}
			...
			for (SettlementBodyVO body : (SettlementBodyVO[]) settlementAggVO
				...
				if (body.getSettlestatus() != null
						&& !(body.getSettlestatus().equals(
								SettleStatus.NONESETTLE.getStatus()) || body
								.getSettlestatus().equals(
										SettleStatus.PAYFAIL.getStatus()))) {
					continue;
				}
				if (body.getPk_balatype() != null) {
					BalaTypeVO btvo = InterfaceLocator.getInterfaceLocator()
							...
							.findBalaTypeVOByPK(body.getPk_balatype());
					if (btvo.getNetbankflag() != null
							&& btvo.getNetbankflag().equals(UFBoolean.TRUE)) {
						bodyLst.add(body);
					}
				}
			}
	 ...
	 * @throws BusinessException
	 */
	public static List<SettlementAggVO> filterSettleInfo4NetSettleFlagSettle(
			SettlementAggVO... aggVOs) throws BusinessException {
		// return filterSettleInfo4NetFlag(false,0, aggVOs);
	 ...
	 * @throws BusinessException
	 */
	public static List<SettlementAggVO> filterSettleInfo4NetSettleFlag(
			SettlementAggVO... aggVOs) throws BusinessException {
	 ...
	 * @throws BusinessException
	 */
	public static boolean isMakeBill(SettlementHeadVO head)
			throws BusinessException {
		// 暂时这么处理，以后可以考虑通过冗余字段来减少查询
		FipRelationInfoVO infovo = new FipRelationInfoVO();
		String billType = (String) head
				.getAttributeValue(SettlementHeadVO.PK_TRADETYPE);
		infovo.setPk_billtype(billType);
		infovo.setRelationID((String) head
				.getAttributeValue(SettlementHeadVO.PK_BUSIBILL));

		if (infovo.getPk_billtype() == null || infovo.getRelationID() == null) {
			// 缺少业务单据信息，独立结算信息，视为未制单。
			return false;
		}
		infovo.setPk_group((String) head
				.getAttributeValue(SettlementHeadVO.PK_GROUP));
		infovo.setPk_org((String) head
				.getAttributeValue(SettlementHeadVO.PK_ORG));
		UFBoolean[][] queryDesBillState = CmpInterfaceProxy.INSTANCE
				.getFipRelationQueryService().queryDesBillState(
						new FipRelationInfoVO[] { infovo }, null);

		boolean isMakeBill = queryDesBillState == null
				|| queryDesBillState[0] == null
				|| queryDesBillState[0][0] == null ? false
				: queryDesBillState[0][0].booleanValue();
