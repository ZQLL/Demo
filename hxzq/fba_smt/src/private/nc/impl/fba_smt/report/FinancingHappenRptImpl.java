...
import nc.itf.fba_smt.report.IFinancingHappenRpt;
...
public class FinancingHappenRptImpl implements IFinancingHappenRpt {
	...
	private DataSet getAvailableDetailDataSet(IContext context,
		...
		for (AvailableDetailVO vo : trademap.values()) {
			...
			if (firstnum.compareTo(UFDouble.ZERO_DBL) == 0) {
				vo.setFirstMarket(null);
			} else {
				vo.setFirstMarket(firstMarket);
			}
			// 将期间减少的体现为负数
			//向客户融出证券
			if (vo.getRczqnum() != null) {
				vo.setRczqnum(UFDouble.ZERO_DBL.sub(vo.getRczqnum()));
			}
			if (vo.getRczqsum() != null) {
				vo.setRczqsum(UFDouble.ZERO_DBL.sub(vo.getRczqsum()));
			}
			//转回可供出售证券
			if (vo.getKrzkgnum() != null) {
				vo.setKrzkgnum(UFDouble.ZERO_DBL.sub(vo.getKrzkgnum()));
			}
			if (vo.getKrzkgsum() != null) {
				vo.setKrzkgsum(UFDouble.ZERO_DBL.sub(vo.getKrzkgsum()));
			}
			//归还融入
			if (vo.getZrtghnum() != null) {
				vo.setZrtghnum(UFDouble.ZERO_DBL.sub(vo.getZrtghnum()));
			}
			if (vo.getZrtghsum() != null) {
				vo.setZrtghsum(UFDouble.ZERO_DBL.sub(vo.getZrtghsum()));
			}
		}
	...
	private Collection<AvailableDetailVO> getStockOutDetailDataSet(
		...
		for (AvailableDetailVO vo : trademap.values()) {
			...
			// 将期间减少的体现为负数
			//客户归还证券
			if (vo.getRcghnum() != null) {
				vo.setRcghnum(UFDouble.ZERO_DBL.sub(vo.getRcghnum()));
			}
			if (vo.getRcghsum() != null) {
				vo.setRcghsum(UFDouble.ZERO_DBL.sub(vo.getRcghsum()));
			}
		}
		Collection<AvailableDetailVO> c = trademap.values();
		return c;
