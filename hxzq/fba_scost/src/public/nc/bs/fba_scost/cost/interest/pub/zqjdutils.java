...
//	
	...
	public  void zqjdautoAudit(BilltypeGroupVO vo,CostingTool costingtool, TradeDataTool tradedatatool,CheckClassTool checkclasstool) throws Exception{
	...
	private void auto(BilltypeGroupVO vo,CostingTool costingtool, TradeDataTool tradedatatool,CheckClassTool checkclasstool)throws Exception{
		...
		//先校验是否生成归还单
//		this.autoreturn(vo,   dataMap ,costingtool, tradedatatool,  checkclasstool);
		//参数控制是否生成兑付单
		boolean falg = this.getBooleanFromInitcode(pk_glorgbook,
				"SEC40");
		if (!falg)
			return;
		//生成相关单据 转兑付单、结转单等
...
//			new BaseDAO().insertVOList(early);
	...
	public void autoreturn(BilltypeGroupVO vo, Map<String, List<PendingBillVO>> dataMap,CostingTool costingtool, TradeDataTool tradedatatool,CheckClassTool checkclasstool) throws Exception {
		CostParaVO costpara = costingtool.getCostParaVO();
		Integer checkstate = costpara.getCheckstate();
		switch (checkstate) {
		case 0:// 审核
			this.setreturn(vo,  dataMap,costingtool, tradedatatool, checkclasstool);
			break;
		  }

		}
	
	private void setreturn(BilltypeGroupVO vo, Map<String, List<PendingBillVO>> dataMap,CostingTool costingtool, TradeDataTool tradedatatool,CheckClassTool checkclasstool) throws Exception{
		CostParaVO costpara = costingtool.getCostParaVO();
		//
		CheckParaVO checkParaVO = costpara.getCheckParavo();
	//将归还单的生成位置移植到这里来   LQM
		...
		for (int j = 0; j < plugins.size(); j++) {
			...
			try {
				...
				ispbusiness = (GZISpecialBusiness) NewObjectService.newInstance(temp[2], sbimpl);
			    } catch (Exception e) {
				throw new BusinessException(sbimpl + "实例化失败！" + e.getMessage());
			}
			ISuperVO[] vos = ispbusiness.query(vo, costingtool,  tradedatatool,  checkclasstool );
			if (null != vos && vos.length > 0) {
				...
				if(zhdlistpks!=null&&zhdlistpks.size()>0){
					this.autozhd(zhdlistpks, vo, dataMap, costingtool, tradedatatool, checkclasstool);
				}	
				//再处理归还单
				if(returnpks!=null&&returnpks.length>0){
					autoghd(returnpks, vo, dataMap, costingtool, tradedatatool, checkclasstool);	
				}
			 }
	...
	//审核转换单
	private void autozhd(List<String>  pks,BilltypeGroupVO vo, Map<String, List<PendingBillVO>> dataMap,CostingTool costingtool, TradeDataTool tradedatatool,CheckClassTool checkclasstool) throws Exception{
		//根据pk重新查询转换单
			AggTransformtradeVO[] aggs = getBusiInstance().queryBillByPK(pks.toArray(new String[0]));
			//审核转换单
			this.costcalc = ((ICostForward) NCLocator.getInstance().lookup(
					ICostForward.class.getName()));
			String pk_user = costingtool.getCostParaVO().getCheckParavo().getPk_user();
			if (aggs != null && aggs.length > 0) {
				//审核之前，先将系统的变量设置完整 checkclasstool  costingtool
				//将系统变量更新
				SetaddreturnUtils setUtils=new SetaddreturnUtils();			 
				setUtils.AddBillTypeClass(dataMap,"HV3F-0xx-01",  vo,  costingtool,tradedatatool,checkclasstool);
						...
						if(pbvo.getPk_billtype().equals("HV3F-0xx-01")){
							...
							try {
								for (IBill ibill : aggs) {
									TransformtradeVO fathervo = (TransformtradeVO) ibill.getParent();// 无孩子
									...
									fathervo.setApprovedate(PubMethod.getInstance().getDateTime(costingtool.getCurrdate()));
									fathervo.setState(SystemConst.statecode[3]);
								}		
							//s	ibillcheck.checkBills(costingtool, tradedatatool);
							} catch (Exception e) {
	...
	//审核归还单
	private void autoghd(String [] pks,BilltypeGroupVO vo, Map<String, List<PendingBillVO>> dataMap,CostingTool costingtool, TradeDataTool tradedatatool,CheckClassTool checkclasstool) throws Exception{
		//根据pk重新查询转换单
			AggZqjdVO[] aggs = getBusiInstance1().queryBillByPK(pks);
	   //		AggTransformtradeVO[] aggs = getBusiInstance().queryBillByPK(zhdlistpks.toArray(new String[0]));
			//审核转换单
			...
			if (aggs != null && aggs.length > 0) {
				...
				setUtils.AddBillTypeClass(dataMap,"HV7A-0xx-02",  vo,  costingtool,   tradedatatool,  checkclasstool);
				...
				if (billtypeclass != null) {
					for (int i = 0; i < billtypeclass.size(); i++) {
						PendingBillVO pbvo = (PendingBillVO) billtypeclass.get(i);
						//只审核归还单，其他单据已经审核了
						if(pbvo.getPk_billtype().equals("HV7A-0xx-02")&&(pbvo.getTrade_date().toString().substring(0, 10)).equals(costingtool.getCurrdate())){
							costingtool.setCurrbilltype(pbvo.getPk_billtype());// 设置当前交易类型
							costingtool.setPk_stocksort(new String[] { pbvo.getPk_stocksort1(), pbvo.getPk_stocksort2() });
							costingtool.setDirection(pbvo.getDirection());
							costingtool.setIsinit(false);// 此处 重新设置缓存标志 非第一次初始化 这个 很重要
							...
							try {
								for (IBill ibill : aggs) {
									...
									vo1.setApprovedate(PubMethod.getInstance().getDateTime(
											costingtool.getCurrdate()));
	...
	private void checkQuantityAndMoney(StockBalanceVO stockBalanceVO, ZqjdVO tradevo) throws ValidationException {
	...
	private void calculateWhenCheck1(ICostingTool costingtool, ZqjdVO tradevo) throws Exception {
		// 补充默认值
//		tradevo = DefaultValTool.setDefaultVal(tradevo);
		...
		stockBalanceVO.setStocks_num(SafeCompute.sub(befStockNum, tradevo.getBargain_num()));
		stockBalanceVO.setStocks_sum(SafeCompute.sub(befStockSum, tradevo.getBargain_sum()));
		// 更新库存
		...
		private IZqjdtradeMaintain getBusiInstance1() {
			IZqjdtradeMaintain iBondbuy = (IZqjdtradeMaintain) NCLocator
					.getInstance().lookup(IZqjdtradeMaintain.class);
			return iBondbuy;
		}
		...
		//生成的转换单审批是处理
		protected void calculate(ICostingTool costingtool, TransformtradeVO tradevo) throws Exception {
			...
			String[] keys = new String[costingtool.getCostParaVO().getCostplanvo().getCostFieldArray().length - 1];
			for (int i = 0; i < keys.length; i++) {
				keys[i] = costingtool.getCostParaVO().getCostplanvo().getCostFieldArray()[i];
			}
			String hckey = getCombinesKey(tradevo, keys, false);
			/**
			 * 此处拼接key必须加上业务组
			 */
			hckey=costingtool.getCurrbilltypegroupvo().getPk_billtypegroup()+hckey;
			...
			// 库存余额表
			StockBalanceVO outstockbalancevo = balanceTool.getStockbalanceVO(queryvo,costingtool);
			...
			//zpm 实际利率 增加开始，位置不要动
			if(outstockbalancevo.getStock_map().get(tradevo.getTrade_date().toLocalString()) == null){
				outstockbalancevo.getStock_map().put(tradevo.getTrade_date().toLocalString(), outstockbalancevo.getStocks_num());
			}
			
			//计算结转应收利息

			UFDouble lx = costcalc.forwardInterestDistill(costingtool, outstockbalancevo,tradevo);
			tradevo.setInterest(lx);
			//计算结转公允价值
			UFDouble fv = costcalc.forwardFairValueDistill(costingtool,outstockbalancevo, tradevo);
			...
			if (tradevo.getBargain_num2() == null || tradevo.getBargain_num2().compareTo(new UFDouble(0)) == 0) {
				
				tradevo.setBargain_num2(tradevo.getBargain_num());// 如果转入数量为空，那么转入数量等于转出数量
			}
			// 获得剩余数量
			stocks_num = pm.sub(outstockbalancevo.getStocks_num(), tradevo.getBargain_num());
			// 判断转出是否合理
			...
			if (stocks_num.compareTo(new UFDouble(0)) == 0) {// 卖空
				...
				if (isFirstPrice) {// 先计算单价
					stocks_sum = pm.div(outstockbalancevo.getStocks_sum(), outstockbalancevo.getStocks_num());
					stocks_sum = pm.multiply(stocks_sum, tradevo.getBargain_num());
				} else {
					stocks_sum = pm.multiply(outstockbalancevo.getStocks_sum(), tradevo.getBargain_num());
					stocks_sum = pm.div(stocks_sum, outstockbalancevo.getStocks_num());
				}
				stocks_sum = pm.setScale(stocks_sum, true, true);
			}
			UFDouble taxOutCost = costingtool.getTaxOutcost(queryvo,costingtool, tradevo.getBargain_num());
			 ...
			 */
			if(tradevo.getBargain_sum()==null||tradevo.getBargain_sum().compareTo(new UFDouble(0))<=0){
				tradevo.setBargain_sum(stocks_sum);
				tradevo.setAttributeValue("bargain_sum_his", null);
			}else{
				tradevo.setAttributeValue("bargain_sum_his", tradevo.getBargain_sum());
			}
//			tradevo.setBargain_sum(stocks_sum);
			/**
			 ...
			 * @date 2015-12-07 09：55
			 */
			if(tradevo.getBargain_sum2()==null||tradevo.getBargain_sum2().compareTo(new UFDouble(0))<=0){
				tradevo.setBargain_sum2(tradevo.getBargain_sum());
				tradevo.setAttributeValue("bargain_sum2_his", null);
			}else{
				tradevo.setAttributeValue("bargain_sum2_his", tradevo.getBargain_sum2());
//				tradevo.setBargain_sum2(tradevo.getBargain_sum());
			}
//			tradevo.setBargain_sum2(stocks_sum);
			tradevo.setSellcost(stocks_sum);
			outstockbalancevo.setStocks_num(stocks_num);
			outstockbalancevo.setStocks_sum(pm.sub(outstockbalancevo.getStocks_sum(), stocks_sum));
			outstockbalancevo.setStocks_tax(pm.sub(outstockbalancevo.getStocks_tax(), taxOutCost));
			// 更新转出库存
			...
			hrkey=costingtool.getCurrbilltypegroupvo().getPk_billtypegroup()+hrkey;
			...
			StockBalanceVO instockbalancevo = balanceTool.getStockbalanceVO(queryvo,costingtool);
			if (instockbalancevo == null) {
				instockbalancevo = balanceTool.getStockbalanceVOByVO(tradevo,costingtool);
			...
			// 获得转入的数量
			stocks_num = pm.add(instockbalancevo.getStocks_num(), tradevo.getBargain_num2());
			// 获得转入的金额
			stocks_sum = pm.add(instockbalancevo.getStocks_sum(), tradevo.getBargain_sum2());
			// 保存转入信息
			instockbalancevo.setStocks_num(stocks_num);
			instockbalancevo.setStocks_sum(pm.setScale(stocks_sum, true, true));
			instockbalancevo.setStocks_tax(pm.add(instockbalancevo.getStocks_tax(), taxOutCost));
			// 更新转入的库存
			...
			scostZrlxUtils utils=new scostZrlxUtils();
				UFDouble zrlx = utils.forwardInterestDistill(costingtool, outstockbalancevo,tradevo);
				if(lastnum.compareTo(new UFDouble(0))>0){
					tradevo.setVdef10(((zrlx.multiply(tradevo.getBargain_num2().div(new UFDouble(lastnum)))).setScale(2, 4)).toString());
				}
				else{
				...
				instockbalancevo.setVdef1(lastnum.sub(tradevo.getBargain_num2()).toString());
				instockbalancevo.setVdef2(lastsum.sub(tradevo.getBargain_sum2()).toString());
			//保存台账
	...
	private  Map<String, List<SuperVO>> calcAutoPayInfo(CostingTool costingtool,
			...
			UFDate trade_date,BilltypeGroupVO vo, TradeDataTool tradedatatool) throws BusinessException {
		
		// 查询债券借贷库存信息
	         String	sql_bow_now = getQueryInfo().getNowSql("HV7A-0xx-01", trade_date.toString());
	         ArrayList<ZqjdVO> 	nowBowResult = (ArrayList<ZqjdVO>) baseDAO.executeQuery(
		...
		for (ZqjdVO v : nowBowResult) {
			buildHbFxBill(pk_group, pk_org, v, trade_date, map,  vo,   tradedatatool,  costingtool);
		}
	 ...
	 * 查询当前债券是否还本到期
	 */
	private  void buildHbFxBill(String pk_group, String pk_org,
			ZqjdVO zqjdvo, UFDate trade_date,
			Map<String, List<SuperVO>> map,BilltypeGroupVO vo, TradeDataTool tradedatatool,CostingTool costingtool) throws BusinessException {
	    //当前时期的借入单是否利息结转日期
	 ...
	 * 生成转对付单
	 */
	public SuperVO genZDFBill(String pk_group, String pk_org,
			ZqjdVO zqjdvo, UFDate trade_date)
		...
		vo.setAttributeValue("bargain_sum", calcLX( zqjdvo.getBargain_num(), agginfo, trade_date) );
		...
		vo.setAttributeValue("creationtime",AppContext.getInstance().getServerTime());
	...
	//	vo.setAttributeValue("approver", AppContext.getInstance().getPkUser());
		...
		vo.setAttributeValue("makedate", AppContext.getInstance().getServerTime());
		//vo.setAttributeValue("approvedate", AppContext.getInstance().getServerTime());
		return vo;
	}
	 ...
	 */
	public UFDouble calcLX(UFDouble num, AggInterest agginfo,
			UFDate trade_date) {
		...
		if (CostConstant.PAYONCEATEND.equals(headvo.getPaytype())) {// 到期一次还本付息----自定义
			for (Rateperiod vo : ratevos) {
				UFDouble lx1 = SafeCompute.multiply(num,
						vo.getYear_rate());
				...
				if (!isAllYear(vo.getStart_day(), vo.getEnd_day())) {
					...
					lx = lx1.multiply(realDays).div(allDays);//update lihaibo 用下面的算法小数点位数不对
...
//							SafeCompute.div(realDays, allDays));
			...
			lx = SafeCompute.multiply(num,
					vo.getYear_rate()).multiply(hasSurplusPercent);
			...
			} else if (CostConstant.PAYPREHALFYEAR.equals(headvo
					.getPeriodtype())) {// 按半年付息
				lx = SafeCompute.div(lx, new UFDouble(2));
			} else if (CostConstant.PAYPREQUARTER
					.equals(headvo.getPeriodtype())) {// 按季付息
...
//				UFDouble allDays = this.getYearDays(headvo, vo.getStart_day(),
//						vo.getEnd_day());// 年天数
				UFDouble allDays = new UFDouble(vo.getDays_num());
				lx = lx.multiply(realDays).div(allDays);//update lihaibo 用下面的算法小数点位数不对
...
//						SafeCompute.div(realDays, allDays));
	 ...
	 */
	public String getBillNO(String pk_group, String pk_org, SuperVO fv,String suffix) throws BusinessException {
		String vbillcode = null;
		try {
			IBillcodeManage ser = NCLocator.getInstance().lookup(
					IBillcodeManage.class);
			vbillcode = ser.getBillCode_RequiresNew("HV7A",
					pk_group, pk_org, fv);
