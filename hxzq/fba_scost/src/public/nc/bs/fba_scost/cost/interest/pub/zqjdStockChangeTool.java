...
public class zqjdStockChangeTool {
	 ...
	 * 根据证券档案ID查询利率信息
	 */
	public AggInterest  queryInterestsetInfo(String pk_securities)throws BusinessException{
		// start 20150612 性能优化 by:mx
		// String str = " pk_securities = '"+pk_securities+"' and isnull(dr,0) = 0 ";
		...
		// Interest[] vos = (Interest[])DBCacheQueryFacade.runQuery(sql, param, new BeanListProcessor(Interest.class));
		/** JINGQT 2015年7月6日 性能优化产生的BUG修正 DEL END */
		/** JINGQT 2015年7月6日 性能优化产生的BUG修正 ADD START */
		List<Interest> vos = (List<Interest>)DBCacheQueryFacade.runQuery(sql, param, new BeanListProcessor(Interest.class));
		/** JINGQT 2015年7月6日 性能优化产生的BUG修正 ADD END */
		// Interest[] vos = (Interest[])new HYSuperDMO().queryByWhereClause(Interest.class,str);
		//end 20150612 性能优化 by:mx
		if(vos == null || vos.size() == 0){
			SecuritiesVO  vo = querySecuritiesVO(pk_securities);
			throw new BusinessException(" 证券档案 ["+vo.getCode() +"、"+vo.getName() +"] 利率没有设置! ");
		}
		// start 20150612 性能优化 by:mx
		// String condition = " pk_interest = '"+vos[0].getPk_interest()+"' and isnull(dr,0) = 0  ";
		//fulq 后续需要计算 提前还款 根据 开始日期 排序
//		String sql_rate = "select * from sim_rateperiod where pk_interest = ?  and isnull(dr,0) = 0";
		...
		// Rateperiod[] ratevos = (Rateperiod[])DBCacheQueryFacade.runQuery(sql_rate, param_rate, new BeanListProcessor(Rateperiod.class));
		...
		List<Rateperiod> ratevos = (List<Rateperiod>)DBCacheQueryFacade.runQuery(sql_rate, param_rate, new BeanListProcessor(Rateperiod.class));
		// end 20150612 性能优化 by:mx
		if(ratevos == null || ratevos.size() == 0){
			SecuritiesVO vo = querySecuritiesVO(pk_securities);
			throw new BusinessException(" 证券档案 [" + vo.getCode() + "、" + vo.getName() + "] 利率表体信息没有设置! ");
		}
...
//	 * @return
//	 */
//	public static List<StockChangeVO> fullWithZero(List<StockChangeVO> lastResult) {
