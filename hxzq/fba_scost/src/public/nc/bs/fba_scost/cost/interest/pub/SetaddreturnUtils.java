...
public class SetaddreturnUtils {
	 ...
	 * @throws Exception
	 */
	public Map<String, List<PendingBillVO>> AddBillTypeClass(
			Map<String, List<PendingBillVO>> returnmap, String pk_billtype,
			BilltypeGroupVO vo, CostingTool costingtool,
			TradeDataTool tradedatatool, CheckClassTool checkclasstool)
			throws Exception {
		List<PendingBillVO> plist = this.getNeedVOs(costingtool, pk_billtype,
				vo, costingtool.getCostParaVO().getCheckParavo(), " state = "
						+ SystemConst.statecode[1], SystemConst.checkstate[0]);
		...
		for (int i = 0; i < plist.size(); i++) {
			...
			BilltypeDetailsVO bdvo = (BilltypeDetailsVO) ((map.get(checkParaVO
					.getPk_group()
			...
			try {
				...
				isimcheck = (IBillCheckPlugin) NewObjectService.newInstance(
						temp[2], classname);
			} catch (Exception e) {
			 ...
			 * 初始化审核类
			 */
			checkclasstool.setCheckClassMap(transtypecode + "##" + classname,
					isimcheck);
	 ...
	 */
	private List<PendingBillVO> getNeedVOs(CostingTool costingtool,
			String pk_billtype, BilltypeGroupVO vo, CheckParaVO checkParaVO,
			String addcondition, int state) throws BusinessException {
		...
		if (vo.getLastapprovedate() != null) {
			...
			sb.append(new UFDate(costingtool.getCurrdate()).toLocalString()
		...
		if (plist == null || plist.size() == 0) {
			PendingBillVO pb = this.bulidPendingBill(vo, checkParaVO,
					pk_billtype, new UFDate(costingtool.getCurrdate()));
			String pk = server.insert(pb);
			PendingBillVO pb1 = (PendingBillVO) server.queryByPrimaryKey(
					PendingBillVO.class, pk);
			if (plist == null) {
				plist = new ArrayList();
			}
			plist.add(pb1);
	...
	public PendingBillVO bulidPendingBill(BilltypeGroupVO vo,
			CheckParaVO checkParaVO, String transtypecode, UFDate trade_date) {
