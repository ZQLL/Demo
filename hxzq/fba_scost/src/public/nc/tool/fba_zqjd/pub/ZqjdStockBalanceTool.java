	...
	public StockBalanceVO qryStockBalnce(BanlanceQueryKeyVO queryvo,
		...
		try {
			...
			if (stockbalancevo == null) {
				...
				//默认设置负债库存 方案1 遗留
//				stockbalancevo.setAttributeValue(SystemConst.Fields_AssetDebt, SystemConst.Values_AssetDebt[1]) ;
			}
		} catch (BusinessException e) {
			Logger.error(e.getMessage());
			ExceptionUtils.wrappBusinessException(e.getMessage());
	...
	private void checkStocksort(ZqjdVO tradevo) throws BusinessException {
		...
		if (StringUtils.isEmpty(tradevo.getPk_stocksort())) {
			throw new BusinessException("单据号：" + tradevo.getBillno() + "，默认库存组织为空，请检查外部数据导入配置库存组织信息！");
		}
	}
	...
	public BanlanceQueryKeyVO createQryBalanceVO(ICostingTool costingtool,
		...
		String pk_stocksort = (tradevo.getPk_stocksort() == null ? SystemConst.Pk_StockSort1 : tradevo.getPk_stocksort());
...
//		String[] newArray = new String[costFieldArray.length + 1];
		// 数组拷贝
//		System.arraycopy(costFieldArray, 0, newArray, 0, costFieldArray.length);
		// 增加 资产负债维度
