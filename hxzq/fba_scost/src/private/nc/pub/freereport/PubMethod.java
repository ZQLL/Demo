...
public class PubMethod {
	...
	private IUAPQueryBS getQueryservice() {
		if (queryservice == null) {
			queryservice = (IUAPQueryBS) NCLocator.getInstance().lookup(IUAPQueryBS.class.getName());
		}
		return queryservice;
	}
	 ...
	 */
	public List<CostPlanVO> getCostPlanVOs(String pk_org, String pk_glorgbook, String pk_group) throws BusinessException {
		...
		return (List<CostPlanVO>) getQueryservice().retrieveByClause(CostPlanVO.class, sqlwhere.toString());
	}
	 ...
	 * @throws BusinessException
	 */
	public CostPlanVO getCostPlanVO(String pk_org, String pk_glorgbook, String pk_group) throws BusinessException {
	...
	public String getDeGroup(String pk_org) throws BusinessException {
		OrgVO orgVo = (OrgVO) getQueryservice().retrieveByPK(OrgVO.class, pk_org);
		if (orgVo != null)
	 ...
	 * @throws BusinessException
	 */
	public <T extends CircularlyAccessibleValueObject> Map<String, T> hashlizeObject(List<T> objs, String[] keys) throws BusinessException {
	 ...
	 * @return
	 */
	public <T extends CircularlyAccessibleValueObject> Map<String, List<T>> hashlizeObjects(List<T> objs, String[] keys) {
		if (objs == null || objs.isEmpty()) {
	...
	public DataSet convertVOToArray(MetaData metaData, List<? extends SuperVO> vos) {
	 ...
	 * @date 2016-03-03
	 */
	public Map<String,SecuritiesVO> getSecuritiesVOSByOrg(String org) throws BusinessException{
		String wheresql="nvl(dr,0)=0 and nvl(state,0)=0";
		List<SecuritiesVO> result=(List<SecuritiesVO>)getQueryservice().retrieveByClause(SecuritiesVO.class, wheresql);
		Map<String,SecuritiesVO> securitesMap=new HashMap<String,SecuritiesVO>();
		if(result!=null&&result.size()>0){
			for(int i=0;i<result.size();i++){
				securitesMap.put(result.get(i).getPk_securities(), result.get(i));
			}
		}
		return securitesMap;
	}
