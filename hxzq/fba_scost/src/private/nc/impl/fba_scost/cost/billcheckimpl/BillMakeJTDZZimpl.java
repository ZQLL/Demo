...
public class BillMakeJTDZZimpl implements IBillMakeJTDZZ {
	...
	public void MakeJTD(CheckParaVO checkParaVO, UFDate date) {
		...
		String User = checkParaVO.getPk_user();
		String time = checkParaVO.getTrade_date().toString();
		try {
			...
			if (yshzqjd.size() > 0) {
				...
				for (int x = 0; x < yshzqjd.size(); x++) {
					...
					if (yshzqjdvo != null) {
						...
						// 如果借入单结束时间大于当前审批时间
						if (new UFDate(enddate).after(new UFDate(appdate))) {
							// 查询之前最后一次审核生成的计提单
						...
						if (mm.size() != 0) {
							...
							jtdVO.setApprover(User);
							jtdVO.setCreator(User);
							jtdVO.setBillmaker(User);
							jtdVO.setMakedate(new UFDateTime(time));
							jtdVO.setCreationtime(new UFDateTime(time));
							jtdVO.setModifier(null);
							jtdVO.setModifiedtime(null);
							...
							if (jtdVO.getTrade_fee() == null) {
								throw new NullPointerException("合同号"
										+ yshzqjdvo.getContractno()
										+ " 的借贷费用为空");
							}
							dao.insertVO(jtdVO);
