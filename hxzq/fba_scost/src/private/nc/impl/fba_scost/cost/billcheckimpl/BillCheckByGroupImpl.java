...
public class BillCheckByGroupImpl implements IBillCheckByGroup {
	...
	public void tallyOneGroup_RequiresNew(BilltypeGroupVO vo, Set<UFDate> set,
			Map<String, List<PendingBillVO>> dataMap, CheckParaVO checkParaVO,
		...
		try {
			...
			NCLocator
					...
					.updateMiddleAndApporveUI_RequiresNew(dataMap,
							pk_billtypegroup, SystemConst.statecode[4], msg);
		}
	}

	public void uncheckOneGroup_RequiresNew(BilltypeGroupVO vo,
			Set<UFDate> set, Map<String, List<PendingBillVO>> dataMap,
			Map<String, List<PendingBillVO>> needuncheckdataMap,
			CheckParaVO checkParaVO, TradeDataTool tradedatatool,
		...
		try {
			for (UFDate date : set) {
				...
				checkTally(pk_billtypegroup, date);
				if (dataMap != null
						&& vo.getLasttallydate() != null
						&& date.toLocalString().compareTo(
								vo.getLasttallydate().toLocalString()) <= 0) {
					// 走取消记账逻辑
					List<PendingBillVO> billtypeclass = dataMap.get(key);
					costingtool.setBilltypeclass(billtypeclass);
					beforeCheck(costingtool, tradedatatool, checkclasstool);
				...
				if (needuncheckdataMap != null) {
					List<PendingBillVO> billtypeclass = needuncheckdataMap
							.get(key);
					costingtool.setBilltypeclass(billtypeclass);
					beforeCheck(costingtool, tradedatatool, checkclasstool);
				...
				if (pk_glorgbook != null) {
					conditions.append(" and pk_glorgbook='")
							.append(pk_glorgbook).append("' ");
				}
				if (pk_group != null) {
					conditions.append(" and pk_group='").append(pk_group)
							.append("' ");
				}
				if (pk_org != null) {
					conditions.append(" and pk_org='").append(pk_org)
							.append("' ");
				}
				conditions.append(" and pk_billtypegroup='")
						.append(pk_billtypegroup).append("' ");
				conditions
						.append(" and trade_date>='")
						.append(checkParaVO.getTrade_date().toLocalString()
								+ " 00:00:00").append("' ");
				String sqlwhere = conditions.toString();
				// 删除库余额表
				PrivateMethod.getInstance().getBaseDAO()
						.deleteByClause(StockBalanceVO.class, sqlwhere);
				// 删除资金日余额表
				PrivateMethod.getInstance().getBaseDAO()
						.deleteByClause(FundBalanceVO.class, sqlwhere);
				// 删除券源划转余额表
				PrivateMethod.getInstance().getBaseDAO()
						.deleteByClause(TransferBalanceVO.class, sqlwhere);
				// 删除筹资管理库存表
				PrivateMethod.getInstance().getBaseDAO()
						.deleteByClause(InventoryInfoVO.class, sqlwhere);
			...
			NCLocator
					.getInstance()
					.lookup(IBillCheckByGroup.class)
					.updateMiddleAndApporveUI_RequiresNew(needuncheckdataMap,
							pk_billtypegroup, SystemConst.statecode[1], msg);
		}
	}

	public void checkOneGroup_RequiresNew(BilltypeGroupVO vo, Set<UFDate> set,
			Map<String, List<PendingBillVO>> dataMap, CheckParaVO checkParaVO,
			TradeDataTool tradedatatool, CheckClassTool checkclasstool,
			List<CostPlanVO> clist, boolean isFirstPrice, boolean iscalfund)
			throws BusinessException {
		...
		try {
			for (UFDate date : set) {
				for (int j = 0; j < clist.size(); j++) {
					...
					costingtool.setCostParaVO(costParaVO);
					costingtool.getBalancetool().setScostcalcachemanager(
							scostcalcachemanager);// 设置库存缓存
					costingtool.setBilltypeclass(billtypeclass);
					costingtool.setCurrbilltypegroupvo(vo);// 当前业务组
					...
					utis.zqjdautoAudit(vo, costingtool, tradedatatool,
							checkclasstool);
					/********** 在此添加生成债券交易记录后续单据 end ***/
					beforeCheck(costingtool, tradedatatool, checkclasstool);
					runCheckClass(costingtool, tradedatatool, checkclasstool);
					// 审核时自动生成归还单 lqm
					utis.autoreturn(vo, dataMap, costingtool, tradedatatool,
							checkclasstool);
					/**
			...
			NCLocator
					...
					.lookup(IBillCheckByGroup.class)
					.updateMiddleAndApporveUI_RequiresNew(dataMap,
							pk_billtypegroup, SystemConst.statecode[3], msg);
	 ...
	 * @throws Exception
	 */
	private void checkTally(String pk_billtypegroup, UFDate date)
			throws Exception {
	 ...
	 * @throws Exception
	 */
	private void beforeCheck(CostingTool costingtool,
			TradeDataTool tradedatatool, CheckClassTool checkclasstool)
			throws Exception {
		// 自动还本付息
				...
				for (Iterator ite = keyset.iterator(); ite.hasNext();) {
					...
					case 0:// 审核
						...
						conditionbuffer.append(
								trade_date.getDateAfter(1).toLocalString()
										+ " 00:00:00").append("'");
						UFDate lastapprovedate = costingtool
					...
					case 1:// 记账
						...
						conditionbuffer.append(
								trade_date.getDateAfter(1).toLocalString()
										+ " 00:00:00").append("'");
						UFDate lasttallydate = costingtool
								.getCurrbilltypegroupvo().getLasttallydate();
						if (lasttallydate != null) {
							conditionbuffer.append(" and trade_date>='");
							conditionbuffer.append(
									costingtool.getCurrbilltypegroupvo()
											.getLasttallydate().getDateAfter(1)
											.toLocalString()
											+ " 00:00:00").append("'");
						}
						break;
					case 2:// 弃审
						...
						conditionbuffer.append(" and trade_date>='");
						conditionbuffer.append(
								trade_date.toLocalString() + " 00:00:00")
								.append("'");
						break;
					default:
						break;
					...
					if (aggs != null) {
						for (int j = 0; j < aggs.length; j++) {
							...
							// 每条单据的交易日期
							UFDate history_trade_date = (UFDate) parent
									.getAttributeValue("trade_date");
							tradedatatool.addData(costingtool
									.getCurrbilltypegroupvo()
									.getPk_billtypegroup()
									+ history_trade_date.toLocalString()
									+ billtype, ibill);
	 ...
	 * @throws Exception
	 */
	private void runTallyClass(CostingTool costingtool,
			TradeDataTool tradedatatool, CheckClassTool checkclasstool)
			throws Exception {
		List billtypeclass = costingtool.getBilltypeclass();
		Map<String, IBillCheckPlugin> checkclassmap = checkclasstool
		...
		if (billtypeclass != null) {
			for (int i = 0; i < billtypeclass.size(); i++) {
				...
				IBillCheckPlugin ibillcheck = checkclassmap.get(pbvo
						.getPk_billtype() + "##" + pbvo.getCheckclass());
				try {
					ibillcheck.tallyBills(costingtool, tradedatatool);
				} catch (Exception e) {
					throw new Exception("记账日期：" + costingtool.getCurrdate()
							+ "插件类内部报错:" + ibillcheck.toString() + "错误日志："
							+ e.getMessage());
				}
			}
		}
	...
	private void updateTradeDatas(CostingTool costingtool,
			TradeDataTool tradedatatool, CheckClassTool checkclasstool)
			throws BusinessException {
		List billtypeclass = costingtool.getBilltypeclass();
		Map<String, IBillCheckPlugin> checkclassmap = checkclasstool
	...
	private void initCost(CostingTool costingtool) throws Exception {
		 ...
		 * 初始化库存缓存
		 */
		costingtool.getBalancetool().getScostcalcachemanager()
		...
		if (billtypeclass != null) {
			for (int i = 0; i < billtypeclass.size(); i++) {
				...
				costingtool.setPk_stocksort(new String[] {
						pbvo.getPk_stocksort1(), pbvo.getPk_stocksort2() });
				...
				try {
					...
					/** 更新错误提示：这里将交易类型更改的字面，方便查找错误 JINGQT 2017年5月18日 ADD START */
					String name = PfDataCache.getBillTypeNameByCode(pbvo
							.getPk_billtype());
					if (null == name) {
						name = pbvo.getPk_billtype();
					}
					/** 更新错误提示：这里将交易类型更改的字面，方便查找错误 JINGQT 2017年5月18日 ADD END */
					throw new Exception("审核日期：" + costingtool.getCurrdate()
							+ "插件类内部报错:" + name + " 错误日志：" + e.getMessage());
				}
			}
		}
	}
	 ...
	 * @throws Exception
	 */
	private void afterCheck(CostingTool costingtool,
			TradeDataTool tradedatatool, CheckClassTool checkclasstool)
			throws Exception {
	 ...
	 * @throws Exception
	 */
	private void runUncheckClass(CostingTool costingtool,
			TradeDataTool tradedatatool, CheckClassTool checkclasstool)
			throws Exception {
		List billtypeclass = costingtool.getBilltypeclass();
		Map<String, IBillCheckPlugin> checkclassmap = checkclasstool
		...
		if (billtypeclass != null) {
			for (int i = billtypeclass.size() - 1; i >= 0; i--) {
				...
				try {
					...
					throw new Exception("弃审日期：" + costingtool.getCurrdate()
							+ "插件类内部报错:" + ibillcheck.toString() + "错误日志："
							+ e.getMessage());
				}
	 ...
	 */
	public void updateMiddleAndApporveUI_RequiresNew(
			Map<String, List<PendingBillVO>> dataMap, String pk_billtypegroup,
			int state, String returnmsg) throws BusinessException {
		try {
			/**
			 * 1.更新中间表数据 记账成功更新中间表
			 */
			if (returnmsg == null)
				PrivateMethod.getInstance().updateMiddle(dataMap,
						pk_billtypegroup, state);
			// /**
	...
	@Override
	public void updateApporveUI_RequiresNew(BilltypeGroupVO vo,
			CheckParaVO checkParaVO, String returnmsg, Integer checkflag)
			throws BusinessException {
		try {
			/**
			 * 2: ApproveVO 插入一行最新的 更新以前的为非最新
	...
	@Override
	public void Before_RequiresNew(BilltypeGroupVO vo, CheckParaVO checkParaVO,
		...
		// 自动生成归还单
		// boolean enable =
		// InitGroupQuery.isEnabled(checkParaVO.getPk_group(),"HV17");
		// //债券借贷:防止模块未启用，反射报错
		/** 模块号更新产生的变更。 2017年7月28日 */
		// boolean enable = InitGroupQuery.isEnabled(checkParaVO.getPk_group(),
		// "HVC0"); // 债券借贷:防止模块未启用，反射报错
		// if (enable) {
		...
		for (int j = 0; j < plugins.size(); j++) {
			...
			try {
				...
				ispbusiness = (ISpecialBusiness) NewObjectService.newInstance(
						temp[2], sbimpl);
			} catch (Exception e) {
			...
			if (null != vos && vos.length > 0) {
				Map<String, IBill[]> returnmap = ispbusiness.insert(vo,
						checkParaVO, vos);
			}
			// if (returnmap != null) {
