...
public class BillCheckByGroupImpl implements IBillCheckByGroup {
    ...
    public void tallyOneGroup_RequiresNew(BilltypeGroupVO vo, Set<UFDate> set, Map<String, List<PendingBillVO>> dataMap,
            CheckParaVO checkParaVO, TradeDataTool tradedatatool, CheckClassTool checkclasstool) throws BusinessException {
    ...
    public void uncheckOneGroup_RequiresNew(BilltypeGroupVO vo, Set<UFDate> set, Map<String, List<PendingBillVO>> dataMap,
            Map<String, List<PendingBillVO>> needuncheckdataMap, CheckParaVO checkParaVO, TradeDataTool tradedatatool,
        ...
        try {
            for (UFDate date : set) {
                ...
                checkTally(pk_billtypegroup, date);
                if (dataMap != null && vo.getLasttallydate() != null
                        && date.toLocalString().compareTo(vo.getLasttallydate().toLocalString()) <= 0) {
                    // 走取消记账逻辑
                    List<PendingBillVO> billtypeclass = dataMap.get(key);
                    costingtool.setBilltypeclass(billtypeclass);
                    beforeCheck(costingtool, tradedatatool, checkclasstool);
                ...
                conditions.append(" and trade_date>='").append(checkParaVO.getTrade_date().toLocalString() + " 00:00:00")
                        .append("' ");
    ...
    public void checkOneGroup_RequiresNew(BilltypeGroupVO vo, Set<UFDate> set, Map<String, List<PendingBillVO>> dataMap,
            CheckParaVO checkParaVO, TradeDataTool tradedatatool, CheckClassTool checkclasstool, List<CostPlanVO> clist,
            boolean isFirstPrice, boolean iscalfund) throws BusinessException {
        ...
        try {
            for (UFDate date : set) {
                for (int j = 0; j < clist.size(); j++) {
                    ...
                    /**
                     * YangJie 2014-04-08 库存缓存初始化策略 1.当成本计算方案内循环需要全面初始化 2.当成本计算方案外循环需要片面初始化：即根据当前时间回溯前几日缓存来使用
                     */
     ...
     * @throws Exception
     */
    private void beforeCheck(CostingTool costingtool, TradeDataTool tradedatatool, CheckClassTool checkclasstool)
            throws Exception {
        // 自动还本付息
     ...
     * @throws Exception
     */
    private void initTradeData(CostingTool costingtool, TradeDataTool tradedatatool, CheckClassTool checkclasstool)
            throws Exception {
        /**
                ...
                for (Iterator ite = keyset.iterator(); ite.hasNext();) {
                    ...
                    case 0:// 审核
                        ...
                        if (lastapprovedate != null) {
                            ...
                            conditionbuffer.append(
                                    costingtool.getCurrbilltypegroupvo().getLastapprovedate().getDateAfter(1).toLocalString()
                                            + " 00:00:00").append("'");
                        }
                        break;
                    case 1:// 记账
                        ...
                        if (lasttallydate != null) {
                            conditionbuffer.append(" and trade_date>='");
                            conditionbuffer.append(
                                    costingtool.getCurrbilltypegroupvo().getLasttallydate().getDateAfter(1).toLocalString()
                                            + " 00:00:00").append("'");
                        }
                        break;
                    case 2:// 弃审
                    ...
                    if (aggs != null) {
                        for (int j = 0; j < aggs.length; j++) {
                            ...
                            tradedatatool.addData(
                                    costingtool.getCurrbilltypegroupvo().getPk_billtypegroup()
                                            + history_trade_date.toLocalString() + billtype, ibill);
     ...
     * @throws Exception
     */
    private void runTallyClass(CostingTool costingtool, TradeDataTool tradedatatool, CheckClassTool checkclasstool)
            throws Exception {
        List billtypeclass = costingtool.getBilltypeclass();
        Map<String, IBillCheckPlugin> checkclassmap = checkclasstool.getCheckClassMap();
        if (billtypeclass != null) {
            for (int i = 0; i < billtypeclass.size(); i++) {
                ...
                try {
                    ...
                    throw new Exception("记账日期：" + costingtool.getCurrdate() + "插件类内部报错:" + ibillcheck.toString() + "错误日志："
                            + e.getMessage());
                }
            }
        }
    ...
    private void updateTradeDatas(CostingTool costingtool, TradeDataTool tradedatatool, CheckClassTool checkclasstool)
    ...
    private void initCost(CostingTool costingtool) throws Exception {
         ...
         * 初始化筹资管理缓存
         */
        costingtool.getBalancetool().getScostcalcachemanager().getCacheByTypeCode(SystemConst.InventoryInfo, costingtool);
     ...
     * @throws Exception
     */
    private void runCheckClass(CostingTool costingtool, TradeDataTool tradedatatool, CheckClassTool checkclasstool)
            throws Exception {
        List billtypeclass = costingtool.getBilltypeclass();
        Map<String, IBillCheckPlugin> checkclassmap = checkclasstool.getCheckClassMap();
        if (billtypeclass != null) {
            for (int i = 0; i < billtypeclass.size(); i++) {
                ...
                try {
                    ...
                    String name = PfDataCache.getBillTypeNameByCode(pbvo.getPk_billtype());
                    if (null == name) {
                        name = pbvo.getPk_billtype();
                    }
                    /** 更新错误提示：这里将交易类型更改的字面，方便查找错误 JINGQT 2017年5月18日 ADD END */
                    throw new Exception("审核日期：" + costingtool.getCurrdate() + "插件类内部报错:" + name + " 错误日志：" + e.getMessage());
                }
            }
        }
    }
     ...
     * @throws Exception
     */
    private void runUncheckClass(CostingTool costingtool, TradeDataTool tradedatatool, CheckClassTool checkclasstool)
            throws Exception {
        List billtypeclass = costingtool.getBilltypeclass();
        Map<String, IBillCheckPlugin> checkclassmap = checkclasstool.getCheckClassMap();
        if (billtypeclass != null) {
            for (int i = billtypeclass.size() - 1; i >= 0; i--) {
                ...
                try {
                    ...
                    throw new Exception("弃审日期：" + costingtool.getCurrdate() + "插件类内部报错:" + ibillcheck.toString() + "错误日志："
                            + e.getMessage());
                }
     ...
     */
    public void updateMiddleAndApporveUI_RequiresNew(Map<String, List<PendingBillVO>> dataMap, String pk_billtypegroup,
...
//        boolean enable = InitGroupQuery.isEnabled(checkParaVO.getPk_group(), "HVC0"); // 债券借贷:防止模块未启用，反射报错
//        if (enable) {
