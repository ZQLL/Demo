...
public class CalcPluginMaintainImpl  implements ICalcPluginMaintain {
	...
	public void buyPlugindeal(SuperVO tradevo1) throws BusinessException {
		...
		if(falg){
			...
			tradevo.setInterestadjust(SafeCompute.sub(lxtz, tradevo.getTaxexpense()));
			//回写实际利率单据信息
	...
	//取消买入插件处理
	@Override
	public void unBuyPlugindeal(SuperVO tradevo1,String pk_group,String pk_org,String tradedate)
	...
	//卖出插件处理
	@Override
	public void sellPlugindeal(StockBalanceVO stockbalancevo,SuperVO tradevo1)throws BusinessException {
		//实际利率处理单据
	...
	//转托管
	@Override
	public void ztgPlugindeal(StockBalanceVO stockbalancevo,SuperVO tradevo1) throws BusinessException {
		//实际利率处理单据
		...
		if(falg1){
			...
			//
			ActualRateBillInfo billvo1 = real1.bulidTransfromInfo(tradevo,ActualrateConst.TOUT);
			ActualRateBillInfo billvo2 = real1.bulidTransfromInfo(tradevo,ActualrateConst.TIN);
			ActualRateBillInfo[] vos = new ActualRateBillInfo[]{billvo1,billvo2};
			real1.save(vos);
		}
	}
	//取消转托管
	...
	public void unAuditcheck(String pk_group,String pk_org,UFLiteralDate date) throws BusinessException {
		...
		if(!new UFLiteralDate(calcdate).before(date)){
			throw new BusinessException("当前组织启用实际利率，实际利率计算日期为: ["+calcdate+"]，请先取消实际利率计算！");
		}
	}
	// 公允摊余成本
	@Override
	public void setFairvalueByRate(UFLiteralDate tradedate,StockBalanceVO sbvo,
	...
	public void fairCheck(String pk_group,String pk_org, String distill_date) throws BusinessException {
		...
		String value = info.queryParavalue(pk_orgbook, ActualrateConst.PARAM_REAL);
		if("Y".equals(value)){
			FairValueRate rate = new FairValueRate();
