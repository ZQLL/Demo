...
//		distilvo.setModifier(modifier);
	 ...
	 * 构造应收利息计提单据---转出、买入、卖出
	 */
	public InterestdistillVO buildInterestdistill(ICostingTool costingtool,ITrade_Data vs) 
	...
	public UFDouble getLxRecord(ITrade_Data vs){
		...
		if(CostConstant.BUY_TRANSTYPE.equals(vs.getTranstypecode())||"HV3A-0xx-22".equals(vs.getTranstypecode())){//买入
			z = vs.getAccrual_sum();
		}else{
			z = vs.getInterest();
		}
	...
	public int getGenstate2(ITrade_Data vs){
		int index = -1;
		if(CostConstant.BUY_TRANSTYPE.equals(vs.getTranstypecode())||"HV3A-0xx-21".equals(vs.getTranstypecode())){//买入
			index = CostConstant.INTEREST_GENSTATE.ZHUANRU.getIndex();
		}else{
			index = CostConstant.INTEREST_GENSTATE.ZHUANCHU.getIndex();
		}
