...
public class AutoPay {
    ...
    public void autoPay(CostingTool costingtool, TradeDataTool tradedatatool) throws BusinessException {
    ...
    public void autoAudit(CostingTool costingtool, TradeDataTool tradedatatool) throws BusinessException {
        ...
        boolean falg = getQueryInfo().getBooleanFromInitcode(pk_glorgbook, CostConstant.PARAM_HBFX);
        if (!falg)
            return;
        Map<String, List<SuperVO>> map = calcAutoPayInfo(costingtool, costplanvo, pk_group, pk_org, trade_date);
        if (map != null) {
    ...
    public void addCache(CostingTool costingtool, TradeDataTool tradedatatool, String[] pks1, String[] pks2)
            throws BusinessException {
        try {
            if (tradedatatool.isIsinit()) {
                Class<IBill> cs = (Class<IBill>) Class.forName("nc.vo.fba_sim.simtrade.stocktrade.AggStocktradeVO");
                // 查询最新数据1
    ...
    public Map<String, List<SuperVO>> calcAutoPayInfo(CostingTool costingtool, CostPlanVO costplanvo, String pk_group,
            String pk_org, UFDate trade_date) throws BusinessException {
        List<StockBalanceVO> list = getQueryInfo().queryLastStock(costplanvo, pk_group, pk_org, trade_date);
        ...
        for (StockBalanceVO v : list) {
            ...
            this.buildEarlyRepayment(costingtool, pk_group, pk_org, v, trade_date, map);
        }
        return map;
    }
     ...
     * 查询当前债券是否还本到期
     */
    public void buildHbFxBill(String pk_group, String pk_org, StockBalanceVO stockvo, UFDate trade_date,
            Map<String, List<SuperVO>> map) throws BusinessException {
        AggInterest agginfo = getQueryInfo().queryInterestsetInfo(stockvo.getPk_securities());
    ...
    private void buildEarlyRepayment(CostingTool costingtool, String pk_group, String pk_org, StockBalanceVO stockvo,
            UFDate trade_date, Map<String, List<SuperVO>> map) throws BusinessException {
        ...
        /** =============qs 从现场代码迁过来的：加了一个参数earlyPercent2。 20170518 add start============== */
        SuperVO bill = this.getEarlyBill(costingtool, pk_group, pk_org, stockvo, trade_date, earlyPercent, earlyPercent2);
        /** =============qs 从现场代码迁过来的：加了一个参数earlyPercent2。 20170518 add end============== */
        lst.add(bill);
        if (map.containsKey(this.early)) {
            map.get(this.early).addAll(lst);
        } else {
     ...
     * 生成提前还本单
     * 
     * @param costingtool 审核工具类
     * @param pk_group 组织
     * @param pk_org 集团
     * @param stockvo 库存
     * @param trade_date 还本日期
     * @param earlyPercent 提前还本比例
     * @return 审核的单据（此单据是用EARLY开头的）
     * @throws BusinessException
     * @author jingqt
     * @date 2015-10-30 上午11:05:47
     */
    private SuperVO getEarlyBill(CostingTool costingtool, String pk_group, String pk_org, StockBalanceVO stockvo,
            UFDate trade_date, UFDouble earlyPercent, UFDouble earlyPercent2) throws BusinessException {
        ...
        vo.setAttributeValue("billno", this.getBillNO(pk_group, pk_org, vo, this.earlyPrefix));
        ...
        BilltypeVO billtypeVO = PfDataCache.getBillTypeInfo(CostConstant.SELL_TRANSTYPE);
        ...
        SecuritiesVO secvo = (SecuritiesVO) new BaseDAO().retrieveByPK(SecuritiesVO.class, stockvo.getPk_securities());
        ...
        ForwardVO fvo = fvf.forwardVOinfo(costingtool, stockvo, (ITrade_Data) vo);
        // 结转的值
        ...
        vo.setAttributeValue("fairvaluedate", AppContext.getInstance().getServerTime());
        // 保存公允价值---
        ...
        UFDouble fact_sum = this.calcTi(stockvo.getStocks_num().multiply(new UFDouble(100.0D)), earlyPercent);
        ...
        try {
            ...
            if (this.isAsset(assetsprop)) {
                // COMMENT 实际利率
                String result = SysInitCache.getInstance().getSysInitValue(stockvo.getPk_glorgbook(),
    ...
    private boolean isAsset(String pk_assetsprop) {
        ...
        if (pk_assetsprop.equals(SystemConst.PK_ASSETSPROP_0001) || pk_assetsprop.equals(SystemConst.PK_ASSETSPROP_0002)
    ...
    private UFDouble calcTi(UFDouble target, UFDouble earlyPercent) {
        return SafeCompute.div(SafeCompute.multiply(target, earlyPercent), new UFDouble(100)).setScale(2, UFDouble.ROUND_UP);
    }
    ...
    private UFDouble isEarlyRepayment(Rateperiod[] ratevos, UFDate trade_date) {
        ...
        for (Rateperiod rateperiod : ratevos) {
            ...
            // 如果日期合适，并且提前还本比例有值
            if (trade_date.isSameDate(end) && pay != null && !pay.equals(UFDouble.ZERO_DBL)) {
                falg = pay;
                break;
            }
        }
    ...
    private UFDouble isEarlyRepayment2(Rateperiod[] ratevos, UFDate trade_date) {
        ...
        for (Rateperiod rateperiod : ratevos) {
            ...
            if (end.compareTo(trade_date) < 0) {
                sum_before = sum_before.add(rateperiod.getPaypercent() == null ? UFDouble.ZERO_DBL : rateperiod.getPaypercent());
            }
            // 如果日期合适，并且提前还本比例有值
            if (trade_date.isSameDate(end) && pay != null && !pay.equals(UFDouble.ZERO_DBL)) {
                falg = pay;
    ...
    public String getBillNO(String pk_group, String pk_org, SuperVO fv, String suffix) throws BusinessException {
        ...
        try {
            ...
            vbillcode = ser.getBillCode_RequiresNew(CostConstant.FXHB_BILLTYPE, pk_group, pk_org, fv);
     ...
     * 
     * @param stockvo 库存信息
     * @param agginfo 利率设置信息
     * @param trade_date 交易日期
     * @author 未知
     * @throws BusinessException
     * @update jingqt <i>2016年4月22日14:50:52 </i>
     */
    public UFDouble calcLX(StockBalanceVO stockvo, AggInterest agginfo, UFDate trade_date) throws BusinessException {
        ...
        if (CostConstant.PAYONCEATEND.equals(headvo.getPaytype())) {// 到期一次还本付息----自定义
            for (Rateperiod vo : ratevos) {
                UFDouble lx1 = SafeCompute.multiply(stockvo.getStocks_num(), vo.getYear_rate());
                /** 2016年5月6日 JINGQT 取实际的年天数。如果跨年，则取最大的年天数 ADD START */
                // UFDouble days = this.getRealDays(headvo, vo.getStart_day(), vo.getEnd_day());
                ...
                // // 实际计息天数
                // UFDouble realDays = this.getRealDaysByVO(vo.getStart_day(), vo.getEnd_day());
                // // 年计息天数
                // UFDouble allDays = this.getYearDays(headvo, vo.getStart_day(), vo.getEnd_day());
                // lx = SafeCompute.multiply(lx1, SafeCompute.div(realDays, allDays));
                // } else {
                // 实际计息天数
                UFDouble realDays = this.getRealDaysByVO(vo.getStart_day(), vo.getEnd_day());
                // 年计息天数
                UFDouble days = this.getYearDays(headvo, vo.getStart_day(), vo.getEnd_day());
            ...
            lx = calc.calcInterestTradedate(stockvo.getStocks_num(), stockvo.getPk_securities(), trade_date,
     ...
     */
    private UFDouble getYearDays(Interest headvo, UFDate start_date, UFDate end_date) {
        ...
        if ("02".equals(paytype)) {// 到期一次还本付息
            for (int k = start_date.getYear(); k < end_date.getYear(); k++) {
                ...
                && start_date.compareTo(new UFDate(start_date.getYear() + "-02-28")) > 0) {
            days_sum = 366;
     ...
     */
    // private UFDouble getRealDays(Interest headvo, UFDate startDay, UFDate endDay) {
     ...
     * 生成付息单
     */
    public SuperVO genFxBill(String pk_group, String pk_org, StockBalanceVO stockvo, UFDate trade_date, UFDouble lx)
            throws BusinessException {
        SuperVO vo = constructVO();
        vo.setAttributeValue("bargain_num", stockvo.getStocks_num());
        vo.setAttributeValue("bargain_sum", lx);
        vo.setAttributeValue("billno", getBillNO(pk_group, pk_org, vo, this.autoSuffix));
        vo.setAttributeValue("creationtime", AppContext.getInstance().getServerTime());
        vo.setAttributeValue("creator", AppContext.getInstance().getPkUser());
        vo.setAttributeValue("billmaker", AppContext.getInstance().getPkUser());
        ...
        BilltypeVO billtypeVO = PfDataCache.getBillTypeInfo(CostConstant.FX_TRANSTYPE);
        vo.setAttributeValue("pk_transtype", billtypeVO.getPk_billtypeid());
     ...
     * 生成还本单
     */
    public SuperVO genHbFxBill(String pk_group, String pk_org, StockBalanceVO stockvo, UFDate trade_date)
            throws BusinessException {
        /** qs 从现场代码迁过来的：对于付息方式是【贴现】的做了特殊处 20170518 add start */
        /**
         * add by lihbj 2017-03-07 start 若为贴现 则bargain_sum（成交金额）和fact_sum（实际收付） = 数量 乘以折扣率
         */
        // 查询当前债券对应的利率设置
        AggInterest agginfo = getQueryInfo().queryInterestsetInfo(stockvo.getPk_securities());
        ...
        // 折扣率
        if (intervo.getDiscountrate() != null && intervo.getPaytype().equals("03")) {
            mz = SafeCompute.multiply(stockvo.getStocks_num(), new UFDouble(intervo.getDiscountrate()));
        } else
            mz = SafeCompute.multiply(stockvo.getStocks_num(), new UFDouble(100));
        /** add by lihbj 2017-03-07 end */
        /** qs 从现场代码迁过来的：对于付息方式是【贴现】的做了特殊处 20170518 add end */
        SuperVO vo = constructVO();
        // UFDouble mz = SafeCompute.multiply(stockvo.getStocks_num(), new UFDouble(100));
