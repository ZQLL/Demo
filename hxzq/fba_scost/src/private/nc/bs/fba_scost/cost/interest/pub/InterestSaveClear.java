...
/**
 * 应收利息买入、转入、卖出结转、转出、付息存应收利息计提表 计当期发生额 清空已计当期发生额
 */
public class InterestSaveClear {
	...
	public void saveInterestDistill(ICostingTool costingtool, ITrade_Data vo)
		...
		InterestdistillVO distillvo = info
				.buildInterestdistill(costingtool, vo);
		if (distillvo != null && distillvo.getInterestdistill() != null
				&& distillvo.getInterestdistill().doubleValue() != 0) {
			if (vo.getGenway() == CostConstant.FORWARD_GENSTATE.ZHUANCHU
					.getIndex()) {
				list.add(distillvo);
			} else if (vo.getGenway() == CostConstant.FORWARD_GENSTATE.ZHUANRU
					.getIndex()) {
				// 无
			} else if (vo.getGenway() == CostConstant.FORWARD_GENSTATE.ZHUANCHUANDRU
					.getIndex()) {
				InterestdistillVO invo = info.buildInterestdistillVO_in(
						costingtool, vo);
				list.add(invo);
				list.add(distillvo);
			}
	...
	public void clearDistillInterest(ICostingTool costingtool)
		...
		String pk_org = costingtool.getCostParaVO().getCheckParavo()
				.getPk_org();
		String pk_glbook = costingtool.getCostParaVO().getCheckParavo()
				.getPk_glorgbook();
		UFDate trade_date = costingtool.getCostParaVO().getCheckParavo()
				.getTrade_date();
