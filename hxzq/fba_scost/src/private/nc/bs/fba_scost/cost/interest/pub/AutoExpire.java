...
public class AutoExpire {
	 ...
	 */
	public void autoExpire(CostingTool costingtool, TradeDataTool tradedatatool)
		...
		// 是否自动到期参数
		boolean flag = getQueryInfo().getBooleanFromInitcode(pk_glorgbook,
				CostConstant.PARAM_ISAUTOEXPIRE);
		if (!flag) {
			return;
		}
		Map<String, List<SuperVO>> map = calcAutoExpireInfo(costingtool,
		...
		if (map != null && map.size() > 0) {
			...
			addExpireCache(costingtool, tradedatatool, pks1, pks2, pks3, pks4,
					pks5);
		}
	}
	 ...
	 * 加入缓存
	 */
	private void addExpireCache(CostingTool costingtool,
			TradeDataTool tradedatatool, String[] pks1, String[] pks2,
			String[] pks3, String[] pks4, String[] pks5)
			throws BusinessException {
		try {
			if (tradedatatool.isIsinit()) {
				// 查询最新数据1
				if (pks1 != null && pks1.length > 0) {
					this.addExpireData(costingtool, tradedatatool,
							CostConstant.AGGBONDSPAYABLEVONAME, pks1);
				}
				if (pks2 != null && pks2.length > 0) {
					this.addExpireData(costingtool, tradedatatool,
							CostConstant.AGGGOFINANCINGVONAME, pks2);
				}
				if (pks3 != null && pks3.length > 0) {
					this.addExpireData(costingtool, tradedatatool,
							CostConstant.AGGINTERBANKLENDINGVONAME, pks3);
				}
				if (pks4 != null && pks4.length > 0) {
					this.addExpireData(costingtool, tradedatatool,
							CostConstant.AGGPROFITTRANSFERREPOVONAME, pks4);
				}
				if (pks5 != null && pks5.length > 0) {
					this.addExpireData(costingtool, tradedatatool,
							CostConstant.AGGSHORTFUNDVONAME, pks5);
				}
			}
		} catch (Exception e) {
	...
	private void addExpireData(CostingTool costingtool,
			TradeDataTool tradedatatool, String aggName, String[] pks)
			throws BusinessException {
		Class<IBill> cs;
		try {
		...
		for (int j = 0; j < ibills.length; j++) {
			...
			String key = costingtool.getCurrbilltypegroupvo()
					...
					+ ((UFDate) (ibill.getParent()
							.getAttributeValue("trade_date"))).toLocalString()
	 ...
	 */
	private String[] insertVO(String expire, Map<String, List<SuperVO>> map)
			throws DAOException {
	 ...
	 */
	private Map<String, List<SuperVO>> calcAutoExpireInfo(
			CostingTool costingtool, CostPlanVO costplanvo, String pk_group,
			String pk_org, UFDate trade_date) throws BusinessException {
		// 查询单据类型bd_billtype
		String sql = "isnull(dr,0)=0 and pk_billtypecode like 'HV8%' ";
		List<BilltypeVO> billTypeList = (List<BilltypeVO>) dao
				.retrieveByClause(BilltypeVO.class, sql);
		for (BilltypeVO vo : billTypeList) {
			// key-单据类型编码 vlue-单据类型编码对应的pk
			billTypeMap.put(vo.getPk_billtypecode(), vo.getPk_billtypeid());
		}
		List<InventoryInfoVO> list = this.getQueryInfo().queryLastInventory(
				costplanvo, pk_group, pk_org, trade_date);
	 ...
	 */
	private boolean checkWhetherAutoCreateBill(InventoryInfoVO checkVO)
			throws DAOException {
		String[] tables = { "FUND_BONDSPAYABLE", "FUND_GOFINANCING",
				"FUND_INTERBANKLENDING", "FUND_PROFITTRANSFERREPO",
				"FUND_SHORTFUND" };
		...
		boolean bln = (Boolean) this.dao.executeQuery(sql,
	 ...
	 * 查询当前筹资是否到期
	 */
	private void buildExpireBill(String pk_group, String pk_org,
			InventoryInfoVO stockvo, UFDate trade_date,
	 ...
	 */
	private void updateMapInfo(Map<String, List<SuperVO>> map, String key,
			SuperVO vo) {
	 ...
	 */
	private SuperVO getExpireBill(String pk_group, String pk_org,
			InventoryInfoVO stockvo, UFDate trade_date)
			throws BusinessException {
		// 交易类型编码
		...
		// 查询该合同号的原始单据
		String whereSql = "isnull(dr,0)=0 and contractno = " + "'"
				+ stockvo.getContractno() + "'";
		...
		if (trans_type == SystemConst.BONDSPAYABLE_01) {
			...
			vo.setAttributeValue("pk_transtype",
					billTypeMap.get(SystemConst.BONDSPAYABLE_02));
			return vo;
		} else if (trans_type == SystemConst.GOFINANCING_01) {
			vo.setAttributeValue("transtypecode", SystemConst.GOFINANCING_02);
			vo.setAttributeValue("pk_transtype",
					billTypeMap.get(SystemConst.GOFINANCING_02));
			return vo;
		} else if (trans_type == SystemConst.INTERBANKLENDING_01) {
			vo.setAttributeValue("transtypecode",
					SystemConst.INTERBANKLENDING_02);
			vo.setAttributeValue("pk_transtype",
					billTypeMap.get(SystemConst.INTERBANKLENDING_02));
			return vo;
		} else if (trans_type == SystemConst.PROFITTRANSFERREPO_01) {
			vo.setAttributeValue("transtypecode",
					SystemConst.PROFITTRANSFERREPO_02);
			vo.setAttributeValue("pk_transtype",
					billTypeMap.get(SystemConst.PROFITTRANSFERREPO_02));
			return vo;
		} else if (trans_type == SystemConst.SHORTFUND_01) {
			vo.setAttributeValue("transtypecode", SystemConst.SHORTFUND_02);
			vo.setAttributeValue("pk_transtype",
					billTypeMap.get(SystemConst.SHORTFUND_02));
			return vo;
		} else
			return vo;
	}
