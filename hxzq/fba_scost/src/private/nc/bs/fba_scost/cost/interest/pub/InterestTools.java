...
public class InterestTools {
	 ...
	 */
	public static Rateperiod getContainsDate(Rateperiod[] dates,
			UFDate trade_date) {
		...
		for (int i = 0; i < dates.length; i++) {
			...
			if ((trade_date.isSameDate(start) || trade_date.after(start
					.asBegin()))
					&& (trade_date.before(end.asEnd()) || trade_date
							.isSameDate(end))) {
				vo = dates[i];
				break;
			}
	 ...
	 */
	public static UFDouble getCostSurplus(Rateperiod[] dates,
			Rateperiod targetVO) {
		...
		for (Rateperiod tempVO : dates) {
			...
			if (tempVO.getStart_day().before(targetVO.getStart_day())) {
				result = PubMethod.getInstance().add(tempVO.getPaypercent(),
						result);
			}
		}
		// 如果没有还过本，则返回1。
		...
		// 这个计算百分比的值。
		return UFDouble.ONE_DBL.sub(PubMethod.getInstance().div(result,
				new UFDouble(100.0d)));
	}
	...
	/**
	 * 计算每期利息<br>
	 * <b>2016年4月25日 添加----计算每期利息前，应该将其之前已经还本的部分除去。</b> 影响范围：
	 * {@link CalcBuySellInterest}中计算利息的方式。
	 * 
	 * @see CalcBuySellInterest
	 * @update JINGQT
	 * @date 2016年4月25日 09:32:38
	 */
	public static UFDouble calcLxByTerm(UFDouble bargain_num, Interest headvo,
			Rateperiod[] ratevos, Rateperiod vo) {
		if (headvo == null || ratevos == null || ratevos.length == 0
				|| vo == null)
			return new UFDouble(0);
		UFDouble lx = null;
		if (CostConstant.PAYONCEATEND.equals(headvo.getPaytype())) {// 自定义--按照程序处理，这里应该是
																	// 到期一次还本付息
			for (Rateperiod vo1 : ratevos) {
				UFDouble lx1 = SafeCompute.multiply(bargain_num,
						vo1.getYear_rate());
				UFDouble days = getRealDays(headvo, vo.getStart_day(),
						vo.getEnd_day());
			...
			lx = SafeCompute.multiply(bargain_num, vo.getYear_rate()).multiply(
					hasSurplusPercent);
	 ...
	 */
	protected static UFDouble getRealDays(Interest headvo, UFDate startDay,
			UFDate endDay) {
		...
		if (CostConstant.PAYONCEATEND.equals(headvo.getPaytype())) {// 自定义--按照程序处理，这里应该是
			...
			if (startDay.getYear() == endDay.getYear()) {// 不跨年
				...
				if (UFDate.isLeapYear(startDay.getYear())
						&& startDay.getMonth() < 3) {// 开始日期是闰年并且包含2月
					days = new UFDouble(366);
				} else if (UFDate.isLeapYear(endDay.getYear())
						&& endDay.getMonth() > 2) {// 结束日期是闰年并且包含2月
					days = new UFDouble(366);
				} else {
					days = new UFDouble(365);
				}
	...
	/**
	 * 加权平均运算 运算规则：num1/(num1-num2) = mny1 / ? ? = (num1-num2) * mny1 / num1
	 * scale 代表保留精度位数
	 * 
	 */
	public static UFDouble averageMny(UFDouble num1, UFDouble num2,
