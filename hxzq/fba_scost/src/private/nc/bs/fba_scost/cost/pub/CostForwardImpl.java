...
public class CostForwardImpl implements ICostForward{
	...
	public UFDouble calcInterest(ICostingTool costingtool, ITrade_Data tradevo)
		...
		boolean iscalc = getQueryInfo().
				getBooleanFromInitcode(pk_glorg, CostConstant.PARAM_CALCBUYSELLINTEREST);
		boolean iscover = getQueryInfo().
				getBooleanFromInitcode(pk_glorg, CostConstant.PARAM_ISCONVERINTEREST);
		UFDouble lx = calcInterest(iscalc,iscover,tradevo);
		return lx;
	}
	
	public UFDouble calcInterestTrade(ITrade_Data tradevo)throws BusinessException{
	...
	public UFDouble calcInterest(boolean iscalc,boolean iscover,ITrade_Data tradevo)
			throws BusinessException {
		UFDouble lx = null;
		if(iscalc){//计算
			if(iscover ||//覆盖
	...
	public UFDouble forwardFairValueDistill(ICostingTool costingtool,
		...
		String pk_glorg = costingtool.getCostParaVO().getCheckParavo().getPk_glorgbook();
		boolean falg = getQueryInfo().
				getBooleanFromInitcode(pk_glorg, CostConstant.PARAM_FAIRFORWARD);
		UFDouble fx = null;
		if(falg){
	...
	@Override
	public UFDouble forwardFairValueDistill_debt(ICostingTool costingtool, ZqjdTallyVO zqjdtallyvo, ITrade_Data vo)
			throws BusinessException {
		// TODO 自动生成的方法存根
		String pk_glorg = costingtool.getCostParaVO().getCheckParavo().getPk_glorgbook();
		boolean falg = getQueryInfo().
