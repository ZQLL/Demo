...
public class QueryBasePubInfo implements IQueryBaseInfo {
	 ...
	 */
	@Override
	public CostPlanVO[] getCostPlanInfo(LoginContext context)
			throws BusinessException {
		if (context == null)
			return null;
		StringBuffer condition = new StringBuffer();
		condition.append("  pk_org = '" + context.getPk_org() + "' ");
		condition.append("  and pk_group = '" + context.getPk_group()
				+ "' and isnull(dr,0) = 0 ");
		condition.append("  and isbusinessplan = 'Y' ");
		condition.append("  and pk_glorgbook = '"
				+ AppContextUtil.getDefaultOrgBook() + "' ");
		CostPlanVO[] planvos = (CostPlanVO[]) new HYSuperDMO()
				.queryByWhereClause(CostPlanVO.class, condition.toString());
		return planvos;
	}
	 ...
	 * 查询当前审核日期
	 */
	public UFDate queryInitTallyVO(LoginContext context, String pk_glorgbook,
			String billtypegroup) throws BusinessException {
		...
		try {
			date = (UFDate) new BaseDAO().executeQuery(sb.toString(),
						...
						public Object handleResultSet(ResultSet rs)
							...
							if (rs.next()) {
								String distill_date = rs
										.getString("lastapprovedate");
								date = distill_date == null ? null
	...
	public String querySecuritesName(String pk_securities)
		...
		try {
			...
			// name = (String)new BaseDAO().executeQuery(sf.toString(), new
			// ResultSetProcessor ()
			name = (String) DBCacheQueryFacade.runQuery(sf.toString(),
					new ResultSetProcessor()
	 ...
	 */
	public String querySecuritesCode(String pk_securities)
			throws BusinessException {
		// 定义证券档案编码code
		String code = null;
		StringBuffer sf = new StringBuffer();
		// 根据pk_securities从证券档案表sec_securities中查询code
		sf.append("select code from  sec_securities where pk_securities = '"
				+ pk_securities + "' ");
		try {
			// 性能考虑，查询缓存
			code = (String) DBCacheQueryFacade.runQuery(sf.toString(),
					new ResultSetProcessor() {
						private static final long serialVersionUID = 1L;
	...
	/**
	 * 公允价值使用 应收利息计提使用 行情增加联合索引
	 */
	public TradeMarketVO queryLastMarket(String pk_securities, UFDate trade_date)
			throws BusinessException {
		...
		List<TradeMarketVO> list = (List<TradeMarketVO>) DBCacheFacade
				.runQuery(sql, new BeanListProcessor(TradeMarketVO.class));
	 ...
	 * 根据参数编码校验-----返回结果字符串
	 */
	public String getStringFromInitcode(String pk_glorgbook, String initcode)
			throws BusinessException {
		String va = null;
		StringBuffer sf = new StringBuffer();
		sf.append(" select value from pub_sysinit where pk_org  = '"
	...
	// */
	// public Map<String,TradeMarketVO> queryMarket(String[]
	// pk_securities,UFDate trade_date) throws BusinessException{
	// if(pk_securities == null || pk_securities.length == 0 || trade_date ==
	// null)
	// return null;
	...
	// vos = (TradeMarketVO[])new
	// HYSuperDMO().queryByWhereClause(TradeMarketVO.class, sf.toString());
	// }else{//采用临时表
	// String tablename = null;
	// try{
	// tablename = TempTableUtils.getTempTablename("temp_trademarket");
	// tablename =
	// TempTableUtils.createTempTable(tablename," pk_securities varchar(60),ts char(19)",
	// null);
	// TempTableUtils.insertIntoTable(tablename, "pk_securities",
	// pk_securities);
	...
	// List<TradeMarketVO> list = (List<TradeMarketVO>)new
	// BaseDAO().executeQuery(sf.toString(), new
	// BeanListProcessor(TradeMarketVO.class));
	...
	public ApproveVO[] getLastapprovedate(LoginContext context)
		...
		sb.append(" and pk_glorgbook ='" + AppContextUtil.getDefaultOrgBook()
				+ "' ");
		ApproveVO[] vos = (ApproveVO[]) new HYSuperDMO().queryByWhereClause(
				ApproveVO.class, sb.toString());
		return vos;
	}

	@Override
	public CheckplanVO getCheckplanVO(LoginContext context,
			String pk_transtypecode) throws BusinessException {
		StringBuffer sb = new StringBuffer();
		sb.append(" isnull(dr,0)=0 ");
		sb.append(" and pk_group = '" + context.getPk_group() + "'");
		sb.append(" and pk_org = '" + context.getPk_org() + "' ");
		...
		CheckplanVO[] vos = (CheckplanVO[]) new HYSuperDMO()
				.queryByWhereClause(CheckplanVO.class, sb.toString());
		if (vos == null || vos.length == 0)
