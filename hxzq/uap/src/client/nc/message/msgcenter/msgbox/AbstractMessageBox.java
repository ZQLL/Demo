...
public abstract class AbstractMessageBox implements IMessageBox
{
	...
	protected void loadTableModelData()
	{
		SwingWorker<Object, Object> worker = new SwingWorker<Object, Object>()
		{
			...
			{
				msglist = getBusiDelegator().qryMessageByCondition(getCondition());
				return null;
			}
	...
	public String convertToSQL(MsgQueryInfo qryinfo)
	{
		...
		{
			sbd.append(NCMessageSqlBuilder.buildSendDateSql(convertTODate(qryinfo.getQuikdate()), null));
		}
		else
		{
			sbd.append(NCMessageSqlBuilder.buildSubjectSql(qryinfo.getSubject(), null));
			
			if(sbd.length()>0)
				sbd.append(NCMessageSqlBuilder.buildSubjectSql(qryinfo.getSubject(), "AND"));
			else
				sbd.append(NCMessageSqlBuilder.buildSubjectSql(qryinfo.getSubject(), null));
			
			if(sbd.length()>0)
				sbd.append(NCMessageSqlBuilder.buildSenderSql(qryinfo.getSender(), "AND"));
			else
				sbd.append(NCMessageSqlBuilder.buildSenderSql(qryinfo.getSender(), null));
			
			//add by lihaibo 部门参照
			if(sbd.length()>0)
				sbd.append(NCMessageSqlBuilder.buildDetailSql(qryinfo.getDetail(), "AND"));
			else
				sbd.append(NCMessageSqlBuilder.buildDetailSql(qryinfo.getDetail(), null));
			
			//add by lihaibo 单据类型参照
			if(sbd.length()>0)
				sbd.append(NCMessageSqlBuilder.buildBillTypeSql(qryinfo.getBillType(), "AND"));
			else
				sbd.append(NCMessageSqlBuilder.buildBillTypeSql(qryinfo.getBillType(), null));
			
			if(sbd.length()>0)
				sbd.append(NCMessageSqlBuilder.buildSendDateSql(convertTODate(qryinfo.getSenddate()), "and"));
			else
				sbd.append(NCMessageSqlBuilder.buildSendDateSql(convertTODate(qryinfo.getSenddate()), null));
			
			if(sbd.length()>0)
				sbd.append(NCMessageSqlBuilder.buildSourceSql(qryinfo.getFrom(), "AND"));
			else
				sbd.append(NCMessageSqlBuilder.buildSourceSql(qryinfo.getFrom(), null));
		}
	...
	public void reQuery()
	{
		List<NCMessage> messages = getBusiDelegator().qryMessageByCondition(getCondition());
	...
	public void processStateChange(List<NCMessage> messages)
	{
		...
		{
			NCMessage ncmsg = getTableModel().getMessageByPK(message2.getMessage().getPk_message());
			if (ncmsg != null)
			{
				ncmsg.getMessage().setIsread(message2.getMessage().getIsread());
				ncmsg.getMessage().setIshandled(message2.getMessage().getIshandled());
			}
		}
