...
public class DataImportMethod {
	 ...
	 * @return Hashtable
	 */
	public Map<String, Object> getConstInfoByDataImportVO(AggDataImportVO aggDataImportVO) {
		// 表体选择字段对照关系
		...
		if(null!=dataImportBVO1&&dataImportBVO1.length>0){
			...
			for (int i = 0; i <dataImportBVO1.length; i++) {
				Object aim_fieldObj  = dataImportBVO1[i].getAttributeValue(DataImportBVO1.AIM_FIELD);
				Object ds_fieldObj = dataImportBVO1[i].getAttributeValue(DataImportBVO1.DS_FIELD);
				Object is_nullObj = dataImportBVO1[i].getAttributeValue(DataImportBVO1.IF_NULL);
				Object dataType = dataImportBVO1[i].getAttributeValue(DataImportBVO1.DATATYPE);
				Object aim_field_cnObj = dataImportBVO1[i].getAttributeValue(DataImportBVO1.AIM_FIELD_CN);
				Object expsystem = dataImportBVO1[i].getAttributeValue(DataImportBVO1.DEF_EXPSYS);
				Object if_ref = dataImportBVO1[i].getAttributeValue(DataImportBVO1.IF_REF);
				Object refVal = dataImportBVO1[i].getAttributeValue(DataImportBVO1.DEF_PKBDINFO);
				Object if_formular = dataImportBVO1[i].getAttributeValue(DataImportBVO1.IF_FORMULAR);
				Object impFormular = dataImportBVO1[i].getAttributeValue(DataImportBVO1.IMPFORMULAR);
				if("Y".equals(if_ref.toString())){
					if(expsystem == null){
						throw new RuntimeException(aim_field_cnObj.toString()+":"+aim_fieldObj.toString()+"没有配置外系统，请确认");
					}
					if(refVal == null){
						throw new RuntimeException(aim_field_cnObj.toString()+":"+aim_fieldObj.toString()+"没有配置参照值，请确认");
					}
				}
				if("Y".equals(if_formular.toString())){
					if(impFormular == null){
						throw new RuntimeException(aim_field_cnObj.toString()+":"+aim_fieldObj.toString()+"是否公式打勾，但没有设置公式，请确认");
					}
				}
				if("N".equals(is_nullObj.toString())){
					if(ds_fieldObj == null){
						throw new RuntimeException(aim_field_cnObj.toString()+":"+aim_fieldObj.toString()+"导入文件字段没有配置，请确认");
					}
				}
				...
				if(dataType == null){
					throw new RuntimeException(aim_field_cnObj.toString()+":没有设置数据类型，请确认");
				}
		...
		if(null!=dataImportBVO2&&dataImportBVO2.length>0){
			...
			for (int i = 0; i < dataImportBVO2.length; i++) {
				Object aim_fieldObj = dataImportBVO2[i].getAttributeValue(DataImportBVO2.AIM_FIELD);
				Object ds_fieldObj = dataImportBVO2[i].getAttributeValue(DataImportBVO2.DS_FIELD);
				Object aim_field_cnObj = dataImportBVO2[i].getAttributeValue(DataImportBVO2.AIM_FIELD_CN);
				Object is_nullObj = dataImportBVO2[i].getAttributeValue(DataImportBVO2.IF_NULL);
				Object dataType = dataImportBVO2[i].getAttributeValue(DataImportBVO2.DATATYPE);
				if("N".equals(is_nullObj.toString())){
					if(ds_fieldObj == null){
						throw new RuntimeException(aim_field_cnObj!=null?aim_field_cnObj.toString():""+":"+aim_fieldObj!=null?aim_fieldObj.toString():""+"导入文件不允许为空字段没有配置，请确认");
					}
				}
				if (aim_fieldObj == null || ds_fieldObj == null){
					continue;
	...
	public Map<String, Object> getConstInfoByDataImportVOHead(AggDataImportVO aggDataImportVO) {
		// 表体选择字段对照关系
		Map<String, Map<String,String>> ht_aim_sd = new HashMap<String, Map<String,String>>();// key:目标字段,value:源字段
		Map<String, Map<String,String>> ht_sd_aim = new HashMap<String, Map<String,String>>();// key:源字段,value:目标字段
		Map<String,List<String>> sd_field = new HashMap<String,List<String>>();// 源字段
		...
		if(null!=dataImportBVO2&&dataImportBVO2.length>0){
			...
			List<String> aim_field_body = new ArrayList<String>();
			for (int i = 0; i < dataImportBVO2.length; i++) {
				Object aim_fieldObj = dataImportBVO2[i].getAttributeValue(DataImportBVO1.AIM_FIELD);
				Object ds_fieldObj = dataImportBVO2[i].getAttributeValue(DataImportBVO1.DS_FIELD);
				Object aim_field_cnObj = dataImportBVO2[i].getAttributeValue(DataImportBVO1.AIM_FIELD_CN);
				Object is_nullObj = dataImportBVO2[i].getAttributeValue(DataImportBVO1.IF_NULL);
				Object dataType = dataImportBVO2[i].getAttributeValue(DataImportBVO1.DATATYPE);
				if("N".equals(is_nullObj.toString())){
					if(ds_fieldObj == null){
						throw new RuntimeException(aim_field_cnObj!=null?aim_field_cnObj.toString():""+":"+aim_fieldObj!=null?aim_fieldObj.toString():""+"导入文件不允许为空字段没有配置，请确认");
					}
	 ...
	 * @param ht_aim_sd
	 * @param ht_sd_aim
	 * @throws Exception	void	TODO（参数说明）
	 */
	public void getImportDataByDS(DataSourceSetVO setvo, boolean isserver, AggDataImportVO aggDataImportVO,
			String wheresql, Map hm_info) throws Exception {
		if (null==aggDataImportVO)
			return;
		// 如果考虑性能问题，这里实际可以根据特殊处理类getQueryCond方法是否有值类判断,目前先直接=true
		boolean isTableVoQuery = true;
		if (isserver) {
			IDataImportQueryService service = (IDataImportQueryService) NCLocator.getInstance().lookup(IDataImportQueryService.class.getName());
			// 如果是服务器端，直接从后台得到数据，并将数据在后台缓存
			service.getDataBySql(setvo, aggDataImportVO, isTableVoQuery, hm_info);
		} else {
	 ...
	 */
	private List<FieldVO> getTableVO(Connection con, DataSourceSetVO setvo, String tablename,
		...
		String schemaPattern = PubMethod.getSchema(setvo.getDstype(), setvo.getDsuser());
...
//					fieldvo.setSqlType(datatype);
//					fieldvo.setMaxLength(columnsize);
//					fieldvo.setPrecision(((datatype == 6 || datatype == 7 || datatype == 8) && scale < 1) ? 8
//							: scale);
//
//					v_vos.add(fieldvo);
//				}
...
//		}
	...
	public void getDataByFilePathHead(AggDataImportVO aggDataImportVO, String filepath,Map hm_info) throws Exception {
		DataImportVO parentVO = (DataImportVO) aggDataImportVO.getParent();
		String relevaFieldName = (String) parentVO.getAttributeValue(DataImportVO.RELEVAFIELDNAME);
			...
			for (int i = 0; i < files.length; i++) {
				...
				if (files[i].getName().toLowerCase().endsWith(".csv")) {
					...
					for (int j = 1; j < iRow; j++) {
						...
						List<String> sd_field_lst = sd_fields_map.get(SystemConst.HEAD);// CSV仅支持单表，所以仅取HEAD.
						String[] sd_fields = sd_field_lst.toArray(new String[] {});
						int length = sd_fields.length;
						int now = 0;
						for (int n = 0; n < length; n++) {
							String[] temp = sd_fields[n].split("\\" + SystemConst.perfix);
							for (int m = 0; m < temp.length; m++) {
								String fieldm = temp[m];
								if(!temp[0].equals("提前还款比例")&&!temp[0].equals("a")){
									row_v.add(CSVUtil.getData(fieldm, ht, vCell));
								}
								// 处理文件头字段名字，当出现下面字段，赋值到子表中
								if(temp[0].equals("起息日")){
									row_b.add(CSVUtil.getData(fieldm, ht, vCell));
									if (j == 1 && firstRow == 0) {
										String fieldmVal = StringHdlUtil.handlerKey(fieldm);
										bodytitmap.put(fieldmVal + 1, bodytitmap.size());
									}
								}else if(temp[0].equals("到期日")){
									row_b.add(CSVUtil.getData(fieldm, ht, vCell));
									if (j == 1 && firstRow == 0) {
										String fieldmVal = StringHdlUtil.handlerKey(fieldm);
										bodytitmap.put(fieldmVal + 3, bodytitmap.size());
									}
								}else if(temp[0].equals("票面利率")){
									row_b.add(CSVUtil.getData(fieldm, ht, vCell));
									if (j == 1 && firstRow == 0) {
										String fieldmVal = StringHdlUtil.handlerKey(fieldm);
										bodytitmap.put(fieldmVal + 2, bodytitmap.size());
									}
								}else if(temp[0].equals("提前还款比例")){
									row_b.add(CSVUtil.getData(fieldm, ht, vCell));
									if (j == 1 && firstRow == 0) {
										String fieldmVal = StringHdlUtil.handlerKey(fieldm);
										bodytitmap.put(fieldmVal + 0, bodytitmap.size());
									}
								}else if(temp[0].equals("a")){
									continue;
								}
								
								if (j == 1 && firstRow == 0 && !temp[0].equals("提前还款比例")) {
									String fieldmVal = StringHdlUtil.handlerKey(fieldm);
									headtitmap.put(fieldmVal + (now-1), headtitmap.size());
								}
								now++;
							}
						}
					...
					Map<String, List<List<List<String>>>> sheetMap = ExcelUtil.readExcel(files[i].getPath(), 2);
					for (String key : sheetMap.keySet()) {
						...
						for (int j = 1; j < iRow; j++) {
							...
							if(sd_fields == null || sd_fields.length ==0){
								throw new BusinessException("导入文件出现异常，请确认文件是否正确。");
							}
							int length = sd_fields.length;
							for (int n = 0; n < length; n++) {
								String[] temp = sd_fields[n].split("\\" + SystemConst.perfix);
								for (int m = 0; m < temp.length; m++) {
									if (relevaFieldName != null && relevaFieldName.equals(temp[m])) {
										isReleFld = true;
									}
									
									Object dataObj = ExcelUtil.getData(temp[m], ht, vCell);
									...
									//row_v.add((String) ExcelUtil.getData(temp[m], ht, vCell));
									// 处理文件头字段名字
									if (j == 1 && firstRow == 0 ) {
										String fieldmVal = StringHdlUtil.handlerKey(temp[m]);

										if (SystemConst.HEAD.equals(key)) {
											headtitmap.put(fieldmVal + n, headtitmap.size());
										} else {
											bodytitmap.put(fieldmVal + n, bodytitmap.size());
										}
									}
								}
							}
							if (!isReleFld && relevaFieldName != null) {// 说明没有在字段对照中配置，因为主子表中关联字段在后台需要设置.所以将字段加到titmap中
								row_v.add((String) ExcelUtil.getData(relevaFieldName, ht, vCell));
								if (j == 1 && firstRow == 0) {
									if (SystemConst.HEAD.equals(key)) {
										headtitmap.put(relevaFieldName + length, headtitmap.size());
									} else {
										bodytitmap.put(relevaFieldName + length, bodytitmap.size());
									}
								}	
							}
							if (SystemConst.HEAD.equals(key)) {
...
//			dataSpecialDealBySpeFile(metaVO, sd_fields, aim_fields, data_v);
	...
	public void getDataByFilePath(AggDataImportVO aggDataImportVO, String filepath,Map hm_info) throws Exception {
		DataImportVO parentVO = (DataImportVO) aggDataImportVO.getParent();
		String relevaFieldName = (String) parentVO.getAttributeValue(DataImportVO.RELEVAFIELDNAME);
		Map<String,List> map_v= new HashMap<String,List>();
		List<String> row_v = null;
...
//			dataSpecialDealBySpeFile(metaVO, sd_fields, aim_fields, data_v);
	...
	public void bakFile(ImportFileSetVO vo, String hitMsg) throws Exception {
		...
		if (isserver.booleanValue()) {
			...
			try {
				...
				throw new Exception("数据已经正常导入 "+hitMsg+"但备份文件" + fromFile.getName()
						+ "失败，请确认文件没有打开!");
			}
		}
	}
	...
	private void copy(File fromFile, String toFileName) throws Exception {
		...
		for (int i = 0; i < fs.length; i++) {
			...
			if (fs[i].isDirectory()
					|| (!fs[i].getName().toLowerCase().endsWith(".csv")&& !fs[i].getName().toLowerCase().endsWith(".xls"))) {
				continue;
			}
	...
	public Map getImportTitle() throws BusinessException {
		IDataImportBusiService busiService = NCLocator.getInstance().lookup(IDataImportBusiService.class);
	...
	public void getDataBySql(DataSourceSetVO setvo, AggDataImportVO aggVO,boolean isTableVoQuery, Map hm_info) throws Exception {
...
//			password = encode.encode(password.trim());
			con = PubMethod.getInstance().getConnect(setvo.getDriverclass(), setvo.getDsurl(),setvo.getDsuser(), password);
			List<Object> headDataLst = getDataByTypeCode(sd_fields, con,SystemConst.HEAD, querysqlhead);
			List<Object> bodyDataLst = getDataByTypeCode(sd_fields, con,SystemConst.BODY, querysqlbody);
			Map<String,List> map_v= new HashMap<String,List>();
			map_v.put(SystemConst.HEAD, headDataLst);
			map_v.put(SystemConst.BODY, bodyDataLst);
			IDataImportBusiService dS = (IDataImportBusiService) NCLocator.getInstance().lookup(IDataImportBusiService.class.getName());
	...
	private void handlerSdFields(Map sd_fields, AggDataImportVO aggVO) {
		...
		DataImportBVO1[] dataImportBVO1=(DataImportBVO1[]) aggVO.getChildren(DataImportBVO1.class);
		DataImportBVO2[] dataImportBVO2=(DataImportBVO2[]) aggVO.getChildren(DataImportBVO2.class);
		...
		if(sd_fields.get(SystemConst.BODY) != null){
			List<String> bodyLst = (List<String>) sd_fields.get(SystemConst.BODY);
	...
	private void addVarHeadFld(DataImportBVO1[] dataImportBVO1, Map<String, String> varHeadFldMap) {
		...
		if(dataImportBVO1 != null  && dataImportBVO1.length>0){
			...
			for(int i=0;i<bvoLen;i++){
				...
				if(bvo.getIf_formular().booleanValue()){
					...
					String impformular = (String) bvo.getAttributeValue(DataImportBVO1.IMPFORMULAR);
					if(impformular == null || "".equals(impformular)){
						throw new RuntimeException(bvo.getAim_field_cn()+" [是否公式]打勾，但您没有设置公式内容，请确认！");
					}
	...
	private void addVarBodyFld(DataImportBVO2[] dataImportBVO2, Map<String, String> varBodyFldMap) {
		if(dataImportBVO2 != null  && dataImportBVO2.length>0){
			...
			for(int i=0;i<bvoLen;i++){
				...
				if(bvo.getIf_formular().booleanValue()){
					FormulaParseFather formulaParse = new nc.bs.pub.formulaparse.FormulaParse();
					String impformular = (String) bvo.getAttributeValue(DataImportBVO2.IMPFORMULAR);
					formulaParse.setExpress(impformular);
					VarryVO[] varrys = formulaParse.getVarryArray();
					for (int j = 0; j < varrys.length; j++)
					{
	...
	private String getSqlByType(Map sd_fields,String type, AggDataImportVO aggVO) throws BusinessException{
		DataImportVO parentVO = aggVO.getParentVO();
	 ...
	 */
	private void appendWhereCondByCondSet(StringBuffer sqlBuf, String tableType, DataImportVO parentVO)throws BusinessException {
		...
		Collection<ImportCondVO> vos = query.retrieveByClause(ImportCondVO.class, conBuf.toString());
	...
	private List<Object> getDataByTypeCode(Map sd_fields, Connection con,String typecode, String querysqlstr) throws Exception {
