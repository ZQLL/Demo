 ...
 * 导入后台缓存处理
 * 封装了前台界面导入的以下信息
 * 1.原始导入数据 _busi_DataLst
 * 2.字段对照信息，正向对照，逆向对照。_normal_MetaData_Cache|_unNormal_MetaData_Cache
 * 3.处理成功的数据，处理成功的含义为：参照、公式、运算等全部能处理成功的数据。
 * 4.处理失败的数据，处理失败的含义为：参照或者公式或者运算出现异常的数据。
 * 5.前台界面展现用的字段信息。
 * @author wyg
 */
...
public class DataImportCacheHdl {
	...
	/*
	 * 用于组缓存字段配置信息(正向 aim field-->sd field)：
	...
	/*
	 * 用于组缓存字段配置信息(反向 sd field-->aim field)：
	 *  Map<String,Map<String,Object>>
	 *      头/主表/子表    	SD字段名称  BVO
	 *       T/H/B
	 */
	 ...
	 * 导入数据缓存
	 * List中每个维度为一行数据，
	 * Map<String,List<Map<String,Object>>> 解析：Map<主表/子表,List<Map<字段，值>>> 行数据;
	 ...
	 * 这些业务数据未经处理，是原始数据
	 * @param hm_info 
	 * @param result 0 title,1 tablename,2 datalist
	 * @param aggDataImportVO 
	 * @throws BusinessException
	 */
	@SuppressWarnings({ "rawtypes" })
	public void cacheBusiData(AggDataImportVO aggDataImportVO, Map<String,List> result, Map hm_info)throws BusinessException
	{
		initOrgInfo();
		//处理字段对照缓存
	...
	/**
	 * 处理参照，如果发现没有对照关系的数据将其放入 failDataLst中
	 * 数据正确的放入 successDataLst
	 */
	 ...
	 */
	@SuppressWarnings({ "rawtypes", "unchecked" })
	private void convertRefData(List<Map> dataLst, String tabNo) throws BusinessException{
	...
	@SuppressWarnings({ "rawtypes" })
	private boolean canConvert(Map rowMap,String tabNo) throws BusinessException{
		...
		if(rowMap != null && rowMap.size()>0){
			...
			while(keyIt.hasNext()){
				...
				if(fldMap.get(fldKey) != null){
					...
					UFBoolean if_ref = (UFBoolean) metaDataVO.getAttributeValue(DataImportBVO1.IF_REF);
	...
	/**
	 * @param rowMap Key:字段名  value:值(文件或者数据源得到的)
	 * @param fldKey
	 * @param tabNo
	 * @return
	 */
	@SuppressWarnings({ "rawtypes", "unchecked" })
	private boolean getValueFromRef(Map rowMap, String fldKey, String tabNo) throws BusinessException{
		...
		if(fldMap.get(fldKey) != null){
			SuperVO metaDataVO = (SuperVO) fldMap.get(fldKey);
			UFBoolean if_ref = (UFBoolean) metaDataVO.getAttributeValue(DataImportBVO1.IF_REF);
			UFBoolean if_formular = (UFBoolean) metaDataVO.getAttributeValue(DataImportBVO1.IF_FORMULAR);
			String defaultValSet = (String)metaDataVO.getAttributeValue(DataImportBVO1.DEFAULTVALUE);
			//判断 是否设置参照
			if(if_formular.booleanValue() || if_ref.booleanValue() || defaultValSet != null){
				...
				if(if_ref.booleanValue()){
					...
					canRefValue = handleRef(rowMap, fldKey, true, metaDataVO, orivalue);
					if(!canRefValue){
						return canRefValue;
					}
				}
	...
	private void handlerDefaultVal(Map rowMap, String fldKey, SuperVO metaDataVO, String orivalue) {
		String defaultVal = (String)metaDataVO.getAttributeValue(DataImportBVO1.DEFAULTVALUE);
		Object  custDefVal = metaDataVO.getAttributeValue(DataImportBVO1.CUSTDEFAULTVAL);
	...
	@SuppressWarnings("unchecked")
	private boolean handleFormular(Map rowMap, String fldKey, boolean canRefValue, SuperVO metaDataVO, String orivalue)throws BusinessException {
		boolean bln = false;
		FormulaParseFather formulaParse = new nc.bs.pub.formulaparse.FormulaParse();
		String impformular = (String) metaDataVO.getAttributeValue(DataImportBVO1.IMPFORMULAR);
		...
		if(varrys == null || varrys.length == 0){
			throw new BusinessException(fldKey+"设置的公式："+impformular+"不正确，请确认！公式配置错误或者不支持此公式。");
		}
		for (int i = 0; i < varrys.length; i++)
		{
		    ...
		    if(varries != null)
		    {
		        ...
		        {
		            Object varryValue = getValByDataType(rowMap, metaDataVO, varries[j]);
	...
	private Object getValByDataType(Map rowMap, SuperVO metaDataVO, String varStr) throws BusinessException{
		String varryValue = null;
		if(rowMap.get(varStr) != null || rowMap.get(varStr.toUpperCase()) !=null){
		...
		String dataType = (String) metaDataVO.getAttributeValue(DataImportBVO1.DATATYPE);
	 ...
	 */
	@SuppressWarnings({"rawtypes","unchecked"})
	private boolean handleRef(Map rowMap, String fldKey, boolean canRefValue, SuperVO metaDataVO, String orivalue) throws BusinessException {
		String expsys = (String) metaDataVO.getAttributeValue(DataImportBVO1.DEF_EXPSYS);
		String pk_bdinfo = (String) metaDataVO.getAttributeValue(DataImportBVO1.DEF_PKBDINFO);
		Map topMap = _normal_MetaData_Cache.get(SystemConst.TOP);
		DataImportVO tVO  = (DataImportVO) topMap.get(SystemConst.TOP);
	
		String trasferData = DataImportBusiDAO.getContrastValue(expsys, pk_bdinfo,orivalue,tVO);
		if(trasferData == null){
			canRefValue = false;
	...
	/**
	 * 
	 * @param dataLst 格式：0 List<行数据> 1 表名 2 MAP< 字段  ,位置>
	 * @param tabNo
	 */
	@SuppressWarnings({ "rawtypes", "unchecked" })
			...
			if(busiData != null && busiData.size()>0){
				...
				for(int i=0;i<busiSize;i++){
					...
					while(itKey.hasNext()){
						...
						String mapField = key.substring(0,key.length()-posiVal.length());
						fldV.put(mapField, rowData.get(Integer.parseInt(posiVal)));
	...
	private void initMetaDataCache(AggDataImportVO aggDataImportVO, Map hm_info) {
		...
		DataImportBVO1[] bvo1 = (DataImportBVO1[]) aggDataImportVO.getChildren(DataImportBVO1.class);
		DataImportBVO2[] bvo2 = (DataImportBVO2[]) aggDataImportVO.getChildren(DataImportBVO2.class);
		...
		if(bvo1 != null && bvo1.length >0){
			...
			for(int i=0;i<bvo1Len;i++){
				if(bvo1[i].getDs_field() != null && !"".equals(bvo1[i].getDs_field())){
		...
		if(bvo2 != null && bvo2.length >0){
			...
			for(int i=0;i<bvo2Len;i++){
				if(bvo2[i].getDs_field() != null && !"".equals(bvo2[i].getDs_field())){
	...
	public void clear(){
		String currentUser = InvocationInfoProxy.getInstance().getUserCode();
		DataImportCacheHdl cacheInstance = _user_Cache_Instance.get(currentUser);
		if(cacheInstance != null){
			cacheInstance = null;
			_user_Cache_Instance.put(currentUser, cacheInstance);
		}
	...
	private void initOrgInfo() {
		...
		try {
			...
			Map<String, String> map = NCLocator.getInstance().lookup(IOrgUnitPubService.class).getNewVIDSByOrgIDSAndDate(new String[] { pk_org }, busiDate);
