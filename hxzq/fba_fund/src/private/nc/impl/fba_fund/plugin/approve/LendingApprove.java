...
public class LendingApprove
  extends AbstractLendingApprove
{
  protected void calculateWhenCheck(ICostingTool costingtool, InterbankLendingVO tradevo)
    throws Exception
  {
    ...
    String pk_glorgbook = costingtool.getCostParaVO().getCostplanvo().getPk_glorgbook();
    
    String[] costFieldArray = costingtool.getCostParaVO().getCostplanvo().getFundCostFieldArray();
    ...
    InventoryInfoVO inventoryInfoVO = tool.getInventoryInfoVO(queryvo, costingtool);
    if (inventoryInfoVO == null)
    {
      if (bln)
      {
        costingtool.handleException(tradevo, null, nc.vo.fba_fund.pub.SystemConst.error[0]);
        return;
      }
  ...
  protected void calculateWhenUnCheck(ICostingTool costingtool, InterbankLendingVO vo)
    throws Exception
  {
    String wherestr = " pk_opbill='" + vo.getPk_interbanklending() + "' and isnull(dr,0)=0";
  ...
  private void calInventory(InventoryInfoVO inventoryInfoVO, InterbankLendingVO tradevo, String trade_date)
    throws Exception
  {
    ...
    if (transtype != null) {
      if (transtype.equals("HV8A-0xx-02"))
      {
        ...
        if (pm.sub(stocks_sum, denomination).compareTo(new UFDouble(0)) < 0) {
          throw new BusinessException(" 交易日期:" + trade_date + " " + nc.vo.fba_fund.pub.SystemConst.error[1]);
        }
        inventoryInfoVO.setStocks_sum(pm.sub(inventoryInfoVO.getStocks_sum(), tradevo.getDenomination()));
        
        inventoryInfoVO.setTranstypecode(tradevo.getTranstypecode().replace("01", "02"));
      }
      else
      {
        inventoryInfoVO.setStocks_sum(pm.add(inventoryInfoVO.getStocks_sum(), tradevo.getDenomination()));
      }
    }
  }
  
  private void buildTally(InterbankLendingVO vo, String pk_glorgbook, UFDate trade_date)
    throws Exception
  {
