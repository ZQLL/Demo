...
public class AbstractLendingApprove
  extends AbstractPlugin
{
  ...
  {
    IInterbanklendingMaintain iInterbanklendingMaintain = (IInterbanklendingMaintain)NCLocator.getInstance().lookup(IInterbanklendingMaintain.class);
    return iInterbanklendingMaintain;
  }
  
  public IBill[] queryData(ICostingTool costingtool)
    throws BusinessException
  {
    IQueryScheme queryscheme = QuerySchemeUtil.createQueryScheme("fba_fund.InterbankLendingVO", costingtool.getCostParaVO()
      .getCondition());
    AggInterbankLendingVO[] aggvos = getBusiInstance().queryBillByPK(getBusiInstance().queryPKs(queryscheme));
    return aggvos;
  }
  ...
  public void checkBills(ICostingTool costingtool, TradeDataTool tradedatatool)
    throws Exception
  {
    String datakey = costingtool.getCurrbilltypegroupvo().getPk_billtypegroup() + costingtool.getCurrdate() + 
    ...
    if ((ibills != null) && (ibills.length > 0)) {
      for (IBill ibill : ibills)
      {
        ...
        vo.setApprover(costingtool.getCostParaVO().getCheckParavo().getPk_user());
        vo.setApprovedate(PubMethod.getInstance().getDateTime(costingtool.getCurrdate()));
        vo.setState(Integer.valueOf(nc.vo.fba_fund.pub.SystemConst.statecode[3]));
      }
    }
  }
  
  protected void calculateWhenCheck(ICostingTool costingtool, InterbankLendingVO vo)
    throws Exception
  {}
  
  protected void calculateWhenUnCheck(ICostingTool costingtool, InterbankLendingVO InterbankLendingVO)
    throws Exception
  {}
  
  public void unCheckBills(ICostingTool costingtool, TradeDataTool tradedatatool)
    throws Exception
  {
    String datakey = costingtool.getCurrbilltypegroupvo().getPk_billtypegroup() + costingtool.getCurrdate() + 
      costingtool.getCurrbilltype();
    ...
    if ((ibills != null) && (ibills.length > 0)) {
      for (IBill ibill : ibills)
      {
        ...
        vo.setState(Integer.valueOf(nc.vo.fba_fund.pub.SystemConst.statecode[1]));
      }
    }
  }
  
  public IBill[] updateData(ICostingTool costingtool, TradeDataTool tradedatatool)
    throws BusinessException
  {
    IBill[] ibills = tradedatatool.getData(costingtool.getCurrbilltypegroupvo().getPk_billtypegroup() + costingtool.getCurrdate() + 
      costingtool.getCurrbilltype());
    if (ibills != null)
    {
  ...
  public IBill[] insertData(ICostingTool costingtool, TradeDataTool tradedatatool)
    throws BusinessException
  {
    return null;
  }
