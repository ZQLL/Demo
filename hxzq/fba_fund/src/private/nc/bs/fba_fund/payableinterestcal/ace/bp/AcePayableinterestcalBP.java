...
public class AcePayableinterestcalBP {
	...
	public PayableInterestCalVO[] queryByQueryScheme(IQueryScheme querySheme) {
		...
		return new SchemeVOQuery<PayableInterestCalVO>(
				PayableInterestCalVO.class).query(querySheme, null);
	}
			...
			if (list != null && list.size() > 0) {
				...
				for (FundTallyVO vo : vos) {
					...
					if (dqlist != null && dqlist.size() > 0) {
						...
						dqvos = (FundTallyVO[]) dqlist
								.toArray(new FundTallyVO[0]);
						dq_denomination = dqvos[0].getDenomination();
					}
					// 计算本次计提总天数
					int caldays = this.getCalDays(distilldate, vo);
					// 构建台账
					List<FundTallyVO> tally = this.calInterestAndBuildTally(
							distilldate, vo, info, caldays, dq_denomination);
					if (null != tally && tally.size() != 0) {
						// 本次计提总利息
						UFDouble this_allinterest = new UFDouble(0.00);
						for (FundTallyVO tallyVO : tally) {
							// 累加
							this_allinterest = pm.add(this_allinterest,
									tallyVO.getThis_interest());
						}
						FundTallyVO lastTally = tally.get(tally.size() - 1);
						// 返回构建的应付利息计提VO
						PayableInterestCalVO distillvo = this.buildDistillVO(
								lastTally, vo, this_allinterest, caldays,
	...
	private int getCalDays(UFDate distilldate, FundTallyVO vo) {
		...
		if (distilldate.asBegin().before(vo.getEnddate().asBegin())) {
			...
			if (null != vo.getIndistill_date()) {
				caldays = UFDate.getDaysBetween(vo.getIndistill_date()
						.asBegin(), distilldate);
			} else {
				// 计提日=起息日，计息一天
				if (distilldate.isSameDate(vo.getBegindate())) {
					...
					// 第一次计提：算头算尾
					caldays = UFDate.getDaysBetween(
							vo.getBegindate().asBegin(), distilldate) + 1;
				}
			}
		} else {
			if (null != vo.getIndistill_date()) {
				...
				caldays = UFDate.getDaysBetween(vo.getIndistill_date()
						.asBegin(), vo.getEnddate().asBegin());
			} else {
				// 计提结束日为空，说明没有计提过，取起息日，到期日来计算
				caldays = UFDate.getDaysBetween(vo.getBegindate().asBegin(), vo
						.getEnddate().asBegin()) + 1;
			}
		}
		return caldays;
	}
	 ...
	 */
	private List<FundTallyVO> calInterestAndBuildTally(UFDate distilldate,
			FundTallyVO vo, QueryInterestBaseInfo info, int caldays,
			UFDouble dq_denomination) throws BusinessException {
		// 新台帐集合
		List<FundTallyVO> tallyList = new ArrayList<FundTallyVO>();
		// 计提日期
		...
		this_interest = pm.div(pm.multiply(denomination, cal_rate),
		...
		for (int i = 0; i < caldays; i++) {
			// 计息日=起息日，构建一条当天的台账数据
			if (null == tally && distillbegindate.isSameDate(vo.getBegindate())
					&& vo.getOperindex().equals(0)) {
			...
			if (i == 0) {
				// 第一天使用台账表里最新数据进行构建
				tally = this.buildTallyVO(distillbegindate, vo, this_interest,
						had_interest, 0, dq_denomination);
			} else {
				// 第二天开始使用上次构建的台账VO进行构建
				tally = this.buildTallyVO(distillbegindate, tally,
						this_interest, had_interest, 1, dq_denomination);
			}
			tallyList.add(tally);
		}
		return tallyList;
	 ...
	 * 构建台账
	 * 
	 * @param distillbegindate
	 *            计提开始日
	 * @param vo
	 *            要计提的台账数据，或构建的上一天台账数据
	 * @param this_interest
	 *            计算好的本此计提利息
	 * @param had_interest
	 *            要计提的台账中的已计提利息（可能为0）
	 ...
	 *            本次计息总天数
	 * @param type
	 *            构建台账方式：0-本次构建的第一天，使用vo构建。1-使用构建的上一天数据构建
	 * @param info
	 *            到期的面额
	 * @return 构建的台账VO
	 * @author QS
	 ...
	 */
	private FundTallyVO buildTallyVO(UFDate distillbegindate, FundTallyVO vo,
			UFDouble this_interest, UFDouble had_interest, int type,
			UFDouble dq_denomination) {
		// 取当前系统时间作为业务发生日期
		...
		if (type == 0) {
			...
			tallyVO.setHad_interest(pm.add(vo.getThis_interest(),
					vo.getHad_interest()));
		}
		// 如果计提日期等于到期日，则计算面额
		if (null != dq_denomination
				&& distillbegindate.isSameDate(vo.getEnddate())) {
			// 面额
			tallyVO.setDenomination(pm.sub(vo.getDenomination(),
					dq_denomination));
			// 到期标识
	 ...
	 */
	private PayableInterestCalVO buildDistillVO(FundTallyVO lastTally,
			FundTallyVO vo, UFDouble this_allinterest, int caldays,
			UFDouble dq_denomination, UFDate distilldate)
			throws BusinessException {
		...
		// 产品或对手名称
		distillvo
				.setProductorcounterparty(lastTally.getProductorcounterparty());
		// 市场
		...
		// 面额
		if (null != dq_denomination) {
			distillvo.setDenomination(pm.sub(vo.getDenomination(),
					dq_denomination));
		}
		// 合同号
		...
		UFDouble all_interest = pm.multiply(new UFDouble(vo.getTimelimit()), pm
				.div(pm.multiply(vo.getDenomination(),
						pm.div(vo.getExcutefacerate(), new UFDouble(100))),
						new UFDouble(vo.getYearcaldays())));
		// 本次计提利息
		distillvo.setThis_cal_interest(this_allinterest);
		// 已计提利息
		distillvo.setAlready_cal_interest(pm.add(vo.getThis_interest(),
				vo.getHad_interest()));
		// 未计提利息=所有利息-本次计提利息-已计提利息
		distillvo.setNot_cal_interest(pm.sub(
				pm.sub(all_interest, this_allinterest),
				pm.add(vo.getThis_interest(), vo.getHad_interest())));
		// 计提开始日
		distillvo.setCal_startdate(null == vo.getIndistill_date() ? vo
				.getBegindistill_date().asBegin() : vo.getIndistill_date()
		...
		distillvo.setPk_transtype(PfDataCache.getBillTypeInfo("HV8F-0xx-01")
				.getPk_billtypeid());
		// 单据号
	...
	public void cancelCal(LoginContext context, UFDate trade_date)
		...
		try {
			...
			// 查询生成实时凭证的单据
			QueryInterestBaseInfo info = new QueryInterestBaseInfo();
			AggPayableInterestCalVO[] data = info.queryDelVoucherData(context,
					trade_date);
			// 查询相应的回购台账数据
	...
	class BillNoGenerator {
		...
		public BillNoGenerator(int size, FundTallyVO vo)
				throws BusinessException {
			this.size = size;
			this.vo = vo;
			this.init();
		...
		private void init() throws BusinessException {
			try {
				this.lst = PubMethod.getInstance().getBatchBillCodes("HV8F",
						vo.getPk_group(), vo.getPk_org(), null, size);
